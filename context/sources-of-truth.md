...
Smart Bidding vs ActiveView                    Smart Bidding como dashboard principal; ActiveView vence nos sites intencionalmente AV (`openzed` e subdomínios). Wavesbee e finanzas.wavesbee usam JBF/GAM SB. Cliquet e finanzas.cliquet são migração incompleta: wrapper JBF com builders antigos ainda requisitando GAM AV `198073784`; o estado só muda após cutover e readback no GAM SB `21922122164`.
...
A dashboard SB ou de outra rede é fonte auxiliar de diagnóstico e reconciliação: não substituir valores GAM nem mover receita entre verticais apenas por classificação de URL/visita da dashboard sem ligação comprovada com o original. Exceções explicitamente autorizadas continuam restritas ao domínio, período e finalidade aprovados. Para Yolokfx01–09/09, a decisão1547678046553636905 em `docs/gam-revenue-claude-yolo-september-2026.md` supersede a antiga ponte de identidade: outros gestores recebem a receita SB e G002 o complemento diário para o total GAM. Não extrapolar essa exceção. A hipótese de navegação de cartão para empréstimo é explicação possível levantada por Rodolfo, não causalidade comprovada.
...
**País do original e limite de exceções — Rodolfo1547683017462513675:** um site pode operar novos países. Uma atribuição aprovada nesta revisão não autoriza converter automaticamente países em relatórios futuros. Preservar o país que aparecer no CSV/GAM e sua receita, mesmo diferente do país habitual; apresentar divergências sem remapeamento silencioso. Exceções de país desta revisão ficam no domínio/período/conjunto autorizado, não são defaults permanentes para novos arquivos. Se só produto/idioma estiver incerto, manter o país conhecido e sinalizar apenas a classificação faltante. Esta regra limita a aplicação temporal das confirmações anteriores sem apagar o histórico nem mudar automaticamente pipelines produtivos. Detalhes de execução: skill `revenue-spend-reporting-pipeline`, referência `gam-daily-excel-consolidation.md`.
