...
Desde 2026-09-11, a operação usa canais isolados por gestor, como exceção explícita ao padrão geral de canal por operação: `shein-g001` a `shein-g006`. O gestor atribuído pode solicitar e autorizar, no próprio canal e somente sobre suas contas/perfis anunciantes, criação, duplicação, clone, relatórios, organização de threads, otimização, pausa/reativação, exclusão/arquivamento, budget, schedule e ativação de campanhas. Budget exige valor e moeda exatos, pre-read e GET/readback, mas não uma segunda aprovação de Rodolfo. Billing, `account_spend_limit`, credenciais, ownership, permissões de app, pixel/CAPI, WordPress/quiz/SMS/ChatPion e acesso cruzado entre gestores permanecem fora dessa delegação. O contrato vivo é `data/ares/meta-ads/operations/SHEIN-US-DIRECT.json` e o procedimento específico é a skill `direct-traffic-shein-operations`; não herdar regras de CAR/CPV apenas porque ambas as verticais usam tráfego direto.

Fase inicial definida por Rodolfo em 2026-09-11: os gestores focam em criação do zero, duplicação e clone. Não há threads fixas; as seis threads de criação inicialmente abertas pelo Ares foram excluídas por pedido posterior de Rodolfo, com ausência 6/6 validada. Threads de Diário, Intraday/Otimização, Criativos e automações de relatório ficam adiadas até os gestores dominarem o fluxo de criação e Rodolfo avançar a fase. A futura pasta `SHEIN_US_ES` ainda não existe porque o intake espanhol não começou; quando surgir, EN e ES compartilham a mesma operação e diferem somente pela língua, mantendo assets/copy separados por idioma.

Arquitetura de credenciais planejada: um perfil anunciante e um User Access Token por gestor, armazenado no 1Password e referenciado por alias sem expor o segredo. Tokens de usuários distintos podem separar o limite de chamadas por usuário no Graph API, mas a Marketing API também aplica limites compartilhados por conta de anúncios, app, Business Use Case, access tier, bursts de mutação e objeto; separar tokens nunca é tratado como bypass de rate limit.
