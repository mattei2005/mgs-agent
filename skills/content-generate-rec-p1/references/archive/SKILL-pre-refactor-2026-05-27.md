...
- `references/rec-p1-structural-audit-playbook.md` — Zeus read-only audit playbook for recurring Atena REC/P1/REC+P1 quality failures: classify deterministic vs semantic vs subjective errors, distinguish real runner hard gates from inert `.md` references, audit REC↔P1 coupling, classify recent commits by layer, and check why `rec-fingerprint.py` does not catch cross-card/P1 generic copy.
- `references/rec-p1-architecture-cleanup-cache-policy.md` — architecture cleanup policy for REC/P1 reform: consolidate a single vertical contract with REC and P1 separated, treat REC+P1 as orchestration (not a third article type), archive historical references, and remove editorial `card-cache` from production content generation while keeping only technical caches such as WP taxonomy and QA fingerprints.
- `references/rec-p1-active-content-map-before-refactor.md` — required sequencing before REC/P1 architecture edits: create an active content map of current authority/read order first, then extract rules/conflicts, then create the GB-CC-EN contract and trim `SKILL.md`. Also captures Rodolfo's correction that editorial `card-cache` must not feed production content, especially when scaling the same card across many sites.

- `references/rec-p1-runner-retry-and-quality-gate-repair-2026-05-27.md` — REC+P1 runner repair lessons: competitor arguments need JSON with annual fee + benefit, featured semantic-audit failures need bounded `allow_fail=True` retry loops instead of immediate abort/bypass, P1 word-count floor should be fixed by deterministic safe filler, and failed attempts may require scoped orphan-media cleanup.

- `references/post-publication-rec-p1-repair-verification-2026-05-26.md` — post-publication repair checklist for REC+P1 pairs after Rodolfo/Raquel flags live issues: update every LazyBlock in both posts, replace old card media, remove generic comparison/descriptor copy, revalidate exact final bodies, re-score Yoast, verify public cache-busted HTML, and patch/commit systemic runner gates.

- `references/rec-p1-prepublish-featured-audit-failure-cleanup-2026-05-26.md` — pre-publication REC+P1 failure path when the REC runner uploads card media but blocks before post creation because featured-image visual audit fails; do not start P1, clean safe orphan media, and report a concise blocker instead of a success summary.

- `references/rec-p1-card-image-competitor-descriptor-hard-gates-2026-05-26.md
- references/rec-p1-scale-quality-gates-2026-05-27.md — recurring REC/P1 scale gates: horizontal card-only LazyBlock image, <=100-character subtitle/excerpt, REC commercial tone vs P1 deeper strategic tone.` — Rodolfo correction: LazyBlock card images must be card-only with no external background/moldura; REC comparative tables need real competitor cards/facts, never generic placeholders; card `texto` must be commercial and benefit-led, not generic issuer/account wording.

- `references/santander-rec-p1-specificity-and-operation-time-2026-05-27.md` — Santander World Elite correction: final summaries must report total operation time, card UI tags cannot be truncated fragments like `Over 1`, premium REC/P1 copy must name real differentiators, and P1 featured cards must look solid/opaque rather than transparent.

- `references/article-final-summary-format-rodolfo-2026-05-26.md` — Rodolfo-approved final Discord summary format for REC, P1 and REC+P1. Use this exact emoji/line-break/order template after runner completion; replace only the placeholder values with real data. All summary URLs must be rendered in Discord anti-preview form (`<https://...>`) so embeds/image previews do not pollute the thread.
...
- `references/rec-p1-operation-time-and-runner-retry-lessons-2026-05-27.md` — Rodolfo correction: final summaries must report user-perceived `Tempo total da operação`, not only successful runner durations; also captures bounded featured-image semantic-audit retry and P1 word-count expansion lessons from the Santander REC+P1 run.
...
- `references/p1-lloyds-source-card-image-hard-gates-2026-05-24.md` — Lloyds P1 incident: official-source content gate, reader fallback limited to the same official URL, no silent P1 image injection when REC card image is empty, and mandatory crop/normalization even when the selected card artwork is correct.
...
- `references/p1-featured-image-layered-composition-2026-05-22.md` — P1 featured-image correction from Barclaycard Avios Plus: mandatory scenario → card → person layer order, safe-area/card-integrity gates, why card-only overlay compositions fail, and verification checklist before reporting a P1 image repair as complete.
...
### REC/P1 architecture refactor sequencing pitfall

When Rodolfo/Zeus is restructuring REC/P1 architecture, do **not** edit the contract, trim this `SKILL.md`, move references, or remove cache code until an active content map has been produced and approved. First map the current files, who reads them, whether they affect runtime, and their target authority. See `references/rec-p1-active-content-map-before-refactor.md`.
...
- VALIDATE COMPOSITION BEFORE UPLOAD: the featured image must be contextual/lifestyle hero art, not a card-only mockup. It must pass all five gates: (1) exact card identity preserved from the reference, preferably horizontal; (2) a realistic human character/person is present; (3) a contextual scene surrounds the card; (4) the image feels cinematic and humanized; (5) the card reads as a payment card, not a badge/crachá. Reject/regenerate images with no person, product-only still life, generic background, huge isolated card, redesigned/different card art, frames/molduras, decorative panels, duplicated cards, badges, UI overlays, phone screens, or extra composition objects. A visually pleasant image still fails if any mandatory gate is missing.
...
## Step 1c - CACHE LOOKUP (DEPRECATED FOR EDITORIAL PRODUCTION)

Architecture note: this legacy section remains only as historical context until the REC/P1 refactor is implemented. Rodolfo/Zeus decided that editorial `card-cache` must not feed production content because stale benefits, APR, descriptors, tags, images, or copy can propagate mistakes across many sites. Future production REC/P1 flows should fetch current official facts and use only technical caches such as `wp-term-cache.json` and QA fingerprints. See `references/rec-p1-architecture-cleanup-cache-policy.md` and `references/rec-p1-active-content-map-before-refactor.md`.
...
## Step 2.5 - CACHE SAVE (DEPRECATED FOR EDITORIAL PRODUCTION)

Architecture note: do not preserve this as an active production rule during the REC/P1 refactor. Editorial facts should not be saved/reused as production source material. Keep this only as legacy/historical documentation until the cache-removal implementation is complete.
