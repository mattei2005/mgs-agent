...
Active editorial specs/contracts live in:
...
Legacy vertical contract/template files such as `contracts/gb-cc-en.md` and `templates/rec-*.md` may remain on disk for rollback or migration context, but the universal specs are the intended active surface when present.

Important architecture note: these markdown contracts are versioned editorial specs and telemetry/preflight inputs for deterministic Python runners. They are not runtime LLM prompts unless Rodolfo explicitly approves a separate LLM-generation migration.
