...
tmp_list=$(mktemp)
h_list=$(wp_curl_auth_http "$tmp_list" "$user" "$pass" "$wp/wp-json/wp/v2/$TAX?search=$search&per_page=100")
list=$(cat "$tmp_list")
rm -f "$tmp_list"
...
tmp_c=$(mktemp)
h_c=$(wp_curl_auth_http "$tmp_c" "$user" "$pass" -H "Content-Type: application/json" \
  -X POST -d "$body" "$wp/wp-json/wp/v2/$TAX")
resp=$(cat "$tmp_c")
rm -f "$tmp_c"
