...
- Desde a primeira falha de qualquer monitor ou operação, intervir automaticamente: investigar a causa-raiz, aplicar correção segura dentro da autoridade vigente, repetir a operação e validar a recuperação e a redução de recorrência. Rodolfo deve ser notificado somente após a recuperação integral validada ou diante de bloqueio decisório/Critical Subset, com diagnóstico exato; nunca aguardar que ele peça investigação, correção ou retry. Se a correção cair no Critical Subset ou não for segura, escalar imediatamente.
- Falhas de `web_search` ou `web_extract` seguem o mesmo limiar de uma ocorrência e exigem diagnóstico/failover imediato. Credencial, cobrança, Critical Subset e restart do próprio gateway nunca são automatizados; escalar esses bloqueios com diagnóstico exato.
