---
name: wp-deploy-file-without-ssh
description: Deploy or update a file on a WordPress server when SSH/SFTP access is unavailable. Uses the WP Plugin Editor form POST (admin session) to write arbitrary PHP files. Also covers the bootstrap-plugin technique to execute PHP and return data via WP REST settings endpoint.
---

# wp-deploy-file-without-ssh

When the target WP server has SSH blocked on all ports (22, 2222, 22022, 8022)
and SFTP is unavailable, files can be deployed using the WP admin Plugin Editor
...
- Have admin credentials (username + regular password, NOT Application Password)
- WP Plugin Editor is not disabled (check: `DISALLOW_FILE_EDIT` in wp-config.php)

## Critical pitfalls discovered

### 1. Application Passwords do NOT work for admin-ajax or form login
WP Application Passwords (used for REST API) are rejected for:
- Form-based login (`wp-login.php` / custom login URL)
- `admin-ajax.php` nonce-based endpoints

**You need the real user password** (from 1Password `eggbev - WordPress` item,
field `password`, user `rodmaster`) for session-based operations.

### 2. admin-ajax `edit-theme-plugin-file` action — nonce expires fast
The `action: edit-theme-plugin-file` AJAX endpoint uses a nonce that expires
in seconds between GET (editor page) and POST (save). This approach **fails
consistently** due to nonce timing.

**Use the form POST instead** (see step 3 below) — it's the same underlying
WP function but the form nonce lives longer and the redirect confirms success.

### 3. REST `/wp/v2/plugins` POST only accepts wordpress.org slugs
Cannot upload a ZIP or arbitrary plugin via this endpoint. It only installs
from the wordpress.org repository.

### 4. WPS Hide Login — find the custom login URL first
Standard `wp-login.php` returns 404. Get the real slug from 1Password or
the `whl_page` WP option. For eggbev: `/rodloguda/`.

### 5. Plugin activation with `die()` returns HTTP 500
If the injected PHP calls `die()` or `exit()`, the REST activation call
(`POST /wp-json/wp/v2/plugins/{slug}`) returns a 500 with empty body.
This is expected — the PHP ran and died before WP could return JSON.
The execution still happened. Read the result via a separate mechanism
(WP option, see bootstrap technique below).

## Workflow — deploy a file via Plugin Editor form

### Prerequisites
- Admin user with regular password (not Application Password)
- Custom login URL (from 1Password or whl_page option)
- Plugin Editor enabled on target site
...
When you need to run arbitrary PHP on the server and retrieve the result:
...
1. Write PHP that stores result in a WP option:
   ...
   die();   // stops WP response — activation returns 500, but code ran
   ```

2. Base64-encode to avoid PHP string escaping issues:
   ```python
   import base64
...
3. Write + activate (activation returns 500 — that's OK)

4. Read result via REST settings (option must be registered or in `show_in_rest`):
   ```bash
   curl -s -u "$USER:$APP_PASS" "$WP_URL/wp-json/wp/v2/settings" \
     | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('mgs_result_key'))"
   ```
   Note: custom options only appear in /wp-json/wp/v2/settings if registered
   with `show_in_rest: true` via `register_setting()`. Otherwise query DB directly
   via a second bootstrap round.

5. Restore plugin to clean state immediately after.
...
| Plugin used as carrier | `simple-author-box/simple-author-box.php` (inactive, safe to overwrite temporarily) |
