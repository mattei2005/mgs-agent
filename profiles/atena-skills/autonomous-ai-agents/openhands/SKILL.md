...
On the MGS Atena host, the local wrapper is:
...
Before claiming that OpenHands ran a requested audit/task, run a small preflight that checks both the binary and model configuration. Do not treat an installed CLI alone as ready for headless automation.

```
terminal(
  command="command -v openhands && openhands --version && /root/mgs-agent/scripts/run-openhands-atena.sh -t 'Print exactly OPENHANDS_OK and then finish.'",
  workdir="/tmp",
  timeout=120
)
```
