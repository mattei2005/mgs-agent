...
- Renderer v6 inspirado em Corte e ROAS: no fechamento D-1, inclui toda campanha com insight em D-1 e exclui campanhas ACTIVE atuais sem insight em D-1, pois elas não rodaram no período fechado. Em hoje/sob demanda, inclui toda campanha efetivamente `ACTIVE` mesmo sem insight e toda campanha com insight no período. A visão desktop é dividida em grupos de Página/UTM; grupos com nome conciliado são ordenados sempre pelo nome completo da página em ordem decrescente `Z→A`, e grupos sem nome conciliado ficam ao final como UTM/`N/D`. Cada grupo traz o resumo Meta/Smart Bidding e uma tabela compacta das campanhas daquela página.
- A identidade visível da campanha usa a chave determinística `sequência·Cnnn·Dnn`, número da linha e sinal `●/○` de entrega. O nome Meta integral permanece no audit e aparece no alerta quando houver nome duplicado ou divergência naming/UTM. Nenhuma campanha pode ser omitida silenciosamente.
...
6. Diário: renderer v5 mobile-first com quatro blocos compactos Meta/Smart Bidding por página e campanhas com/sem entrega, apoio read-only à clonagem e detector de anomalia de receita por página materializados; permanecem seleção direta vertical/Messenger Pages/domain, formação de baseline e aprovação separada de cron/post.
...
- Visualização Diário v5: quatro blocos compactos mobile-first — Meta por Página/UTM, Smart Bidding/Pricing por Página/UTM, campanhas com entrega e ACTIVE sem entrega — usando UTM exata e confirmação de Page ID. Economia Smart Bidding aparece uma vez por página e nunca é repetida por campanha.
