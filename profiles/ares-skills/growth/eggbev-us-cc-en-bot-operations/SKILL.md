...
### Autoridade de início imediato — decisão de Rodolfo em 31/08/2026

Nicolas tem autoridade permanente nesta operação para instruir e dar o OK final, por pedido, em criação ou clone com início imediato sem schedule, sem nova aprovação do Rodolfo. O padrão continua sendo próximo dia às 00:00 ET quando Nicolas não pedir exceção. Todo pedido imediato exige escopo e OK explícitos, request ID na allowlist do Engine v3, `start_time` atualizado no execute com buffer técnico Meta-safe, pré-leitura e readback exato. A delegação não autoriza schedule recorrente, billing, credencial, mudança de estratégia ou ampliação de escopo.
...
- Renderer unificado v8: no fechamento D-1, inclui toda campanha com insight em D-1 e exclui campanhas ACTIVE atuais sem insight em D-1; em hoje/sob demanda, inclui toda campanha efetivamente `ACTIVE` mesmo sem insight e toda campanha com insight. Abre com três linhas curtas de conta; cada Página/UTM usa somente uma linha de saúde e uma linha compacta com emojis, seguida da tabela com `ROI real` e `ROI est.` por campanha.
- `ROI real` usa `(NET_REVENUE−INVESTIMENT)/INVESTIMENT`; `ROI est.` usa `(estimatedRevenue−INVESTIMENT)/INVESTIMENT`, sempre após match econômico exato `CUSTOMER_ID + DOMAIN + DATE + CAMPAIGN_ID + UTM_ADGROUP`. Hoje pode usar `/estimated/revenue/utm_adgroup` apenas com UTM única; histórico usa `performance_per_campaigns.REVENUE_ESTIMATED` da data exata. Freshness, denominador ou identidade inválidos ficam `N/D`. Se `NET_REVENUE` ou `REVENUE_ESTIMATED` vier zero em todas as linhas econômicas enquanto a página possui receita, tratar como cobertura zero-only: `N/D`, nunca `-100%`.
- Todo pedido de relatório executa novas leituras Smart Bidding das rotas aplicáveis e valida `/estimated/delay`; mensagens, audits e snapshots anteriores nunca são reutilizados como valores atuais.
- Cada campanha recebe alias determinístico `SRC-{sequência-Cnnn-Dnn}-{digest curto}` persistido no audit e utilizável apenas como referência de intake na thread Clonar Campanhas. `🧬` indica Nome/UTM/Página conciliados; `🟡`, cobertura incompleta; `⚠️`, nome duplicado ou divergência. O alias nunca executa clone nem prova elegibilidade; todo pedido exige preflight vivo. `Msg` é exclusivamente `messaging_conversation_started_7d`, e ROI/RPS de página nunca são apresentados como ROI individual da campanha.
- Antes de renderizar qualquer período, consultar a dash do dia atual via `/report/messenger`. Exibir `BD_REVENUE` como `BC agora` no cabeçalho de cada Página/UTM, inclusive em relatórios D-1, com data e freshness da leitura atual. Sem leitura válida, mostrar `N/D`; zero só pode aparecer quando a fonte viva retornar zero.
- Smart Bidding econômico de página aparece uma única vez no cabeçalho da Página/UTM. Investimento, receita, broadcast atual, drip, leads, RPS e EPC nunca são repetidos nem somados entre campanhas. A única exceção por linha são `ROI real` e `ROI est.`, calculados somente após join econômico exato campanha+UTM e freshness válida.
- Os grupos são braço read-only da clonagem: dão visibilidade por página e por campanha para o gestor decidir quais e quantas campanhas clonar, mas nunca executam clone nem transformam ordenação visual em elegibilidade automática.
...
- Abaixo dos grupos, mostrar no máximo cinco bullets `Fique de olho`. Queda comprovada identifica Page/UTM, valor atual, mediana, percentual e amostras, sugerindo conferência de disparo, bloco/funil e entrega com o responsável; Ares não configura essas camadas nesta rota.
- Não há limite silencioso de linhas. As tabelas dentro dos grupos de Página usam chave semântica compacta, nunca corte arbitrário; o nome Meta integral fica no audit e aparece em alertas de ambiguidade/divergência. Como em Corte e ROAS, cada parte aceita no máximo dez campanhas, repete o cabeçalho e mostra `parte/total`; a paginação Discord permanece fence-safe.
- O bloco por campanha combina métricas Meta Ads de decisão/diagnóstico com somente `ROI real` e `ROI est.` da Smart Bidding quando houver match econômico exato. A restante economia Smart Bidding/Pricing fica no cabeçalho separado da Página/UTM. O join de identidade exige `utm_campaign` do creative Meta = `UTM_CAMPAIGN` Smart Bidding e `object_story_spec.page_id` Meta = `FB_PAGE_ID` Smart Bidding; o join econômico adiciona `CUSTOMER_ID + DOMAIN + DATE + CAMPAIGN_ID + UTM_ADGROUP`.
- Campos por campanha: sinal `●/○`, chave `sequência·Cnnn·Dnn`, status, Budget, spend, `messaging_conversation_started_7d`, custo por mensagem iniciada, Purchase ROAS, `ROI real`, `ROI est.`, CPM e CTR. Os ROIs usam Smart Bidding no match exato; campanhas ACTIVE sem entrega ou sem fonte econômica válida permanecem visíveis com `N/D`.
- Campos Smart Bidding/Pricing por Página/UTM: investimento, receita do período, `BC agora` vindo de `/report/messenger.BD_REVENUE`, drip, ROI, LEADS, RPS e EPC quando expostos pela rota direta compatível. Fórmula local é apenas fallback rotulado.
- Freshness Smart Bidding aparece com timestamp/idade/campo ou `N/D`; sem timestamp ou acima de 2h, todas as métricas externas por Página/UTM permanecem `N/D`, nunca zero.
- UTM ausente/múltipla, página ausente/duplicada, Page ID divergente ou fonte stale falha fechado e aparece em `Fique de olho`/audit; a tabela não inventa join nem valor.
...
- ROI atual e ROI estimado do **Corte e ROAS** espelham exatamente a tela Smart Bidding **Messenger Pages**. ROI atual usa `REVENUE` e `INVESTIMENT` de `/report/messenger`: `(REVENUE−INVESTIMENT) / INVESTIMENT`. No dia ET atual, ROI estimado usa `estimatedRevenue` de `/estimated/revenue/utm_adgroup`, reconciliado por `COMPANY_DOMAIN + UTM_CAMPAIGN`, com o mesmo denominador. Em período histórico, a estimativa futura não é aplicada e permanece `N/D`, reproduzindo o frontend.
- RPS e CPM bloco do **Corte e ROAS** usam, respectivamente, `REVENUE×1.000/SESSIONS` e `REVENUE×1.000/GAM_IMPRESSIONS` da linha Messenger Pages. São métricas Página/UTM report-only; quando exibidas em várias campanhas da mesma Página/UTM, repetem o mesmo contexto da página e nunca são somadas nem tratadas como economia individual da campanha.
- A composição econômica canônica do Corte e ROAS é `/campaigns/Messenger` para identidade/Page ID, `/report/messenger` para economia atual, `/estimated/revenue/utm_adgroup` para receita futura do dia atual e `/estimated/delay` para freshness. O relatório espelha o readback da Smart Bidding e registra freshness; Meta writes continuam fail-closed quando a fonte estiver stale ou irreconciliável.
...
1. Smart Bidding: Corte e ROAS está materializado em paridade Messenger Pages v25 com `/report/messenger` + `/estimated/revenue/utm_adgroup`, match `COMPANY_DOMAIN + UTM_CAMPAIGN`, período atual/histórico idêntico ao frontend e `4/4` páginas live sem divergência. Permanecem pendentes somente o timestamp verificável da rota Messenger para o guardrail de LEADS e a seleção multi-rota específica do Diário.
2. Engine v3: conta Eggbev cadastrada na release 3.4.1; `from_zero_prestaged`, `pure_clone` e `clone_prestaged` de 1–5 ads permanecem habilitados. O schema/planner de `clone_page_switch` existe apenas para compatibilidade/auditoria, mas a allowlist da conta o bloqueia na account prevalidation antes de qualquer write. Mídia nova continua pre-stageada sob demanda antes de selar o manifest.
...
4. `clone_page_switch`: o incidente de 31/08/2026 foi recuperado após autorização explícita de Nicolas. A campanha-shell foi reutilizada, um novo AdG com a Page alvo foi criado e o AdG incorreto ficou PAUSED. A Meta rejeitou a troca de `promoted_object.page_id` no AdG copiado (`1885090`), `/ad copies` entre PBIAs de Pages diferentes (`2238280`) e o create do novo AdG com `explore_home` (`2490392`). Os três ads diretos corretos chegaram a ACTIVE/sem issues, mas permaneceram com zero spend/impressões em 16 checks. A recuperação final copiou cada ad já validado dentro do mesmo AdG/Page com `creative_parameters`, ativou os três copies com `source_ad_id` e pausou os três diretos. Insights confirmou a entrada em serving após cinco checks: USD 36,20, 606 impressões e 25 cliques. Não houve delete, budget ou billing. Para impedir nova shell sem entrega, `clone_page_switch` permanece fora da allowlist `supported_modes`; novos manifests falham antes de qualquer write até uma arquitetura global determinística aprovada por Rodolfo representar a rota completa.
5. ROAS: comando aprovado de alteração intraday e eventual fórmula de recomendação de threshold.
6. Diário: renderer unificado v8 com blocos Página/UTM em duas linhas, aliases `SRC`, sinais de integridade e colunas de ROI real/estimado por join econômico exato materializados; permanecem seleção direta vertical/Messenger Pages/domain, formação de baseline e aprovação separada de cron/post.
7. Canário live: o recovery `clone_page_switch` de 31/08/2026 concluiu serving real com campanha/AdG/três lineage ads ACTIVE, zero issues e Insights de USD 36,20 / 606 impressões / 25 cliques. Isso valida somente a correção scoped; não reabre o modo bloqueado nem substitui a futura conferência visual de `Template name=JSON-AGT` na primeira campanha da política prospectiva.
...
O simulador read-only canônico é `python3 scripts/ares-eggbev-creation-intake-simulate.py`. O runner real é `python3 scripts/ares-eggbev-creation.py`: reserva assets somente após pedido scoped, faz pre-stage resumível registry-first, sela manifest, mostra resumo+digest e delega writes ao Engine v3. **Antes de selar qualquer manifest from-zero, o scan live deve exigir zero colisões exatas entre cada nome canônico pretendido e todas as campanhas não deletadas; audit/checkpoint nunca pode converter dois candidatos live com o mesmo nome em “fonte canônica única”. Depois de cada bundle, o readback deve comprovar exatamente um campaign ID por nome pretendido. Campanha homônima fora do checkpoint é incidente de duplicação: interromper bundles seguintes, reconciliar spend/status/atividade pelo ID real, bloquear novos writes e obter a decisão do gestor para pausa ou remoção; nunca escolher silenciosamente uma das duas.** O JSON Messenger fixo vive em `data/ares/meta-ads/templates/eggbev-us-cc-en-messenger-welcome.json`, com identidade semântica `ecc2204e5f94203434a212737bb0110ed3d53780478a701c80809d0807f819ad`; cada novo creative deve carregá-lo. Arquivo ausente, inválido ou semanticamente divergente bloqueia antes do write. Depois do readback completo, verifica por GET cada creative criado: Page e `url_tags` devem corresponder ao manifest, enquanto o JSON parseado em `asset_feed_spec.additional_data.page_welcome_message` deve corresponder diretamente ao arquivo canônico. Só então move o tratado `01_READY → 02_TESTING` e registra IDs/linhagem; divergência fica `POSTPROCESS_PENDING`, preserva IDs e nunca repete a criação. Falha geral fica `RECOVERY_PENDING` ou `POSTPROCESS_PENDING`, nunca repete POST cegamente.
...
O início padrão continua no dia seguinte às 00:00 `America/New_York`. Por decisão de Rodolfo em 31/08/2026, Nicolas pode instruir e dar o OK final para início imediato por request, sem aprovação adicional do Rodolfo; o request ID precisa entrar na allowlist canônica e o execute atualiza o início para o horário corrente ET com buffer técnico mínimo da Meta. O primeiro request histórico desta rota foi `eggbev-pg-5024-20260830-nicolas-01`. Nunca converter essa autoridade por pedido em schedule recorrente ou aplicar início imediato quando Nicolas não o solicitar explicitamente.
...
- Visualização Diário v8: três linhas curtas de conta; cada Página/UTM abre com uma linha de saúde e uma linha econômica com emojis, seguida da tabela compacta por campanha com alias `SRC`, entrega, status, Budget, Spend, Msg, custo/msg, ROAS, `ROI real`, `ROI est.`, CPM e CTR. Os ROIs de campanha exigem match econômico exato e freshness; Smart Bidding da página continua uma única vez, com receita total, Drip e Broadcast separados.
...
- Fonte canônica do Corte e ROAS: a composição exata de **Messenger Pages** — `/campaigns/Messenger` para identidade, `/report/messenger` para `REVENUE`/`INVESTIMENT`, `/estimated/revenue/utm_adgroup` para a estimativa do dia atual e `/estimated/delay` para freshness. O match da estimativa é `COMPANY_DOMAIN + UTM_CAMPAIGN`; nenhum valor econômico é derivado de `/report/performance_per_campaigns` para as colunas ROI atual/estimado, RPS ou CPM bloco.
- `Custo/msg iniciada` = spend Meta ÷ `messaging_conversation_started_7d`. ROI atual = `(REVENUE−INVESTIMENT)/INVESTIMENT`; ROI estimado do dia atual = `(estimatedRevenue−INVESTIMENT)/INVESTIMENT`. São métricas Página/UTM report-only e não substituem Meta Purchase ROAS.
...
- `clone_page_switch`: preserva estrutura, público, placements, estratégia, copy e mídia, mas troca Facebook Page, `pg_XXXXX`, links/UTMs, JSON Messenger **e o nome visível da campanha**. A página deve ser indicada por Nicolas. Se o pedido omitir a Page/`pg_XXXXX`, pausar o intake e perguntar qual página exata será usada; nunca inferir nem selecionar automaticamente. Uma escolha delegada só pode ocorrer após nova instrução explícita de Nicolas no próprio pedido. Sem Page exata, não selar manifest e não fazer write Meta.

Naming obrigatório por modo:

- `pure_clone` e `clone_prestaged`: preservar o nome-base integral e adicionar o próximo `DUPnn` livre após scan das campanhas não deletadas;
- `clone_page_switch`: renomear automaticamente para `[sequência da Page alvo] - [nome exato da Page alvo] - ENG - US - ([pg_XXXXX alvo]) [Cnnn da fonte] [próximo DUPnn livre]`. Substituir sequência/Page/pg da fonte e preservar somente o `Cnnn`. A sequência da Page alvo vem do registry/naming reconciliado, o nome vem do GET Meta e o `pg_XXXXX` do match Smart Bidding `UTM_CAMPAIGN + FB_PAGE_ID`. Sem qualquer componente comprovado, falhar antes de selar o manifest. O helper canônico é `ares_campaign_v3.eggbev_clone.page_switch_campaign_name`.

Exemplo: `165 - Tina Walter - ENG - US - (pg_5071) C003 DUP02` → `162 - Amy Shook - ENG - US - (pg_5024) C003 DUP03`.
...
- No `pure_clone` shallow, o GET normalizado do creative pode omitir `asset_feed_spec.call_to_actions`; copiar esse payload incompleto para AdG Messenger falha com `1815675`. Fazer readback dos shells e anúncios, preservar os IDs existentes e o payload live de vídeos/títulos/labels/Page/UTM/JSON, remover `reasons_to_shop`/`shops_bundle`, restaurar do manifest fonte apenas o `call_to_actions` canônico com `app_destination=MESSENGER` e criar somente os ads comprovadamente ausentes. Preservar o digest inicial e registrar qualquer digest de recovery; nunca recriar campanhas/AdGs já conciliados.
...
- O incidente parcial de `clone_page_switch` foi recuperado somente após autorização explícita de Nicolas para mudar o método: reutilizar a campanha-shell, criar novo AdG já com a Page alvo e pausar o AdG antigo. A tentativa de `/ad copies` diretamente dos ads da Page fonte falhou por PBIA conflitante (`2238280`); creatives/ads diretos com o payload alvo validado ficaram ACTIVE sem issues, porém em zero spend/impressões por 16 checks. A recuperação final fez `/copies` dos próprios ads já rematerializados para a mesma Page/AdG usando `creative_parameters`, validou Page/UTM/Messenger/media, ativou os três copies com lineage e pausou os três diretos. Insights confirmou serving com USD 36,20, 606 impressões e 25 cliques. Esse precedente é somente recovery scoped e não reabre o modo: novos `clone_page_switch` da conta falham na prevalidation até arquitetura global aprovada por Rodolfo.
...
A regra de naming ativa é a seção “Naming obrigatório por modo” acima; nunca aplicar o padrão de preservação do nome-base a `clone_page_switch`.

Gestores autorizados escolhem e confirmam o budget diário de cada duplicação. Para Nicolas, a autorização permanente concedida por Rodolfo em 30/08/2026 cobre aumentar ou reduzir o budget Eggbev sem novo OK de Rodolfo; valor exato, pré-leitura e readback Meta continuam obrigatórios. Billing, `account_spend_limit`, credenciais e escala automática continuam separados. Default de produção: campanha, ad set e todos os ads `ACTIVE`, com início no dia seguinte às 00:00 `America/New_York`; por decisão de Rodolfo em 31/08/2026, Nicolas pode aprovar início imediato sem schedule por pedido, sujeito à allowlist do request, buffer Meta-safe e readback. `PAUSED` somente para canário técnico explicitamente pedido.

A conta `1034081997659047` está cadastrada no Engine v3 release 3.4.1. `pure_clone` não exige mídia nova no registry; `clone_prestaged` continua exigindo pre-stage dos assets do pedido. `clone_page_switch` permanece fora da allowlist desta conta e não pode ser selado/executado em write enquanto a arquitetura determinística não for aprovada e liberada. Pedidos genéricos de “dup” iniciam perguntas curtas apenas para campos ausentes: modo, quantidade de duplicações e budget; depois, assets/copy ou Page/UTM/JSON conforme o modo. Prompt canônico: `data/ares/discord/thread-prompts/1543333373945053184.txt`. Relatório determinístico: `python3 scripts/ares-eggbev-clone-config-report.py --check`.
...
- `ROI atual`: valor de `roi_real`, calculado por `(REVENUE − INVESTIMENT) / INVESTIMENT × 100` com os campos da linha `/report/messenger` exatamente como Messenger Pages;
- `ROI estimado`: valor de `roi_estimated`, calculado no dia ET atual por `(estimatedRevenue − INVESTIMENT) / INVESTIMENT × 100`, após match `COMPANY_DOMAIN + UTM_CAMPAIGN` em `/estimated/revenue/utm_adgroup`. Períodos históricos ficam `N/D`.

Ambas são Página/UTM informativas. Campanhas da mesma Página/UTM podem repetir o mesmo valor no layout congelado, mas o relatório nunca soma nem descreve esse valor como economia individual da campanha. Meta Purchase ROAS continua sendo a única métrica de decisão do ciclo.
...
Por confirmação de Nicolas em `2026-08-30` e correção de paridade em `2026-08-31`, `R/E` mostra sempre dois sinais nesta ordem: **ROI atual / ROI estimado futuro**. O Ares reproduz a composição da tela **Messenger Pages**: `/campaigns/Messenger` para identidade/Page ID, `/report/messenger` para `REVENUE` e `INVESTIMENT`, `/estimated/revenue/utm_adgroup` para `estimatedRevenue` do dia atual e `/estimated/delay` para freshness. O match estimado é exato por `COMPANY_DOMAIN + UTM_CAMPAIGN`; não usa `CAMPAIGN_ID` nem `NET_REVENUE`. Ares consulta a API, não raspa a interface. O valor é Página/UTM e pode se repetir nas campanhas conciliadas da mesma página sem ser somado ou atribuído individualmente.
...
### Política v25 — paridade exata com Smart Bidding Messenger Pages

Por instrução explícita de Nicolas em `2026-08-31`, Smart Bidding Messenger Pages é a fonte de verdade de todos os relatórios **Corte e ROAS**. O pipeline antigo que usava `NET_REVENUE` de `/report/performance_per_campaigns` foi supersedido para essas colunas.

- ROI atual: `REVENUE` e `INVESTIMENT` da linha `/report/messenger`, fórmula `(REVENUE−INVESTIMENT)×100/INVESTIMENT`.
- ROI estimado: somente no dia ET atual, `estimatedRevenue` de `/estimated/revenue/utm_adgroup`, match exato `COMPANY_DOMAIN + UTM_CAMPAIGN`, fórmula `(estimatedRevenue−INVESTIMENT)×100/INVESTIMENT`.
- RPS e CPM bloco: `REVENUE×1.000/SESSIONS` e `REVENUE×1.000/GAM_IMPRESSIONS` da mesma linha Messenger Pages.
- Granularidade: Página/UTM; o layout congelado pode repetir o valor nas campanhas reconciliadas da mesma página, mas nunca soma nem atribui essa economia à campanha.
- Histórico: não aplica a estimativa live; mostra `N/D` como o frontend.
