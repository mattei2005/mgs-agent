...
- Renderer unificado v7: no fechamento D-1, inclui toda campanha com insight em D-1 e exclui campanhas ACTIVE atuais sem insight em D-1; em hoje/sob demanda, inclui toda campanha efetivamente `ACTIVE` mesmo sem insight e toda campanha com insight. Abre com três linhas curtas de conciliação/totais, Msg+Drip+Broadcast e freshness; depois agrupa Página/UTM por nome `Z→A`, com economia externa uma única vez por página e tabela compacta das campanhas.
...
- Smart Bidding econômico aparece uma única vez no cabeçalho da Página/UTM. Investimento, receita, broadcast atual, drip, ROI, leads, RPS e EPC nunca são repetidos em cada campanha nem somados entre linhas da mesma página.
...
- O bloco por campanha combina somente métricas Meta Ads de decisão e diagnóstico. A economia Smart Bidding/Pricing fica no cabeçalho separado da Página/UTM. O join exige `utm_campaign` do creative Meta = `UTM_CAMPAIGN` Smart Bidding e `object_story_spec.page_id` Meta = `FB_PAGE_ID` Smart Bidding. Quando a Smart Bidding expuser economia somente no nível Página/UTM, os totais aparecem uma única vez no cabeçalho da página; nunca somar essas métricas entre campanhas.
- Campos Meta por campanha: sinal `●/○` de entrega, chave `sequência·Cnnn·Dnn`, status, Budget, spend, `messaging_conversation_started_7d`, custo por mensagem iniciada, Purchase ROAS, CPM e CTR. Campanhas ACTIVE sem entrega permanecem no próprio grupo da página com sinal `○` e métricas do período `N/D`.
...
6. Diário: renderer unificado v7 com resumo de conta em três linhas, economia por Página/UTM, aliases `SRC` para intake de clone e sinais visíveis de integridade Nome×UTM×Página materializados; permanecem seleção direta vertical/Messenger Pages/domain, formação de baseline e aprovação separada de cron/post.
...
- Visualização Diário v7: três linhas curtas de conta seguidas de blocos Página/UTM e tabelas compactas por campanha com alias `SRC`, entrega, status, Budget, Spend, Msg, custo/msg, ROAS, CPM e CTR. Smart Bidding aparece uma vez por página, com receita total, Drip e Broadcast separados; ROI/RPS de página nunca são atribuídos à campanha.
