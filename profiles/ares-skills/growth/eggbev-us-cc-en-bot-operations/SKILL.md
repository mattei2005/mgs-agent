...
7. receber/pedir o consolidado na thread Diário — desenho 08:00 ET com D-1 completo, sinal atual 08:00, quatro blocos compactos mobile-first e alertas curtos; cron/post continuam pendentes de enablement separado;
...
- Renderer v5 mobile-first: no fechamento D-1, inclui toda campanha com insight em D-1 e exclui campanhas ACTIVE atuais sem insight em D-1, pois elas não rodaram no período fechado. Em hoje/sob demanda, inclui toda campanha efetivamente `ACTIVE` mesmo sem insight e toda campanha com insight no período. Divide a visualização em quatro blocos compactos: `Páginas — Meta Ads`, `Páginas — Smart Bidding`, `Campanhas com entrega` e `ACTIVE sem entrega`; não usa cards por campanha nem uma linha horizontal gigante.
- A identidade visível da campanha usa a chave determinística `sequência·Cnnn·Dnn`, número da linha e Página/UTM compacta. O nome Meta integral permanece no audit e aparece no alerta quando houver nome duplicado ou divergência naming/UTM. Nenhuma campanha pode ser omitida silenciosamente.
- Smart Bidding econômico aparece uma única vez por Página/UTM. Investimento, receita, broadcast, drip, ROI, leads, RPS e EPC nunca são repetidos em cada campanha nem somados entre linhas da mesma página.
- Os blocos são braço read-only da clonagem: dão visibilidade por página e por campanha para o gestor decidir quais e quantas campanhas clonar, mas nunca executam clone nem transformam ordenação visual em elegibilidade automática.
...
- Abaixo dos blocos, mostrar no máximo cinco bullets `Fique de olho`. Queda comprovada identifica Page/UTM, valor atual, mediana, percentual e amostras, sugerindo conferência de disparo, bloco/funil e entrega com o responsável; Ares não configura essas camadas nesta rota.
- Não há limite silencioso de linhas. A tabela de campanhas usa chave semântica compacta, nunca corte arbitrário; o nome Meta integral fica no audit e aparece em alertas de ambiguidade/divergência. A paginação Discord permanece fence-safe.
- O bloco por campanha combina somente métricas Meta Ads de decisão e diagnóstico. A economia Smart Bidding/Pricing fica no bloco separado por Página/UTM. O join exige `utm_campaign` do creative Meta = `UTM_CAMPAIGN` Smart Bidding e `object_story_spec.page_id` Meta = `FB_PAGE_ID` Smart Bidding. Quando a Smart Bidding expuser economia somente no nível Página/UTM, os totais de investimento, receita, broadcast, drip, ROI e leads aparecem uma única vez na linha da página; nunca somar essas células entre campanhas nem descrevê-las como valores exclusivos da campanha.
- Campos Meta por campanha com entrega: chave `sequência·Cnnn·Dnn`, Página/UTM compacta, status, Budget, spend, `messaging_conversation_started_7d`, custo por mensagem iniciada, Purchase ROAS, CPM e CTR. Campanhas ACTIVE sem entrega ficam em bloco separado com chave, Página/UTM, `start_time` ET, Budget e status.
- Campos Smart Bidding/Pricing por Página/UTM: investimento, receita, broadcast, drip, ROI, LEADS, RPS e EPC quando expostos pela rota direta compatível. Fórmula local é apenas fallback rotulado.
