...
4. `clone_page_switch`: o incidente de 31/08/2026 foi recuperado após autorização explícita de Nicolas: a campanha-shell foi reutilizada, um novo AdG com a Page alvo e três creatives/ads diretos foram criados, o AdG incorreto ficou PAUSED e a hierarquia correta ficou ACTIVE sem delete, budget ou billing. A Meta rejeitou tanto a troca de `promoted_object.page_id` no AdG copiado (`1885090`) quanto `/ad copies` entre PBIAs de Pages diferentes (`2238280`); o create do novo AdG também exigiu remover `explore_home` após `2490392`. Para impedir nova shell sem entrega, `clone_page_switch` foi removido da allowlist `supported_modes` desta conta: novos manifests falham antes de qualquer write. A arquitetura determinística global target-AdG/direct-ad permanece sujeita à aprovação estrutural de Rodolfo.
...
- O incidente parcial de `clone_page_switch` foi recuperado somente após autorização explícita de Nicolas para mudar o método: reutilizar a campanha-shell, criar novo AdG já com a Page alvo, pausar o AdG antigo e tentar rematerializar ads com Page/UTM/JSON. A tentativa de `/ad copies` falhou por PBIA conflitante (`2238280`), então a recuperação scoped usou creatives/ads diretos com o payload aprovado e readback completo; não houve delete. Esse precedente não reabre o modo: novos `clone_page_switch` da conta falham na prevalidation porque o modo saiu da allowlist até arquitetura global aprovada por Rodolfo.
