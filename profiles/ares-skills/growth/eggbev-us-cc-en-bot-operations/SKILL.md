...
- A tabela única combina Meta Ads + Smart Bidding + Pricing/monetização por campanha. O join exige `utm_campaign` do creative Meta = `UTM_CAMPAIGN` Smart Bidding e `object_story_spec.page_id` Meta = `FB_PAGE_ID` Smart Bidding. Quando a Smart Bidding expuser economia somente no nível Página/UTM, os mesmos totais de investimento, receita, broadcast, drip, ROI e leads podem aparecer em várias campanhas da página; nunca somar essas células entre linhas nem descrevê-las como valores exclusivos da campanha.
