...
A família canônica é `direct_traffic_car`; “CPV” identifica apenas o consumidor histórico Creditoparaveiculo, não o nicho nem o executor. Ela funciona como overlay de financiamento veicular sobre a família-base `direct_traffic_web_cbo`, registrada em `data/ares/meta-ads/strategy-families/direct-traffic-web-cbo.json`. O contrato CAR fica em `data/ares/meta-ads/strategy-families/direct-traffic-car.json`, seus consumidores em `direct-traffic-car-consumers.json` e os parâmetros account-scoped em `direct-traffic-car-account-profiles.json`. O builder mantém a conta 13 como default retrocompatível, mas toda outra conta — inclusive a 05 com wrapper `b01fb05` — exige profile explícito; nunca corrigir account, Page, UTM ou operação por patch ad hoc dentro do pedido. Preserve os nomes de arquivos/rotas antigos como compatibilidade e onboarde cada novo domínio por contrato próprio, sem herdar conta, evento, budget, schedule, Page, UTM, estado ou relatório de outro consumidor.
...
2. Selecionar seis criativos elegíveis por bundle de duas campanhas e exigir IDs Meta vertical/square `ready` no media registry v3. As Pages autorizadas de SHEIN/CAR podem estar atribuídas no Business Manager; ainda assim, validar Page e `ADVERTISE` ao vivo antes do write. O upload da mídia ocorre somente depois que o pedido resolve a conta exata e usa três lanes paralelas por asset; reconciliar títulos uma vez, reutilizar uma variante parcial, enviar somente a ausente e executar um único wait-ready + association readback para a coorte antes do registry commit. Nunca pre-stagear para contas inferidas ou múltiplas.
...
Os monitores de alterações externas das contas 13 e 05 rodam de forma account-scoped a cada cinco minutos sobre `act_<ACCOUNT_ID>/activities` em Graph v26.0. Eles alertam mudanças materiais manuais, Meta/native-rule ou de app confiável sem audit Ares correspondente — status, budget, lance, público, programação, regras, campanha, conjunto e anúncio — e nunca desfazem/corrigem automaticamente. São rotinas distintas dos watchers de primeiro gasto. Na conta 05, alertas genuínos ficam na thread de Criação `1542892971186454719`, nunca na Intraday; estado, audit e lock são separados da conta 13. A correlação de writes do Ares exige o par real `actor_id + application_id` do token/app e procura evidência tanto no audit geral quanto no audit/checkpoint do Campaign Engine v3. O nome do ator retornado pela Meta identifica o proprietário do token, não o processo que originou a requisição; autoria operacional depende do request/audit local correlacionado. Eventos do próprio Ares com audit correspondente são silenciados.
