...
def validate(url: str, conversion_event: str | None = None) -> dict:
    ...
    if not campaign:
        errors.append("utm_campaign must match bNNfbNNcNN")

    adgroup = ADGROUP_RE.fullmatch(values["utm_adgroup"])
    if not adgroup:
        errors.append("utm_adgroup must match bNNfbNNcNNgNN")
...
def build(args) -> str:
    ...
    campaign = f"b{two_digit(args.bm, 'bm')}fb{two_digit(args.account, 'account')}c{two_digit(args.campaign, 'campaign')}"
