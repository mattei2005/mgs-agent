...
`data/ares/meta-ads/policies/global-budget-limit-policy.json` prevalece sobre caps, pisos, envelopes, pools e tetos locais. Enquanto estiver `INACTIVE_UNTIL_EXPLICIT_REACTIVATION`, esses valores não bloqueiam nem reduzem campanha, lote, escala ou reativação explicitamente autorizados. Continuam obrigatórios: budget exato, autoridade para o write, pre-read, readback, saúde da conta e todos os gates não financeiros. Billing e `account_spend_limit` não foram liberados; automatic scaling continua exigindo política própria ativa. Valores antigos permanecem históricos.
...
- Em `autonomous_guarded`, executar somente ações e tiers nomeados na configuração da operação, sobre IDs imutáveis explicitamente allowlisted. Fazer plano completo antes do write, somar todo budget CBO configurado-ativo da conta em centavos inteiros e validar conta/moeda/fuso/saúde/hierarquia. Com a política global vigente, o total live é evidência/reporting e não cap de execução; o lote autorizado não é reduzido por envelope interno. Isso nunca altera billing ou `account_spend_limit`.
...
- Na rotina diária CPV, os valores históricos USD500/20%/30%/USD150 não são gates enquanto a política global estiver inativa. Toda campanha nova mantém o budget explícito do pedido — USD25 quando esse for o valor solicitado ou o padrão autorizado — e campanhas existentes não mudam retroativamente. Iniciar o ciclo às 17:00 São Paulo e preparar entre 17:00–23:30. A estratégia padrão autorizada é `MAXVOL` (`LOWEST_COST_WITHOUT_CAP`) até nova decisão explícita. A data exibida no nome é sempre a **data de entrega/início**, não a data de criação. A rota alvo é agendamento nativo às 00:30 do dia seguinte com GET; nunca aceitar horário atual herdado. Testes pedidos como desativados permanecem PAUSED sem agendamento até aprovação.
...
- Quando Rodolfo pedir clone na rotina CPV, selecionar no preflight a campanha elegível de maior ROI Smart Bidding da data operacional dentro do mesmo tipo de veículo. O pedido natural escolhe uma de três rotas sem ambiguidade: (1) `criar do zero com criativos novos` = campanha/adset/creative/post/mídia novos, podendo carregar apenas `source_ad_id` nos anúncios quando a conta exigir lineage para serving; (2) `clonar com criativos novos` = clone da fonte seguido de três creatives/posts novos do Drive; (3) `duplicar igual` = copiar a campanha sem trocar mídia, copy, estrutura, público, estratégia ou budget. Na duplicação, usar o próximo número sequencial, preservar a URL base, reescrever `utm_campaign`/`utm_adgroup` para o novo `cN/gNN` e terminar o nome com `C{fonte:03d}-DUP{ordem:03d}`. A ordem vai de `001` até `N` dentro do pedido, e todas as duplicações apontam diretamente para a campanha fonte informada. Exemplo: cinco duplicações de C37 usam `C037-DUP001` até `C037-DUP005`. Alterar `url_tags` rematerializa o Creative ID; auditar novos IDs e confirmar mídia/copy, UTMs, `source_ad_id` e igualdade de `effective_object_story_id`. `MOTO→CARRO` e `CARRO→MOTO` são proibidos. Nenhum ID vira template fixo. Persistir modo, fonte, número-fonte, número-destino e evidência antes do manifest. Replacement sequencial valida as novas campanhas PAUSED antes de deletar/readback as antigas uma por uma.
...
- Em toda conclusão de criação programada CPV, informar budget ativo da conta, delta exato do pedido, moeda USD e fonte; não informar saldo/cap interno enquanto a política global estiver inativa. Após qualquer erro, diagnosticar, fazer readback, reconciliar efeitos parciais, corrigir somente a camada faltante do mesmo request autorizado e continuar até concluir. Nunca repetir POST não idempotente às cegas nem ampliar escopo/budget.
...
12. **Tratar HTTP 200 sem ID como sucesso.** Em write não idempotente, `success=false`, payload com `error` ou ausência de ID não confirma criação. Em Graph batch, o child pode retornar HTTP `200` e ainda carregar `body.error`; o transporte deve elevar esse erro Meta sanitizado pelo `code/subcode` em vez de falhar depois apenas por ausência de `copied_*_id`. Fazer GET por nome exato + linhagem antes de qualquer retry; se nada aparecer, classificar como falha. `execution_options=[validate_only]` retorna `success=true` sem ID por definição e nunca prova write real.
...
14. **Confundir autenticação por código, confirmação do anúncio e superfície pública da API no `31/3858385`.** O código por e-mail/SMS e o checkbox **`Confio nesse anúncio e ele está correto.`** são etapas semanticamente distintas: a segunda é um acknowledgement pós-autenticação no Ads Manager. Não afirmar novamente que “a Meta não tem rota alguma”. A investigação de 05/09/2026 encontrou no codegen oficial o campo interno/read-only `AdAccountGet.auth_flow_for_trust_tier_state`, com estados `NOT_STARTED`, `PENDING`, `IN_REVIEW`, `VERIFIED`, `FAILED`, `EXPIRED` etc.; porém o `AdAccount` público, `POST /act_{id}/ads`, `POST /{ad_id}/copies` e `POST /act_{id}` não expõem parâmetro/mutação de trust acknowledgement, e o GET live desse campo pelo app MGS retornou `code=100`/campo inexistente. A conclusão correta é: existe estado/rota interna do Ads Manager, provavelmente GraphQL dependente da sessão web, mas nenhuma rota **pública e documentada** da Marketing API foi encontrada para marcar o checkbox. Para identificar a mutação exata, observar o Network de uma sessão Ads Manager autenticada no instante do clique/publicação, removendo cookies, `access_token`, `fb_dtsg`, `lsd` e cabeçalhos sensíveis; nunca tratar endpoint interno/reverso como contrato público nem reproduzi-lo automaticamente. Automação visual só pode clicar quando o Ares possuir uma sessão autenticada própria para o mesmo perfil/conta. O readback de Network de 05/09/2026 identificou a consulta interna `POST https://adsmanager.facebook.com/ads/bi/user_ad_publish_challenge_info/`; ela recebe conta + estado de sessão/CSRF/build/telemetria e não mostrou parâmetro explícito `trust/confirm/ack`. Sua resposta separa `is_reauthed_for_ad_publish`, `new_advertiser_verification_state`, `verification_state`, `apf_enrolled` e `apf_cleared`. No incidente CPV13, `apf_cleared=true` coexistiu com `is_reauthed_for_ad_publish=false`, e o `/copies` server-side continuou em `3858385` sem side effect; portanto essa rota é consulta de estado, não prova da mutação de acknowledgement. Antes de culpar propagação, confirmar que o perfil Facebook do browser é o mesmo usuário que emitiu o token; se for, investigar a mutação real no `batchedCall`/GraphQL e classificar como divergência de app/origem/sessão, nunca como campo público presumido. Enquanto o checkpoint persistir, manter objetos parciais PAUSED, preservar request/IDs e não repetir `/copies` às cegas; bug report/comunidade Meta continuam como escalação quando a confirmação já foi feita e o bloqueio reaparece.
...
33. **Presumir que `validate_only` é sempre idempotente, inclusive em `/copies`.** Em endpoints normais de criação, o preflight `validate_only` pode seguir a política bounded vigente para `5xx`/propagação. Porém, em `/copies`, um teste CPV retornou `copied_ad_id` e criou objetos apesar de `validate_only`; qualquer `copied_*_id` ou `ad_object_ids` é side effect real: persistir imediatamente, fazer GET/readback e nunca repetir a cópia por tratá-la como mero teste. Erro de parâmetro, permissão, segurança ou compliance nunca recebe retry automático.
