...
14. **Interpretar `code=31`/`error_subcode=3858385` como problema de payload ou IP da VPS.** A Meta documenta tokens como portáveis entre navegador e servidor. Esse subcode é um checkpoint de confiança do anunciante e pode exigir duas etapas sequenciais no Ads Manager da mesma conta/perfil que emitiu o token. Primeiro, procurar `Verifying your changes → Start Authentication` e concluir e-mail/SMS. Depois, o anúncio/draft pode continuar mostrando `Requer análise do anúncio (nº 3858385)` com o checkbox **`Confio nesse anúncio e ele está correto.`**: revisar budget, URL, início e término, marcar o checkbox e clicar em publicar. A autenticação por código não substitui essa segunda confirmação. A API não deve inventar nem inferir a declaração de confiança; somente uma sessão Ads Manager autenticada e verificada pode marcá-la. Screenshot não concede controle da aba do operador: automação visual só pode clicar quando o Ares possuir uma sessão de navegador autenticada para o mesmo perfil/conta e puder confirmar checkbox + publicação por readback. Enquanto a confirmação não for publicada ou o checkpoint não desaparecer, manter objetos parciais PAUSED, preservar request/IDs e não repetir `/copies` às cegas. Só então retomar `validate_only`/recovery missing-only. Se a API continuar bloqueada após as duas etapas publicadas, registrar como variante API-only e escalar com as threads/bug report oficiais.
