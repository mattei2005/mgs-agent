...
- Quando Rodolfo pedir clone na rotina CPV, selecionar no preflight a campanha elegível de maior ROI Smart Bidding da data operacional dentro do mesmo tipo de veículo. O pedido natural escolhe uma de três rotas sem ambiguidade: (1) `criar do zero` = campanha/adset/creatives/ads novos; (2) `clonar com criativos novos` = clone nativo raso da fonte, seguido de três creatives novos do Drive; (3) `duplicar igual` = deep copy fiel da campanha vencedora sem substituir o conteúdo criativo. Na terceira rota, auditar os Creative IDs, aceitar rematerialização pela Meta e confirmar o mesmo post/social proof pela igualdade de `effective_object_story_id`; campanha/adset são novos e o desempenho semelhante não é garantido. `MOTO→CARRO` e `CARRO→MOTO` são proibidos. Nenhum ID vira template fixo. Persistir modo, fonte e evidência antes do manifest. Copiar campos para objetos do zero não pode ser descrito como clone. Replacement sequencial valida as novas campanhas PAUSED antes de deletar/readback as antigas uma por uma.
