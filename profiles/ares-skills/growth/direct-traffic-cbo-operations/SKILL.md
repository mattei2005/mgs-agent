...
- No Intraday Creditoparaveiculo, repetir exatamente o resumo inicial imediatamente antes de `Tabela consolidada — visão desktop`, mantendo resumo + primeira página da tabela na mesma parte Discord. O atraso SB aparece como `Xh YYmin` quando atingir 60 minutos; abaixo disso, `Nmin`. `Rewarded CR` é consolidada e fica somente nos dois resumos; remover a linha dos cards e a coluna da tabela, pois repetir o mesmo valor por campanha induz granularidade falsa.
