...
- Em toda conclusão de criação programada CPV, informar também: budget ativo da conta, saldo restante dentro do cap operacional, moeda USD e fonte do cálculo. Após qualquer alerta `V3 BLOQUEADO`, diagnóstico/readback pode continuar, mas a solução proposta só recebe write corretivo após autorização explícita de Rodolfo ou Nicolas.
- O loop CPV é `analisar D1/D2/D3 → pausar/escalar conforme regra → decidir necessidade de nova coorte → criar se liberado`; não é “criar todo dia” de forma isolada. Hold de criação preserva Diário, Intraday, first-delivery e ações aprovadas, bloqueia novos slots sem expiração automática e só é liberado por Rodolfo ou Nicolas após revisão do ciclo.
