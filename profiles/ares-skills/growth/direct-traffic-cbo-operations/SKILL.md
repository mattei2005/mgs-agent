...
- Quando Rodolfo pedir clone na rotina CPV, selecionar no preflight a campanha elegível de maior ROI Smart Bidding da data operacional dentro do mesmo tipo de veículo. O pedido natural escolhe uma de três rotas sem ambiguidade: (1) `criar do zero` = campanha/adset/creatives/ads novos; (2) `clonar com criativos novos` = clone da fonte seguido de três creatives novos do Drive; (3) `duplicar igual` = copiar a campanha sem trocar mídia, copy, estrutura, público, estratégia ou budget. Na duplicação, usar o próximo número sequencial, reescrever naming e link/UTMs para o novo `cN/gNN` e terminar o nome da campanha com `DUP{ordem:03d}C{fonte:03d}`. A ordem vai de `001` até `N` dentro do pedido, e todas as duplicações apontam diretamente para a campanha fonte informada. Exemplo: cinco duplicações de C37 usam `DUP001C037` até `DUP005C037`. Alterar `url_tags` pode rematerializar Creative ID; auditar IDs e confirmar mídia/copy, URLs/UTMs e `effective_object_story_id`. `MOTO→CARRO` e `CARRO→MOTO` são proibidos. Nenhum ID vira template fixo. Persistir modo, fonte, número-fonte, número-destino e evidência antes do manifest. Copiar campos para objetos do zero não pode ser descrito como clone. Replacement sequencial valida as novas campanhas PAUSED antes de deletar/readback as antigas uma por uma.
