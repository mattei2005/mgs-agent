...
- Cooldown e pós-processamento nunca rodam como um comando foreground monolítico com `sleep` próximo do timeout. Usar runner determinístico com checkpoint por camada e timeout maior que `cooldown + pior I/O`, ou processo rastreado em background. Após compaction/restart, ler o checkpoint e a expiração da lane antes de aguardar; cooldown já transcorrido nunca é repetido. Intraday/Diário e writer da mesma `app_key + ad_account_id` compartilham o ledger/lock da lane e o relatório read-only deve deferir quando puder competir com write/recovery ativo.
