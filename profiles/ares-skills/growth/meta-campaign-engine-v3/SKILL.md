...
5. O v3 representa três formas distintas e nunca as trata como sinônimos: `from_zero_prestaged` = criar do zero; `clone_prestaged` = clonar estrutura/lineage e substituir por criativos novos do Drive; `pure_clone` = duplicar sem trocar mídia, copy, estrutura, público, estratégia ou budget. Em `pure_clone`, usar o próximo número sequencial, reescrever naming e link/UTMs para o número novo e adicionar ao final do nome o sufixo exato `COPY C{número-fonte-imediato}`; nunca usar `DUP`. Exemplo: se C11 for o próximo slot, duplicar C10 cria C11 com UTMs de C11 e nome terminado em `COPY C10`. Novos IDs técnicos são inevitáveis. Alterar `url_tags` pode rematerializar o Creative ID; validar mídia/copy equivalentes, URLs/UTMs e `effective_object_story_id`, sem exigir Creative ID igual. Quando o pedido disser “melhor campanha”, recalcular a origem no preflight dentro da mesma vertical. O planner divide 1–100 campanhas em bundles 2+2+…+1 por conta.
...
17. Em `clone_prestaged`, cada anúncio exige `source_ad_id` não zero e nasce por `POST /{source_ad_id}/copies` com `creative_parameters`; criação direta por `act_{account}/ads` é proibida nessa rota. Em `from_zero_prestaged`, ocorre o inverso: IDs fonte são proibidos e campaign/adset/creative/ad nascem somente pelos edges diretos `act_{account}`. Campaign copy, adset copy, normalização de shell, ad copies e normalização de nomes continuam batches sequenciais exclusivos do clone; criação do zero usa `campaign_create → adset_create → creative_create → ad_create → campaign_finalize → consolidated_readback`.
...
- Se uma recuperação criar qualquer child ausente, persistir imediatamente todos os IDs e encerrar a janela como `children_created_readback_pending`; não executar o readback consolidado na mesma janela. Retornar `READBACK_DEFERRED`/`PARTIAL_DEFERRED_QUOTA` com cooldown fresco de `max(reset, 300)+5s`. No resume, reservar somente o custo de readback, sem repetir write ou reservar novamente 60 pontos do bundle. `code=17/subcode=2446079` depois de POST confirmado é estado resumível de leitura, não campanha fracassada.
- Cooldown e pós-processamento nunca rodam como um comando foreground monolítico com `sleep` próximo do timeout. Usar runner determinístico com checkpoint por camada e timeout maior que `cooldown + pior I/O`, ou processo rastreado em background. Após compaction/restart, ler o checkpoint e a expiração da lane antes de aguardar; cooldown já transcorrido nunca é repetido. Intraday/Diário, first-delivery watcher e writer da mesma `app_key + ad_account_id` compartilham o ledger/lock da lane. Enquanto o request persistido estiver em estado resumível (`ASSETS_RESERVED`, `MEDIA_READY`, `MANIFEST_SEALED`, `READBACK_DEFERRED`, `PARTIAL_DEFERRED_QUOTA`, `POSTPROCESS_PENDING`, `RECOVERY_PENDING` ou `FIRST_DELIVERY_ARM_IN_FLIGHT`), todos os readers recorrentes da conta deferem silenciosamente; lock livre entre ticks do writer não autoriza consulta concorrente durante o cooldown. Ao concluir, remover `failure`, `retry_after_epoch` e `operator_authorization` do state e marcar os flags de recuperação como `false`, para que os readers retomem sem carregar erro histórico.
