...
5. O v3 representa quatro formas distintas e nunca as trata como sinônimos: `from_zero_prestaged` = criar do zero; `clone_prestaged` = clonar estrutura/lineage e substituir por criativos novos do Drive; `pure_clone` = duplicar preservando estrutura, público, estratégia, mídia e copy; `clone_page_switch` = clonar estrutura/lineage e ads-fonte, preservar mídia/copy e rematerializar os parâmetros criativos aprovados para outra Page/UTM/JSON. Naming, tracking, budget e início seguem o contrato canônico da operação, nunca um default cruzado. Creditoparaveiculo mantém próximo sequencial, UTMs novas e sufixo `COPY C{fonte}`; Eggbev `1034081997659047` preserva o nome-base e usa `DUPnn`, aceita budget escolhido pelo gestor sob gate financeiro e exige `ACTIVE` às 00:00 ET. O planner divide 1–100 campanhas em bundles 2+2+…+1 por conta.
