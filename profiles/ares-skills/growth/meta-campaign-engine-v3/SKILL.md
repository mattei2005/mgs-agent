...
20. Em Eggbev from-zero, o runner operacional materializa e sela o manifest, mas nunca cria objetos Meta diretamente: `scripts/ares-eggbev-creation.py` delega toda execução ao Engine v3. O request aplica reconciliação scoped Drive×Meta, reserva somente após o pedido, pre-stage registry-first com títulos `asset_id + checksum curto`, três ou cinco ads por campanha, uma headline aprovada por ad em rotação determinística e pós-processamento `01_READY → 02_TESTING` apenas depois do readback Meta completo. Os nomes dos ads são derivados automaticamente como `AD NN - {canonical_stem}`; budget é input humano obrigatório. Nicolas tem autoridade permanente de Rodolfo para definir, reduzir ou aumentar budgets Eggbev, inclusive a baseline de USD45, sem nova aprovação do Rodolfo; cada write exige valor exato, pré-leitura e readback Meta. Billing, `account_spend_limit`, credenciais e escala automática permanecem separados. Depois de toda criação Eggbev, o runner faz GET de cada creative e compara Page, `url_tags` e o JSON parseado em `asset_feed_spec.additional_data.page_welcome_message` com o manifest aprovado. Divergência deixa `POSTPROCESS_PENDING`, preserva IDs e proíbe replay da criação.
...
- Em create direto Eggbev no Graph v26, `instagram_positions=["explore"]` é inválido e retorna `code=100/subcode=2490589` (“IG Explore Placement Is Deprecated”); remover somente `explore` e manter `explore_home`. Esta regra supersede a evidência anterior de `subcode=2490392` que exigia o parent `explore`. A correção não autoriza Audience Network nem Advantage+ Placements. Três headlines sem `title_label` proprietário do creative do Ads Manager foram rejeitadas (`code=100/subcode=1885878`); use uma headline aprovada por ad em rotação determinística, não reutilize IDs/labels internos de outro creative. `validate_only` foi aceito nos edges diretos campaign/adset/adcreative/ad com GET antes/depois e zero objeto criado, mas essa evidência não se estende a `/copies`.
- Em `clone_page_switch` e em qualquer `clone_prestaged` com troca de Page, alterar `object_story_spec.page_id`/JSON no `creative_parameters` não altera o `promoted_object.page_id` do ad set copiado. O executor deve representar e aplicar explicitamente a atualização do ad set, além de qualquer normalização de targeting/placements, e confirmar tudo no readback antes de ativar. Se a operação exige Page nova ou proíbe placements presentes na fonte e o manifest/runtime não possui esses updates, falhar fechado antes do write; nunca publicar com Page do creative diferente da Page promovida no ad set.
