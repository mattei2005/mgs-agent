...
`X-Ad-Account-Usage` and `X-Business-Use-Case-Usage` are persisted separately when Meta returns them. Do not infer server capacity only from the local counter.
