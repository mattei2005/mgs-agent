...
10. Logs dos crons Meta em `logs-aquisicao` devem usar título com `nome da conta — dia — horário no timezone da conta — tipo do cron` e tabela alinhada com estas colunas base: `Nome da campanha`, `PG ID` quando útil, `Início`, métricas aplicáveis, `Ação`, `Motivo`, `Status`. Para HOA, não exibir `ID REC`: Rodolfo considerou poluição para gestores, porque o nome completo da campanha já é único quando inclui a numeração (`... - 004`). IDs de recomendação podem continuar no audit JSON para rastreabilidade técnica, mas não devem aparecer na tabela normal de gestores. `Nome da campanha` deve ser legível no mobile e pode normalizar apenas a exibição para 3 dígitos (`... - 009`) sem renomear a campanha na Meta; o nome bruto fica no audit. `Início` deve ser data real em formato `dd/mm/yyyy`, nunca idade decimal tipo `1.17d`. Não incluir colunas redundantes `Nome da página`, `Página`, `Campaign ID` ou `Meta ID` no relatório normal; IDs técnicos ficam no audit/API. Extrair `PG ID` do padrão `(pg_12345)` no nome da campanha quando a coluna existir. Em `Regra usada`/`Motivo`, intraday deve mostrar o identificador e a descrição curta (`R1 — ...`, `R2 — ...`, `R3 — ...`, `R4 — ...`, `R5 — ...`). O cron diário separado deve mostrar só `reativar-todas` — não rotular como `fora R1-R5`, porque a distinção já está no tipo do cron/título.
...
14. Para recomendações que exigem ação humana, criar/usar thread do checkpoint em `logs-aquisicao` e incluir `Nome da campanha`, `Ação que eu tomaria`, `Motivo` e `Estado local`. O identificador técnico de recomendação pode ficar só no audit. Respostas curtas de Rodolfo (`feito`, `ignorar`, `segurar`, `pausei`, `reativei`, `não mexer nessa campanha`) devem ser aceitas quando acompanhadas do nome completo da campanha; se a referência for parcial e bater em múltiplas campanhas, Ares deve pedir desambiguação. Registrar em state/audit e validar por GET na próxima leitura.
...
Nome da campanha               | PG ID    | Início     | Spend | MO | CPMO | Ação que eu tomaria | Motivo
-------------------------------|----------|------------|-------|----|------|---------------------|-------
Elena Santana - ES - ESP - 009 | pg_22091 | 20/06/2026 | 6.21  | 0  |      | OBSERVAR            | Learning < 3d; R1 acionou
```
...
- Respostas curtas de Rodolfo na thread devem mapear para state/audit quando vierem com nome completo de campanha ou referência única: `feito`, `ignorar`, `segurar 1 checkpoint`, `pausei`, `reativei`, `não mexer nessa campanha`. Se houver múltiplas campanhas compatíveis, pedir desambiguação.
