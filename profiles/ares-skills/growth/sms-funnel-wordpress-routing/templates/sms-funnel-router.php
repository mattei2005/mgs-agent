 ...
 * Description: Encaminha cliques de SMS por veículo, gestor e etapa usando vehicle, utm_medium, var_phone e step.
 * Version: 1.2.0
...
function mgs_smsf_render_overview_page(){
    ...
    <div class="wrap"><h1>MGS SMS Funnel</h1><p>O roteamento usa obrigatoriamente <code>veículo + utm_medium + step</code> para separar Carro/Moto e G001–G006.</p>
...
function mgs_smsf_render_route_page(){
    ...
    <hr><h2>Como usar nos links</h2><p><code>vehicle=<?php echo esc_html($vehicle); ?>&amp;utm_medium=<?php echo esc_html($group.'-s'); ?>&amp;step=01</code></p><p><code>vehicle=<?php echo esc_html($vehicle); ?>&amp;utm_medium=<?php echo esc_html($group.'-s'); ?>&amp;step=02</code></p><p><code>step=03</code> permanece sem ação até ser configurada uma futura Lista 4.</p></div><?php
}
...
function mgs_smsf_vehicle_from_path($request_uri){$path=strtolower((string)wp_parse_url((string)$request_uri,PHP_URL_PATH));if(preg_match('#(^|[-_/])moto([-_/]|$)#',$path))return 'moto';if(preg_match('#(^|[-_/])carro([-_/]|$)#',$path))return 'carro';return '';}
function mgs_smsf_normalize_vehicle($raw_vehicle,$request_uri){$raw_vehicle=strtolower(trim((string)$raw_vehicle));$path_vehicle=mgs_smsf_vehicle_from_path($request_uri);if($raw_vehicle!==''){if(!in_array($raw_vehicle,mgs_smsf_vehicle_keys(),true))return '';if($path_vehicle!==''&&$path_vehicle!==$raw_vehicle)return '';return $raw_vehicle;}return $path_vehicle;}
function mgs_smsf_handle_click(){
    if(is_admin()||wp_doing_ajax()||wp_doing_cron()||!isset($_GET['var_phone'],$_GET['step']))return;
    $phone=preg_replace('/\D+/','',wp_unslash($_GET['var_phone']));$step=mgs_smsf_normalize_step(wp_unslash($_GET['step']));$group=mgs_smsf_normalize_medium(wp_unslash($_GET['utm_medium']??''));$vehicle=mgs_smsf_normalize_vehicle(wp_unslash($_GET['vehicle']??''),wp_unslash($_SERVER['REQUEST_URI']??''));
