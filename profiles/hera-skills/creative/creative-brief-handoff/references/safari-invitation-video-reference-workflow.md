...
Use quando Hera receber pedido de convite/vídeo criativo com referência externa (YouTube/Shorts/Reels/anexo) e o usuário exigir comparação GPT vs Grok.
...
4. **Cookies/proxy são setup persistente, não por vídeo.**
   ...
   Caminho local aprovado para cookie jar da Hera:
   ```text
...
5. **Grok explícito exige validação real do wrapper MGS.**
   ...
   ```bash
   /root/mgs-agent/scripts/mgs-grok-generate.py image|video --profile hera ...
   ```
