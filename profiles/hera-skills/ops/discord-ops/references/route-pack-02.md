...
REPORT-INFRA usa somente Discord Embed pelo helper canônico `/root/mgs-agent/scripts/send-report-infra-embed.sh`: `content` vazio, sem mentions, sem thread e sem segunda cópia em texto. Não usar `send_message` direto para esse report. O helper usa o transporte central do Zeus; se falhar, registrar o erro e a pendência sem simular entrega nem alterar permissões automaticamente.
