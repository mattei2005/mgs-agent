...
- For advanced report filters, use the metric's technical `id`, not necessarily its visible label. In Messenger Pages, Broadcast `SENDS`/`DELIVEREDS` map to `BD_SENDS`/`BD_DELIVEREDS`; the zero-delivery expression is `BD_SENDS>0 BD_DELIVEREDS=0`.
- Do not claim that Messenger Pages can exclude active restrictions with `NOT("Restricted")` or `STATUS=Broadcast`: this report does not expose `RESTRICTED_UNTIL` or `NOTES/#2022`, and restricted pages may remain `Broadcast`. Reconcile page IDs against Messenger Page or the authoritative restriction dataset.
...
- `references/messenger-pages-generic-filter-fields.md` — current Messenger Pages advanced-filter field IDs, the `BD_SENDS>0 BD_DELIVEREDS=0` pattern, and why active restrictions require a separate Page/dataset reconciliation.
