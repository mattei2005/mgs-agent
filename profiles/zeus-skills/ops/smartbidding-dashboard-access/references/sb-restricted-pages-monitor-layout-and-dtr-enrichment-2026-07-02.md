...
- `Entrou` must represent a restriction that actually began after the previous completed scan. First detection by the monitor, first insertion into Smart Bidding, or absence from the alert state is not sufficient evidence that the restriction is new.
- A current DTR `#2022` generated by a new campaign attempt can expose a restriction that was already active. Compare the DTR release timestamp with the nominal restriction window (normally 30 days) and the previous completed scan. If substantially less than the normal window remains, classify it as `Restrição preexistente detectada`, synchronize SB if required, but do not count or alert it under `Novas restrições`.
