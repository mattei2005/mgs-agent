...
Current fixed-30 live-repair correction from Rodolfo 2026-08-03: any `Update + Save`, including a no-op content edit, resets the status log of the entire Broadcast Template version to gray. Therefore the immutable template ID, not an individual message slot, is the transaction/lock unit. For the dedicated fixed-30 executor: never touch 30/30-green templates; replace every red slot in one batch while preserving each slot's IDs/links/media; for purple without red, preserve visible copy and perform a no-op reset; then confirm 30 gray and run one Approval. Wait `PAGES × 30 × 12s` plus margin before readback. This explicitly supersedes the earlier assumption below that editing one slot resets only that slot and reopens purple retry without claiming causal Page attribution. The legacy 10/20/30 manager stays frozen.

Historical rule (superseded for this executor): do **not** use `Erase All` for normal production repair. The earlier assumption was that editing one message reset only that message and that whole-template gray was a bug; live confirmation on 2026-08-03 proved the whole-template reset is expected in this flow.
...
- When Rodolfo identifies Zuout Finanzas templates by the legacy base `https://tarjeta.zuout.com/lpszuout`, scan `MESSAGES[].LINK_1` across the full `COMPANY=digital-trust` Broadcast Template scope; do not rely on template `DOMAIN` or name alone. For the validated linked 30-message Zuout US-CC-ES template, replace only `LINK_1` with the MCT Smart Routing sequence `001–023`, then repeat `001–007` in slots `24–30`, preserving `utm_source=mct`, the template's current `utm_medium`, `[utm_campaign]`, `[utm_content]`, all message text/media/IDs, zero-width characters and approval/status fields. Confirm the 23 route identities exist before writing, back up the full row, and require immediate plus independent-session readback.
