...
6. Se a URL publicada responder HTTP 200, Rodolfo tiver ordenado explicitamente o write e a operação ainda não existir no catálogo, a rota pode ser gravada com `jbf_operation` vazia somente como `success_with_adops_pending`. Reportar contagem exata por publisher/família e nunca declarar prontidão AdOps completa. O cadastro de URLs/produtos no catálogo e a geração/preenchimento de `jbf_operation` pertencem ao time de AdOps da Smart Bidding; Zeus não deve executar nem oferecer esse cadastro como próximo passo. Após criar a pool com operação vazia, aguardar AdOps e fazer apenas conferência/readback se Rodolfo solicitar.
