...
Backend caveat: `/broadcast/Messenger` returns `MESSAGES` as a JSON-encoded message array with `APPROVED`, `INVALID_FORMAT`, `REJECTED`, `ERROR`, `REJECTED_REASON`, `LINK_1`, etc.; the UI may render it as a count. Approval colors and reasons are aggregated at the Broadcast Template/message level across all linked pages. They do **not** identify which Page ID caused a red/purple/gray result. Joining `/broadcast/Messenger[].ID` to `/campaigns/Messenger[].BROADCAST_TEMPLATE_ID` yields the universe of pages linked to an affected template, not proof that each linked page caused the error. Label exports accordingly (`pages linked to templates with purple`, never `purple pages`) unless a page-level Meta/Ciro approval log explicitly names the Page ID. See `references/messenger-backend-fields-and-company-scope-2026-06-29.md` and `references/sb-utility-rollout-broadcast-pages-correction-2026-07-02.md`.
