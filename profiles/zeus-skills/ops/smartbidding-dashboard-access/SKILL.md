...
- **General Bot ↔ SB registration audit is NOT the same as page-health/restricted-page workflow.** When Rodolfo asks for a conferência geral, auditoria de IDs, match entre Bot/DTR e Dash/SB, usuários do bot na SB, páginas cadastradas, or similar, the scope is **all DigitalTRChat bot users registered in 1Password**, regardless of migration-sheet status, page status, whether the page is sending, restricted, blocked, on-hold, active, inactive, or removed from operational sheets. The goal is registry consistency: every bot user/page/profile in DTR should match SmartBidding `Accounts > Messenger > Page` by `USER_LOGIN`, `PAGE_ID` (small PG id), `FB_PAGE_ID`, page name, and segurador/profile. Do not use `Migração 22/06`, active-user filters, `Removidos acumulado`, zero-delivery filters, or restricted-page scope for this audit unless Rodolfo explicitly asks for the operational page-health workflow.
