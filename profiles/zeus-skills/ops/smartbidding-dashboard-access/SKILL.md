...
- **Specific Bot/DTR ↔ SB PAGE ID conference only.** This rule applies only when Rodolfo is explicitly asking to confer/check/audit the `PAGE ID` cadastro relationship between what exists in the Bot/DigitalTRChat and what exists in the SmartBidding dashboard (`Accounts > Messenger > Page`). In this specific situation only, the scope is **all DigitalTRChat bot users registered in 1Password**, regardless of migration-sheet status, page status, sending status, restricted state, `On-hold`, `Blocked`, `Campaign`, `Broadcast`, active/inactive status, or operational-sheet removal. The goal is registry consistency: every bot user/page/profile in DTR should match SB by `USER_LOGIN`, segurador/profile, `PAGE_ID` small PG id, `FB_PAGE_ID`, and page name. For corrections, the three SB fields Rodolfo identified must match DTR: `PAGE_ID`, `FB_PAGE_ID`, and `UTM_CAMPAIGN=pg_<PAGE_ID>`. Auto-correct only safe same-user existing SB rows; do not create missing rows, delete extra rows, or write cross-user/global-FB matches without a manual decision. `PUT /campaigns/Messenger/update-many` may return success but not persist `PAGE_ID`/`UTM_CAMPAIGN`; use the UI save route `POST /campaigns/Messenger` with a whitelisted page-edit payload, canary first, then read back the three fields. See `references/dtr-sb-page-id-registration-conference-2026-07-05.md`. Do **not** generalize this scope to restricted-page monitoring, page-health sync, zero-delivery checks, migration-sheet workflows, campaign diagnostics, or any other SB/DTR automation; those keep their own existing scopes and rules.
