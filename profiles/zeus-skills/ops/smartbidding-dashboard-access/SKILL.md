...
- Utility canary approval loop completion rule: when a cycle returns `TODOS VERDES` / no messages left to replace, the wrapper must post one final notice in `discord:1522487422510694450`, pause the temporary cron automatically, and stay silent on any later accidental tick via a local completed flag. Do not keep posting every minute after completion.
