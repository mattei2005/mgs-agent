...
Correções de enum/dropdown do modal: campos da Sheet são labels humanos, mas a API precisa receber os valores internos usados pelas rows existentes, senão o GET retorna texto mas o modal fica com dropdown vazio. Mapear `Status READY -> Ready`, `Country United States -> US`, `Vertical Credit card -> CC`, `Source Facebook -> FACEBOOK`. Para `BROADCAST_TIME`/Scheduled Times, não deixar `null`: copiar a lista de horários de uma row existente do mesmo login/template/site de referência. A validação final não pode ser só direct `GET /campaigns/Messenger/{ID}`; precisa confirmar que os enums internos aparecem no readback full-scope (`COUNTRY=US`, `VERTICAL=CC`, `SOURCE=FACEBOOK`, `STATUS=Ready`) e, quando possível, abrir o modal/inspecionar UI para garantir que os dropdowns não ficaram em `Select ...`.
