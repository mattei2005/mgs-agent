...
- **Specific Bot/DTR ↔ SB PAGE ID conference only.** This rule applies only when Rodolfo is explicitly asking to confer/check/audit the `PAGE ID` cadastro relationship between what exists in the Bot/DigitalTRChat and what exists in the SmartBidding dashboard (`Accounts > Messenger > Page`). In this specific situation only, the DTR scope is **all DigitalTRChat bot users registered in 1Password**, regardless of migration-sheet status, page status, sending status, restricted state, `On-hold`, `Blocked`, `Campaign`, `Broadcast`, active/inactive status, or operational-sheet removal. The SB scope is the full child-publisher set under both companies: normalize `Digital trust`/`Digital trust 2`, include every child `publisherId` under `digital-trust` + `digital-trust-2` (not only `active` publishers), and hard-stop if the current full-scope guard is not met (around `56` publishers / `3,237` Messenger Page rows as of 2026-07-06). The goal is registry consistency: every bot user/page/profile in DTR should match SB by `USER_LOGIN`, segurador/profile, `PAGE_ID` small PG id, `FB_PAGE_ID`, and page name. Compare page names only after Unicode NFC normalization so decomposed accents do not create false divergences in Excel. For corrections, the three SB fields Rodolfo identified must match DTR: `PAGE_ID`, `FB_PAGE_ID`, and `UTM_CAMPAIGN=pg_<PAGE_ID>`; PAGE_NAME-only corrections are allowed when Rodolfo explicitly names the Excel rows and must be validated by readback. Auto-correct only safe same-user existing SB rows; do not create missing rows, delete extra rows, or write cross-user/global-FB matches without a manual decision. `PUT /campaigns/Messenger/update-many` may return success but not persist `PAGE_ID`/`UTM_CAMPAIGN`; use the UI save route `POST /campaigns/Messenger` with a whitelisted page-edit payload, canary first, then read back the edited fields. See `references/dtr-sb-page-id-registration-conference-2026-07-05.md` and `references/dtr-sb-page-id-scope-unicode-page-name-2026-07-06.md`. Do **not** generalize this scope to restricted-page monitoring, page-health sync, zero-delivery checks, migration-sheet workflows, campaign diagnostics, or any other SB/DTR automation; those keep their own existing scopes and rules.
