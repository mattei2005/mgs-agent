...
- `references/dtr-restricted-pages-authoritative-cron-2026-07-06.md` + `references/dtr-restricted-pages-cron-authoritative-flow-2026-07-06.md` — Rodolfo correction for restricted-page cron: new restricted-page alerts must be DTR/Bot-authoritative, using active sheet users/seguradores without `X`, skipping pages already restricted in SB, reading latest Completed per remaining page, handling `#2022` pure/mixed, applying SB `RESTRICTED_UNTIL` with readback, and including large `FB_PAGE_ID` in reports. The second file records the validated post-fix runtime evidence and hard pitfalls from the correction session.
