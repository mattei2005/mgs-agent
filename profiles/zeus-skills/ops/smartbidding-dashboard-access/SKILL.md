...
Intent correction: if Rodolfo says `cadastra essa` while giving login/email + large `FB Page ID` + small `Page ID`/PG + page name, especially with the registration Sheet link, interpret it as this SB Page registration flow — not as agent/user authorization. Locate the exact Sheet row and execute all fields. For readback, accept `LOGIN` as the login field when `USER_LOGIN` is null on newly created rows, and use direct `GET /campaigns/Messenger/{ID}` if the full list has not refreshed yet. Detailed session notes: `references/sb-page-registration-from-sheet-intent-and-readback-2026-07-07.md`.
