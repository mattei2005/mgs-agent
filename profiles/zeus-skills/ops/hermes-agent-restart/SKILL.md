...
- **`/root/.hermes/.env` é o arquivo base/template** — NÃO é o `.env` ativo dos agentes. Configs reais (incluindo `DISCORD_ALLOW_BOTS`, tokens, etc.) ficam em `/root/.hermes/profiles/{agent}/.env`. Ao validar se uma variável está aplicada, sempre checar o profile correto: `grep VAR /root/.hermes/profiles/zeus/.env` — nunca `/root/.hermes/.env`.
