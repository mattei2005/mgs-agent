...
13. **Auto-commit ativo não garante commit/push saudável** — em auditoria geral, sempre checar `/root/mgs-agent/logs/auto-commit-watcher.log`, `/root/mgs-agent/logs/auto-push.log` e `git status --short`. O serviço pode estar `active` mas bloqueando todas as rodadas por guardrail de arquivo sensível. Se o bloqueio vier de artefatos runtime/audit/state (ex.: `data/ares/meta-ads/audit/**/*.json`, `reports/**/*.json`, `state/**/*.json`, `data/**/*-state.json`), a correção canônica é ignorar/excluir esses paths no `.gitignore` e no `GIT_PATHSPECS` do `auto-commit-watcher.sh`, não relaxar o regex sensível. O `monitor-auto-push.sh` também deve tratar `BLOQUEADO: arquivo sensível detectado` no log do auto-commit como falha operacional quando esse for o estado mais recente do watcher, porque nesse caso o push nunca inicia. Se o bloqueio for por nome de documentação contendo `token`, `password`, `secret`, `webhook` ou `credential`, escanear padrões reais de segredo sem imprimir conteúdo; se não houver segredo, preferir renomear o arquivo para remover a palavra sensível em vez de relaxar o guardrail.
