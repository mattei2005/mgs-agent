...
Use when `monitor-service-restarts.sh` reports a Zeus/Atena/Ares/Hera gateway restart near a package-manager maintenance window, especially Monarx on Hostinger.
...
4. Verify current health: Zeus/Atena/Ares/Hera/mgs-autocommit active and no restart loop.
...
# Package maintenance may update OS/security packages, but Zeus/Atena/Ares/Hera gateways
...
$nrconf{override_rc}->{qr(^hera-gateway\.service$)} = 0;
```
...
needrestart -r l -b 2>/tmp/needrestart.err | grep -E 'zeus-gateway|atena-gateway|ares-gateway|hera-gateway' || echo 'OK no Hermes gateways listed'
```
