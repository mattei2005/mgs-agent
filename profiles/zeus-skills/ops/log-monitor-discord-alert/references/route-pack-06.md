...
- Em alertas Git, não apresentar `git rev-list <tag>..main` como “commits após a última release” sem qualificação: merges podem incorporar commits com datas anteriores à tag e inflar essa contagem sem representar pendências. Para decisão operacional, exibir `runtime/base comum..upstream` como **pendentes no runtime** e `último upstream notificado..upstream atual` como **novos desde o último alerta**, declarar essas bases no payload e cobrir por teste que a métrica bruta desde a tag não aparece como pendência.
