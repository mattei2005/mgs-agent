...
- When reconciling a previously reported stale ownership or routing sentence in a SOUL, never clear the risk from a search command that returns zero matches alone. Read the exact live SOUL and its versioned mirror directly, compare them, and verify the relevant line before declaring the sentence absent or corrected. A failed glob, filename filter, encoding edge case, or wrong search root must not become a false executive conclusion. For a single-line correction, retain protected pre-change copies and prove the old/new files have equal line counts with exactly the authorized line changed.
- Post-change proof requires a successful fresh-session answer plus canonical-source readback. Inspect the fresh row in `state.db` read-only when available. Current Hermes v0.21 may create a valid `source=tool` session while leaving `sessions.system_prompt` NULL; in that state, never fabricate a marker count or fail a proven cutover solely because the column is empty. Require the fresh session ID/source, an answer that demonstrates the rule without supplying its content in the test prompt, and exactly one distinctive marker in both live SOUL and versioned mirror. If `system_prompt` is populated, additionally require the marker exactly once there. An OAuth failure before session creation is not partial behavior validation merely because live/versioned SOUL hashes match.
...
2. Do not restart merely to load SOUL. With the Zeus profile already active, prove cutover using `hermes chat -Q --source tool -q ...`; ask for the rule's consequence without embedding its answer. The legacy `hermes -p zeus ...` form is not accepted by current v0.21 CLI, and `hermes -z` does not provide the same explicit source-tag proof.
3. Verify the returned fresh session ID in `state.db` read-only; the schema keys `sessions` by `id`. Require `source=tool`, a correct rule-aware answer, live/versioned mirror equality, and one distinctive marker in each canonical file. When `sessions.system_prompt` is non-empty, require the marker exactly once there; when v0.21 stores it as NULL, record that limitation and rely on the fresh behavioral answer plus canonical readbacks instead of inventing persistence evidence.
