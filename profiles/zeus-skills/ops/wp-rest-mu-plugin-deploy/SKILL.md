...
17. **SFTP wpfiles usa path relativo, não absoluto:** O usuário `wpfiles` tem como chroot o webroot do WP. Usar `get wp-content/mu-plugins/yoast-rest-meta.php` (sem barra inicial), não `get /wp-content/mu-plugins/yoast-rest-meta.php`. Path com `/` retorna "File not found" mesmo quando o arquivo existe. Confirmado em openzed.com 2026-04-26.

18. **`GET /wp/v2/plugins?status=mustuse` retorna erro 400:** O endpoint REST de plugins não suporta `status=mustuse` — retorna `rest_invalid_param`. Must-use plugins só são visíveis via browser em `wp-admin/plugins.php?plugin_status=mustuse` ou via elFinder `ls` em `wp-content/mu-plugins/`. Para validar via REST que o mu-plugin está funcionando, checar os campos registrados na resposta de posts (`meta._hide_from_home`, `_yoast_wpseo_title` no root).

20. **`os.environ` setado em `execute_code` NÃO propaga para `terminal()` subprocessos:** Dentro de um bloco `execute_code`, cada `terminal()` call roda em subprocess separado — não herda variáveis definidas via `os.environ[...] = ...` no Python. Portanto, setar credenciais via `os.environ` e usar em `terminal()` subsequente retorna string vazia. Padrão correto: chamar `op item get ... --reveal` diretamente dentro do `terminal()`, ou criar função helper Python que chama `terminal("op item get ...")` e retorna o valor. Confirmado na sessão 2026-04-26 ao buscar credenciais da REST API openzed.com — o `op item get` com `--fields label=api_auth_user` estava retornando o resultado mas como JSON do campo completo (182 chars), não o valor puro. Solução: usar `--vault` explícito + `--reveal` sem `--format` → retorna o valor puro como string.

19. **WP File Manager pode sobrar ativo após deploy manual do Rodolfo:** Se o deploy foi feito manualmente (não pelo fluxo Zeus), o Passo 8 (desativar + deletar WP File Manager) pode ter sido pulado. Antes de declarar EXIT CHECKLIST completo após deploy manual, verificar `GET /wp/v2/plugins?status=active` e confirmar que `wp-file-manager` não aparece. Se aparecer, desativar+deletar via REST ou reportar ao Rodolfo como pendência. Confirmado em openzed.com pós-recovery 2026-04-26.
