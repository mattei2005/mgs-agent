...
- Mesmo com o texto do follow-up correto, o reply visual pode ficar errado quando `_run_agent()` drena uma mensagem pendente dentro do handler da mensagem anterior: o retorno final volta para `_process_message_background(event_original)`, que enviaria usando o anchor original. Preserve `gateway_reply_to_message_id`/`gateway_reply_source` no `agent_result` do follow-up e faça `BasePlatformAdapter` respeitar esses atributos no send final.
