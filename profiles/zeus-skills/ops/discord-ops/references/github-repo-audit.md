...
- When a historical archive contains Hermes profile backups (`profiles/{agent}/.env`), inspect those `.env` members explicitly and compare current-vs-historical hashes for `DISCORD_BOT_TOKEN`, `ANTHROPIC_API_KEY`, `OP_SERVICE_ACCOUNT_TOKEN`, webhooks, and GitHub PATs. Do not rely only on a generic high-signal regex summary: active Discord bot tokens are JWT-like/opaque and may not match provider-specific regexes, but they are operationally critical if still current.
