...
- Tag columns C/F must be populated for every internal WordPress article URL represented in the sheet. Lookup order: post ID → `/wp-json/wp/v2/posts/{id}` → `tags`; if `tags: []`, use the WordPress category taxonomy as fallback rather than leaving internal article tag cells blank. External final URLs may have blank P1 tags because they are not WordPress articles.
