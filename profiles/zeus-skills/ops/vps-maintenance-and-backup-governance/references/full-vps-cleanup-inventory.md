...
- **Generated local test databases require evidence-lineage and family-consumer checks.** For roots such as `private/{test-pg,ui-test,ui-browser,catalog-test,TEST-payroll,TEST-periods}-*`, search retained JSON/reports/inventory for exact-path references and inspect helpers that reopen a directory by prefix or “latest” selection. Preserve every evidence-linked cluster until that evidence is explicitly superseded or migrated with validated readback; also preserve one compatible current cluster for each family still consumed by a helper, unless the helper first regenerates a fresh source or is refactored and tested. Only the remaining exact, unreferenced clusters may advance to a destructive manifest after the normal process/mount/Git and initiative-closure gates. “Untracked” proves only Git status, not absence of audit or test value.
