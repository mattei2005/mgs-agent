...
4. If another MGS profile has a currently working `openai-codex` OAuth provider, copy only that provider block into the affected profile’s `auth.json` and keep/restore the affected profile’s intended `active_provider`.
...
5. Validate the profile with a real smoke call, e.g. `hermes -p <agent> -z 'Responda apenas: OK_<AGENT>'`.
...
- `active` systemd service does not mean model auth works. Run a direct `hermes -p <agent> -z ...` smoke test.
