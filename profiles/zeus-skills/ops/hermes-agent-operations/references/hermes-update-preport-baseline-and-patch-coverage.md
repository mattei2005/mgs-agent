...
- Do not mutate the active production venv during the no-restart stage. Clone it to a versioned next-venv, upgrade the target package plus required active extras there, and run the full guard/config checks with an overridable `PYBIN`. Swap/activate that venv only inside the separately authorized detached restart window.
