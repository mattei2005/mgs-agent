...
Treat desired policy and active protection separately. Hermes upstream does not auto-compact by itself. The MGS residual implementation is active through `scripts/monitor-hermes-memory-capacity.py`, which discovers immediate operational profiles, checks USER/MEMORY every ten minutes, and invokes `scripts/hermes-memory-autocompactor.py` at `>=90%` with an adaptive verified reduction and a preferred `89%` target below the alert threshold. Each accepted one-to-one entry must pass deterministic literal restoration plus its own independent semantic verifier; do not add a third global model verdict over the already verified set, because that creates nondeterministic all-or-nothing rejection without adding a new invariant. The cron schedule is `4,14,24,34,44,54 * * * *` under `flock`; the monitor allows up to `1200` seconds for a verified per-entry run, so later ticks skip rather than overlap; state is `data/hermes-memory-capacity-state.json`; protected backups are under `/root/.hermes/secure-backups/memory-autocompaction/`; success/failure embeds go metadata-only to `#limites-90` (`1527401973698007060`). Do not request per-occurrence approval. `monitor_hermes_pending_writes.py` remains the owner of aged pending records and `capacity_overflow` dead-letters, but its legacy raw-capacity alerts/proposals are disabled by default; `--capacity-alerts` is compatibility-only and must not be enabled alongside the dedicated autocompactor monitor.

When native Honcho is active, do not run a second automatic user-modeling loop into file-backed USER/MEMORY. Keep `memory.nudge_interval: 0` for every MGS profile: Honcho's persisted messages, peer representation and dialectic conclusions own automatic longitudinal learning. Foreground `memory` writes remain available for explicit always-active facts. Automatic skill reviews remain enabled, but the runtime whitelist must exclude the `memory` tool whenever `review_memory=False`; guard this with the supplemental patch `patches/hermes/honcho-background-file-memory-freeze-2026-08-21.patch` and its regression test. This freezes routine USER/MEMORY growth at the source instead of relying on endless semantic compaction.

Do not claim the protection is active from code/config alone. Require: profile discovery readback; unit tests for threshold, dry-run, atomic state, cooldown and durable outbox; an exact-dedup apply canary proving backup/write/readback; a real semantic two-pass dry-run at the production `89%` preferred target; Discord transport mock; a production run showing active profiles/stores and zero unexpected failures; and exact cron readback. The compactor must fail closed on model timeout, malformed output, protected-literal drift, semantic-verifier rejection, concurrent source change, invalid backup or post-write readback. A failed occurrence preserves the source when provable and produces a metadata-only alert; never weaken validation to force headroom.
