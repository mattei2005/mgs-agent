...
- When Rodolfo explicitly says that text came from Claude/Claude.ai, treat it as an external proposal, not operational truth: reconcile it against live runtime, MGS sources, audit, Git, and sessions. If Rodolfo's current message directs Zeus to execute whatever is valid, execute the supported non-critical parts immediately; execute a Critical Subset item only when that same current message also supplies Rodolfo's required explicit confirmation. If the proposal is partly or wholly wrong, do not execute the unsupported part—state the operational correction and provide the response Rodolfo should use.
