...
- **Colisão pós-update `gateway run --replace`:** mesmo em update controlado/no-restart, validar se o update oficial disparou replacements órfãos (`hermes_cli.main --profile <agent> gateway run --replace`) que deixam systemd em loop `Gateway already running`. Se ocorrer, não repetir restart foreground: parar só os serviços afetados, matar apenas os PIDs órfãos `--replace`, `reset-failed`, subir de novo e validar `active/running`. Zeus fica fora do reparo salvo restart planejado separado. Playbook: `references/hermes-update-gateway-replace-collision-2026-07-05.md`.
