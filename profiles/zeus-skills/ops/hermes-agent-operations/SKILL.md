...
**Guard contra precheck canônico stale:** o precheck nunca pode manter nome hardcoded de um patch runtime antigo enquanto `ensure-hermes-mgs-patches.sh` já promoveu outro mais novo. `run-hermes-update-controlled.sh` deve descobrir o `mgs-runtime-customizations-*.patch` mais recente, verificar que o guard o referencia e testar esse mesmo artefato contra `origin/main`. Drift de patch antigo não representa o risco atual e pode bloquear/enganar a decisão de port. Quando um port novo for criado, promover o patch no guard antes do precheck final; a checagem deve falhar fechado se latest patch e guard divergirem. O padrão validado, junto com manutenção VPS em lotes, backup npm/Corepack e boundary de update Hermes sem restart, está em `references/vps-and-hermes-staged-maintenance-2026-07-10.md`.
