...
- Before creating or changing any cron, apply `/root/mgs-agent/context/cron-scheduling-policy.md`: inventory root/system crons, systemd timers, every operational Hermes profile and application schedulers; normalize timezones; allocate a start minute unused by another schedulable job; inspect duration, locks and shared resources; then repeat the global audit after exact readback. Continuous every-minute watchdogs are baseline, not a waiver.
