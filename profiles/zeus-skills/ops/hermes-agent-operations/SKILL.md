...
- Before creating or changing any cron, apply `/root/mgs-agent/context/cron-scheduling-policy.md`: inventory root/system crons, systemd timers, every operational Hermes profile and application schedulers; normalize timezones; require zero collision with another non-baseline operational job and, when continuous/dense infrastructure baselines cover all 60 minutes, allocate the least-contended minute with explicit audit; inspect duration, locks and shared resources; then repeat the global audit after exact readback.
