...
**MGS active implementation:** the dedicated corporate mailbox and automatic daily pipeline are active. Runtime contract: `/root/mgs-agent/data/finance-gam-revenue-contract.json`; canonical operating source: `/root/mgs-agent/docs/finance-gam-email-automation.md`. The first complete same-date report pair in the approved intake window validates every row and immediately invokes Meta/Google spend collection through that report date. Revenue imports and cutoff advance only after both durable spend state and the PostgreSQL spend ledger cover the same date; later schedules are recovery fallbacks. Preserve canonical `g001–g006-(d|s)` media even when a manager is operating another manager's site. When a medium has no canonical manager, combine the responsible site owner—or G002 only for explicitly shared-by-all sites—with the operation suffix. Resolve operation from an explicit suffix, then the domain's single observed suffix, then the configured current operation; block mixed or unknown operation. Any source, mapping, spend, date-gap or readback uncertainty keeps both cursor and cutoff unchanged. Every validation, lineage, cursor, dedupe, backup, recovery, revision guard and production-readback gate remains mandatory.
