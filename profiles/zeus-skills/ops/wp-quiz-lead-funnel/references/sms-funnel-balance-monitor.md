...
- Cron regular: hourly at minute 24 during Eastern hours `00,01,07–23`; no regular consultations from 02:00 through 06:59. The daily cycle resumes at 07:24 and produces 19 regular balance consultations/day.
...
7. Fetch `GET /api/daily-sents` and calculate the projection from the three most recent completed calendar days. Exclude the current partial-day bucket so the hourly execution time does not distort the average. Project the time until the available balance reaches zero using `credits / average_daily_sent`. The `7,000` threshold controls when the alert is sent; it is not subtracted from the balance projection. If the endpoint fails, keep the balance alert working and mark the projection unavailable.
