...
3. Rewarded trigger CTA must follow ActView's DOM contract:
   ...
   - For an **optional/asynchronous SMS gate**, render the initial ActView CTA as an `<a>` without `href`, `data-av-rewarded`, `av-rewarded`, or inline close handler so ActView cannot intercept the first lead-submit click. After confirmed lead success, explicit skip, or unchecked optional consent: mark the SMS gate resolved, add class-only `av-rewarded`, set `window.mgsRewardedClickInProgress`, attach the canonical inline close/no-fill callback, and replay the CTA. The skip/unchecked paths must make zero lead REST calls; the confirmed-submit path must wait for HTTP/application success before replay.
