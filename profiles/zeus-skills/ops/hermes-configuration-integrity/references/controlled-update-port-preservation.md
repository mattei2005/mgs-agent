...
- A doctor regression rewrote the active launcher while only `HERMES_HOME` was isolated. The launcher was restored byte-identically from the active venv; isolating `HOME` as well prevented recurrence and 390 overlap tests passed again with launcher hash unchanged.
