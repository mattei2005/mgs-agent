...
13. Do not create a `cronjob deliver=origin` for completion notices in MGS Discord. If a previously created bounded notifier exists, inventory its script/job ID, verify it is completed/removed, remove the orphan scheduler script, and report the cleanup; REPORT-INFRA remains the canonical asynchronous closure when no active origin turn can safely resume.
