> Demoted from standalone skill `airtable` into umbrella `productivity-workspace-apis`. Relative support paths have been re-homed: references -> `references/airtable/`, templates -> `templates/airtable/`, scripts -> `scripts/airtable/`, assets -> `assets/airtable/`.
