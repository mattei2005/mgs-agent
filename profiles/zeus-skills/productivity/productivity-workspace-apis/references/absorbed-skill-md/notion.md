> Demoted from standalone skill `notion` into umbrella `productivity-workspace-apis`. Relative support paths have been re-homed: references -> `references/notion/`, templates -> `templates/notion/`, scripts -> `scripts/notion/`, assets -> `assets/notion/`.
