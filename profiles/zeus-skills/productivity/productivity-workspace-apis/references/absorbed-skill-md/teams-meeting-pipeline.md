> Demoted from standalone skill `teams-meeting-pipeline` into umbrella `productivity-workspace-apis`. Relative support paths have been re-homed: references -> `references/teams-meeting-pipeline/`, templates -> `templates/teams-meeting-pipeline/`, scripts -> `scripts/teams-meeting-pipeline/`, assets -> `assets/teams-meeting-pipeline/`.
