...
Alert immediately in `#b011-app-rate-limit` with Rodolfo mention and title `B011 — Messenger bloqueado por permissões/BM`. Include DTR pages, Graph pages, valid-token/check counts, the operational impact (DTR may register the send while Meta rejects OAuth/no delivery), and action: pause B011 sends, request BM review/verification or move the app to a healthy BM, then resume only after advanced permissions are active and Ciro confirms a delivered canary. Repeat an unchanged blocked incident only after the standard 6-hour cooldown. When Graph pages return, post one green recovery alert and require the Ciro canary before normal volume.
