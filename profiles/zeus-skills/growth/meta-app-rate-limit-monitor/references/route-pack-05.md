...
- recurring `meta-app-roles-watch` remains enabled. B001-3 is unpaused; the dedicated B013-2 cron pause is independent.
...
Scope          Registry-driven current B001-B010 replacement lineage, including B001-3/B004-3/B005-3/B006-3. B013-2 is excluded from this script’s /roles alert path and handled by b013-dtr-link-watch.
...
Use the Meta roles cron for the current registry-driven B001–B012 app lineage. B013-2 remains on the separate DTR/ChatPion route because its users are fetched through DTR/ChatPion + Meta `debug_token`, not `/app/roles`.
...
- B013-2 is the current canonical dedicated DTR app name. Use the exact registry-pinned 1Password item, current `NO APP = B013-2`, `b013-dtr-link-watch.sh`, and state `/root/mgs-agent/data/b013-dtr-link-monitor-state.json`. The script must fail closed unless credential metadata and channel routing still match the registry. Retired B011/B012/B013 labels are historical only and must remain excluded from generic `/roles` and Sheet reconciliation.
- For Rodolfo requests like “manda alerta real em todos os 11 canais”, scope is registry-driven: the current B001–B010 replacement lineage plus B013-2. As of 2026-08-21 this is exactly `B001-3`, `B002-2`, `B003-2`, `B004-3`, `B005-3`, `B006-3`, `B007`, `B008-2`, `B009-2`, `B010-2`, and `B013-2`. B013-2 must route through the dedicated DTR monitor, never the Meta `/roles` monitor.
- Current Sheet assignment parsing must preserve alpha and hyphen suffixes such as `B001-3`, `B004-3`, `B005-3`, `B006-3`, and `B013-2`. The live assignment header is `NO APP` after the 2026-08-13 rebuild; do not normalize replacement labels to predecessor app keys, and do not fall back to blank `APP PROVISORIO` rows.
...
B013-2  #b013-2-app-status  1522830283240505385
