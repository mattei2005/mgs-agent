...
### Current B002-5, B004-6, B007-4, B009-5 and B011-4 cutovers — 2026-09-15

Rodolfo replaced five restricted generations during one continuous operation: `B002-4→B002-5`, `B004-5→B004-6`, `B007-3→B007-4`, `B009-4→B009-5` and `B011-3→B011-4`.

- Exact current items and routes: `BOT B002-5 Token - Afonso Araujo` / channel `1521251220130496723` / `expected_sheet_roles=11`; `BOT B004-6 Token - San Dra` / `1521251334496456815` / `11`; `BOT B007-4 Token - Siti Uwiyah` / `1520510823426949313` / `18`; `BOT B009-5 Token - Arif Nugroho` / `1521252284623884288` / `16`; `BOT B011-4 Token - Yani Diana Delima` / `1537256907373289575` / `15`.
- Every exact 1Password item has the matching `app_name` and all required fields. Fresh preflight plus isolated canary proved app metadata, paginated `/roles`, `/me` and `debug_token` HTTP 200 for all five; every token is valid, app-bound and tied to a new app ID; each initial role resolved with zero unresolved identities.
- The canonical Service Account Sheet has exact current assignments and zero predecessor rows: B002-5 `11`, B004-6 `11`, B007-4 `18`, B009-5 `16`, B011-4 `15`. One residual owner row for B004-5 (`I51`) and one residual B007-3 row (`I79`) were changed narrowly to the replacement label and verified by exact readback. There are no blank or duplicate segurador identities.
- Production started every replacement from fresh app-scoped state; predecessor role IDs, restriction errors and cooldowns were retired, not reused. Each route passed a contained baseline plus a second clean cycle before alert containment was removed. Both canonical pause sets are empty.
- Closure parity snapshot before later normal acceptances was B002-5 `4 present/7 X`, B004-6 `3 present/8 X`, B007-4 `2 present/16 X`, B009-5 `1 present/15 X`, and B011-4 `1 present/14 X`, with zero unmatched roles or marker mismatches. Role acceptance remains live, so later cycles may increase these counts while clearing X automatically.
- Full current 12-app cycles processed all 12 registry items with zero errors and no stale predecessor item. Real additions generated canonical alerts verified by Discord readback: B004-6/Adrielly Freitas (`1549519239499620514`, `1549519240556576860`, `1549519241168822307`) and B007-4/Marites Pattung (`1549520418216616089`, `1549520419608993974`, `1549520420829405205`, `1549520421844549683`).
- Backups: `/root/mgs-agent/backups/meta-app-b0024-to-b0025-cutover-20260915-160149/`, `/root/mgs-agent/backups/meta-app-b0045-to-b0046-cutover-20260915-155632/`, `/root/mgs-agent/backups/meta-app-b0073-to-b0074-cutover-20260915-161509/`, `/root/mgs-agent/backups/meta-app-b0094-to-b0095-cutover-20260915-162125/`, and `/root/mgs-agent/backups/meta-app-b0113-to-b0114-cutover-20260915-162932/`.

### Historical B002-4 cutover — 2026-09-07

Rodolfo replaced restricted `B002-3` with `B002-4`. Current canonical runtime:

- `B002-4` uses the exact 1Password item `BOT B002-4 Token - Sabrina Pereira`, with `app_name=B002-4`, channel `1521251220130496723` (`b002-2-app-status`) and `expected_sheet_roles=11`.
- Fresh live preflight and isolated canary proved app metadata, paginated `/roles`, `/me` and `debug_token` HTTP 200; the token is valid, app-bound and uses a different app ID from B002-3; both currently accepted roles resolved with zero unresolved identities.
- The canonical Service Account Sheet has 11 exact `NO APP=B002-4` rows, zero `B002-3` rows, and no blank or duplicate segurador identity.
- Production started from fresh B002-4 app-scoped state. B002-3 was moved to retired metadata and none of its role IDs, restriction errors or cooldowns were reused.
- The first contained production baseline reconciled the Sheet to two present/blank rows and nine absent/`X` rows. The immediate second cycle wrote nothing, returned zero errors and sent zero alerts, proving parity and deduplication.
- B002-4 was removed from both pause layers after the clean second cycle; the then-independent B009-3 pause was preserved until the B009-4 cutover below. Future role acceptances are processed and alerted by the normal shared cron.
- Cutover backup: `/root/mgs-agent/backups/meta-app-b0023-to-b0024-cutover-20260907-103037/`, with SHA-256 manifest verified.

### Historical B009-4 cutover — 2026-09-07

Rodolfo replaced restricted `B009-3` with `B009-4`. Current canonical runtime:

- `B009-4` uses the exact 1Password item `BOT B009-4 Token - Ninin Ninin`, with `app_name=B009-4`, channel `1521252284623884288` (`b009-2-app-status`) and `expected_sheet_roles=16`.
- Fresh live preflight and isolated canary proved app metadata, paginated `/roles`, `/me` and `debug_token` HTTP 200; the token is valid, app-bound and uses a different app ID from B009-3; every currently accepted role resolved with zero unresolved identities.
- The canonical Service Account Sheet has 16 exact `NO APP=B009-4` rows, zero `B009-3` rows, and no blank or duplicate segurador identity.
- Production started from fresh B009-4 app-scoped state. B009-3 was moved to retired metadata and none of its role IDs, restriction errors or cooldowns were reused.
- Accepted roles advanced live during onboarding. The contained production route converged to seven present/blank rows and nine absent/`X` rows; the next stable cycle wrote nothing, returned zero errors and sent zero alerts.
- B009-4 was removed from both pause layers after the clean stable cycle. The canonical pause file is now empty and future role acceptances are handled by the normal shared cron.
- Cutover backup: `/root/mgs-agent/backups/meta-app-b0093-to-b0094-cutover-20260907-104421/`, with SHA-256 manifest verified.

### Historical B004-5, B007-3 and B011-3 cutovers with still-current B001-5 — 2026-09-03
...
### Current B013-6 cutover — 2026-09-15

Rodolfo replaced `B013-5` with `B013-6` on the dedicated DTR/ChatPion route.

- The immutable 1Password item remains `s2dnq2ipplrfmetbj66taasmde`; its current title is `BOT B013-6 Token - Giselle Santos, backup: Emgai Datquang`, `app_name=B013-6`, and the dedicated channel remains `1522830283240505385` (`b013-2-app-status`). The title had been updated while the two non-secret profile lines in notes still named the B013-5 profiles; Zeus changed only those two metadata lines, preserved every credential value exactly and verified title/fields/notes consistency by readback.
- App metadata, app-token `/roles`, `/me` and `debug_token` returned HTTP 200; `/roles` exposed two resolved roles, the user token is valid/app-bound and the new app ID differs from B013-5.
- The canonical Service Account Sheet contains only `NO APP=B013-6` for this lineage and zero `B013-5` rows. The target set changed live `40→39→40` during cutover; audit, inventory, REPORT-INFRA, Git and session reconciliation found no attribution evidence, so this is recorded as a concurrent/unattributed Sheet scope change rather than an anomaly. The closure readback has 40 rows, zero blank/duplicate identities and no state/marker mismatch.
- A stateful isolated canary proved 39 targets, five linked, 34 confirmed unlinked, zero unknown and healthy capability with 34 DTR/Graph pages. Production started from a fresh B013-6 state and did not reuse any B013-5 account verdict, alert cooldown or restriction incident.
- During live onboarding the target set and link states continued changing normally. Closure snapshot: 40 targets, seven linked, 31 confirmed unlinked and two inconclusive rows preserved fail-closed; capability healthy with 41 DTR pages, 41 Graph pages and 41 connected pages; Sheet reconciliation required zero further writes and reported no mismatch.
- Accurate automatic change families were verified by Discord readback. Lodovitus Alem was confirmed removed (`1549540097966219404`, `1549540130241646697`, `1549540131327967333`, `1549540132690993234`, `1549540133525790803`); Alex Silva Araujo and Rian Silva were later confirmed removed (`1549541788384100378`, `1549541819400847372`, `1549541820298690622`, `1549541821653327977`); a later live Sheet addition plus three removals and two inconclusive rows were reported (`1549544943364870179`, `1549544975350501518`, `1549544976575107126`, `1549544978571853865`, `1549544979968294923`).
- Runtime cutover was the exact 18-occurrence label substitution `B013-5→B013-6` in `b013-dtr-link-watch.sh`, plus fresh registry/state. Cron jobs `498fb0d95e10` and `f42618f5ced7` remain enabled on the existing 24-hour cadence.
- Rollback backup: `/root/mgs-agent/backups/meta-app-b0135-to-b0136-cutover-20260915-172020/`.

### Historical B013-5 cutover — 2026-09-04
...
- Current immutable 1Password item ID: `s2dnq2ipplrfmetbj66taasmde`; current title: `BOT B013-5 Token - Yani Diana Delima, backup: Mawar Vetran`; `app_name=B013-5`, required credential fields are present, and the dedicated channel remains `1522830283240505385` (`b013-2-app-status`). Alert profile names are metadata read fresh from this item, never hard-coded display values.
...
- The dedicated script now pins the exact B013-5 item, requires `app_name=B013-5`, reads only `NO APP=B013-5`, renders `Meta APP - B013-5`, preserves the existing channel and displays separate `INVEST HOJE` and `INVEST 3D` columns.
...
### Historical B001-4 and B002-3 cutovers — 2026-08-28

Rodolfo explicitly replaced retired `B001-3` with `B001-4` and retired `B002-2` with `B002-3`. Historical runtime at that time:
...
- recurring `meta-app-roles-watch` remained enabled. B001-4 and B002-3 were active and unpaused at that time; both generations have since been superseded.
...
### Historical B009-3 cutover — 2026-08-28

Rodolfo explicitly replaced retired `B009-2` with `B009-3`. Historical runtime at that time:
...
- B009-3 was active and unpaused at that time; it has since been superseded by B009-4.
...
Channels       B001-5 1521251196294135858 (live channel name still b001-2-app-status); B002-5 1521251220130496723 (live channel name still b002-2-app-status); B003-3 1521251246860931223; B004-6 1521251334496456815 (live channel name still b004-3-app-status); B005-4 1521251961662341160; B006-4 1521252068319297666; B007-4 1520510823426949313; B008-3 1521252172929564744; B009-5 1521252284623884288 (live channel name still b009-2-app-status); B010-3 1521252369331916902; B011-4 1537256907373289575; B012-2 1537256951879172136
```

Use the Meta roles cron for the current registry-driven B001–B012 app lineage. B013-6 remains on the separate DTR/ChatPion route because its users are fetched through DTR/ChatPion + Meta `debug_token`, not `/app/roles`. Future B013 replacement suffixes remain on that same dedicated route by lineage.
...
- Scope is the current 12-app role registry: B001-5, B002-5, B003-3, B004-6, B005-4, B006-4, B007-4, B008-3, B009-5, B010-3, B011-4, and B012-2. B013-6 remains excluded on its dedicated DTR/ChatPion route.
...
The immediate pause from message `1544837470687076454` originally covered `B001-4`, `B004-4`, `B007-2`, and `B011-2`. Later restricted generations were replaced again. As of 2026-09-15, B001-5, B002-5, B003-3, B004-6, B005-4, B006-4, B007-4, B008-3, B009-5, B010-3, B011-4 and B012-2 are active; both pause sets are empty.
...
X-App-Usage >=85%                    risk alert; for B007-4/Openzed act fast
...
- B013-6 is the current canonical dedicated DTR app name. Resolve the exact registry-pinned 1Password item by immutable ID `s2dnq2ipplrfmetbj66taasmde`, not by its mutable title; the current title is `BOT B013-6 Token - Giselle Santos, backup: Emgai Datquang`. Require `app_name=B013-6`, current `NO APP = B013-6`, `b013-dtr-link-watch.sh`, and state `/root/mgs-agent/data/b013-dtr-link-monitor-state.json`. Every manager-facing B013 native embed—including connection summary, possible restriction, automatic restriction confirmation, capability blocked and recovery—must display `PERFIL` and `PERFIL BACKUP` read fresh from the same item. Accept complete dedicated fields, the strict title suffix `- <perfil>, backup: <perfil backup>`, or notes lines `Perfil admin/Perfil` and `Perfil backup`; when multiple complete sources exist they must agree, otherwise fail closed rather than show a stale name. The script must fail closed unless credential metadata and channel routing match the registry. Retired B013/B013-2/B013-3/B013-4 labels are historical only; B013-5 remains excluded on its dedicated route.
- For Rodolfo requests covering all app channels, scope is registry-driven: the current B001–B012 replacement lineage plus B013-6. B013-6 must route through the dedicated DTR monitor, never the Meta `/roles` monitor.
- Current Sheet assignment parsing must preserve alpha and hyphen suffixes such as `B001-5`, `B002-5`, `B003-3`, `B004-6`, `B005-4`, `B006-4`, `B007-4`, `B008-3`, `B009-5`, `B010-3`, `B011-4`, `B012-2`, and `B013-6`. The live assignment header is `NO APP` after the 2026-08-13 rebuild; do not normalize replacement labels to predecessor app keys, and do not fall back to blank `APP PROVISORIO` rows.
...
- Role/current-user table columns are standardized across the complete active B001–B013 set as `BOT EMAIL | SEGURADOR | PERFIL ID | PÁGINAS | INVEST HOJE | INVEST 3D`. Do not use `STATUS` in B013 current-user tables, and do not omit either investment field from either monitor route. `INVEST HOJE` is the Smart Bidding Messenger Daily `INVESTIMENT` for the current report date and is explicitly partial. `INVEST 3D` is the consolidated total for the three complete calendar dates immediately before the current report date; it never includes the current partial date. Never substitute or display the seven-day total. Render both values as BRL (`R$`), matching the current Smart Bidding dashboard/Sheet convention. Fetch both metrics together and lazily only when an alert table is actually being rendered; one fetch is reused across every app/table in that monitor run. A source or profile match failure renders `n/d` and must never block or delay the underlying app-health alert beyond the bounded enrichment timeout. `BOT EMAIL` comes from Google Sheet tab `Migracao 22/06`, column A / `User`; `PERFIL ID` comes from column K / `USUARIO`; `PÁGINAS` comes from column E / `PG`. Match by `Segurador` name. Prefer sheet `USUARIO`, but if a current Meta role does not match the sheet, display the Meta `/roles.user` ID as fallback instead of `sem ID` so the row remains actionable. Sort rows alphabetically by full `BOT EMAIL` so entries group by site/bot user, but display only the local part before `@` for the entire operational set (e.g. `disparosopenzed@gmail.com` → `disparosopenzed`). Rodolfo explicitly confirmed the domain is irrelevant in B013 too.
- B013 `📦 REMOVIDOS ACUMULADOS` must **not** include `MOTIVO`. If the user appears in accumulated removals, the operational state is already clear: profile/link is off/disconnected. Use the same compact schema `BOT EMAIL | SEGURADOR | PERFIL ID | PÁGINAS | INVEST HOJE | INVEST 3D` there as well; avoid reason/error text in the manager-facing accumulated list unless Rodolfo explicitly asks for diagnostics.
...
- A Sheet `OBS` value in the form `Perfil Antigo: <nome> - <perfil_id>` declares a standard segurador profile replacement. Parse the optional trailing numeric ID separately from the old name; the current row remains the canonical identity for BOT EMAIL, Segurador, USUARIO, PG and `NO APP`.
- Treat the old and current profile as one logical investment lineage. In alert enrichment, aggregate `INVEST HOJE` and `INVEST 3D` across the current Segurador name plus every parsed predecessor name, counting each normalized Smart Bidding profile key once. This preserves continuity when historical rows still use the old name, when Ciro migrates all pages to the new profile, and across a transition window split between both names.
- Suppress/remove every parsed predecessor name and profile ID from `Removidos acumulados` / confirmed-removal presentation for that replacement. Ciro's standard migration sends all pages from the segurador to the new profile and automatically unlinks the old profile in DigitalTRChat; that old unlink is planned housekeeping, not an active removal incident. Apply the same investment-lineage rule to both the generic B001–B012 renderer and the dedicated B013 renderer.
...
B013-6  #b013-2-app-status  1522830283240505385
B002-5  #b002-2-app-status  1521251220130496723
B003-3  #b003-2-app-status  1521251246860931223
B004-6  #b004-3-app-status  1521251334496456815
B005-4  #b005-3-app-status  1521251961662341160
B006-4  #b006-2-app-status  1521252068319297666
B007-4  #b007-app-rate-limit  1520510823426949313
B008-3  #b008-2-app-status  1521252172929564744
B009-5  #b009-2-app-status  1521252284623884288
B010-3  #b010-2-app-status  1521252369331916902
B011-4  #b011-app-status  1537256907373289575
B012-2  #b012-2-app-status  1537256951879172136
```

When Rodolfo asks to add humans to these channels, resolve their Discord IDs via guild member search and set explicit user permission overwrites on all 13 channels. Known validated IDs:
...
Meta App Rate Limit — B007-4/Openzed
...
```
