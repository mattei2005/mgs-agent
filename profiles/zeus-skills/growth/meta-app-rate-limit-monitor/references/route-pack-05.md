...
- B013 is the current canonical app name. Use the unique current `BOT B013 Token` item pinned by immutable ID, `NO APP = B013`, `b013-dtr-link-watch.sh`, and state `/root/mgs-agent/data/b013-dtr-link-monitor-state.json`. `BOT B013` is the retired reserve; all B011/B012 runtimes are non-operational and must remain excluded from generic `/roles` and Sheet reconciliation.
...
- Sheet `NO APP` parsing must preserve alpha suffixes such as `B013`; do not normalize `B013` to `B001`. Hyphen suffixes such as `B005-2` must also remain intact.
...
- If a profile/segurador in `Removidos acumulados` has been reassigned in the sheet to a different `NO APP` (example: B003 → B013), remove it from the old app’s accumulated removals. Accumulated removals are active incidents only while the sheet still says that user belongs to that app.
