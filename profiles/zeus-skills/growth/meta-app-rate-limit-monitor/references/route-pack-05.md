...
For “todos os 11 canais”, validate the registry-driven current set: `B001-2`, `B002-2`, `B003-2`, `B004-3`, `B005-3`, `B006-3`, `B007`, `B008-2`, `B009-2`, `B010-2`, and `B013`. B013 routes to `#b013-app-status` / `1522830283240505385`; there must be no stale alternate runtime state, script, sheet label, or alert title.
...
- B013 is the current canonical app name. Use the unique current `BOT B013 Token` item pinned by immutable ID, current `NO APP = B013`, `b013-dtr-link-watch.sh`, and state `/root/mgs-agent/data/b013-dtr-link-monitor-state.json`. The script must additionally fail closed unless the item still declares `app_name = B013` and the canonical alert channel ID; immutable-ID pinning alone is insufficient when a replacement reuses the same 1Password item. `BOT B013` is the retired reserve; all B011/B012 runtimes are non-operational and must remain excluded from generic `/roles` and Sheet reconciliation.
- For Rodolfo requests like “manda alerta real em todos os 11 canais”, scope is registry-driven: the current B001–B010 replacement lineage plus B013. As of 2026-08-17 this is exactly `B001-2`, `B002-2`, `B003-2`, `B004-3`, `B005-3`, `B006-3`, `B007`, `B008-2`, `B009-2`, `B010-2`, and `B013`. B013 must route through the DTR monitor, never the Meta `/roles` monitor.
- Current Sheet assignment parsing must preserve alpha suffixes such as `B013` and hyphen suffixes such as `B004-3`, `B005-3`, and `B006-3`. The live assignment header is `NO APP` after the 2026-08-13 rebuild; do not normalize `B013` to `B001`, and do not fall back to blank `APP PROVISORIO` rows.
