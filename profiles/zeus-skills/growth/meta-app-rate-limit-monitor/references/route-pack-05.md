...
- Final live parity after the latest scheduled full cycle was B002-5 `4 present/7 X`, B004-6 `3 present/8 X`, B007-4 `2 present/16 X`, B009-5 `1 present/15 X`, and B011-4 `1 present/14 X`, with zero unmatched roles or marker mismatches. Role acceptance remains live and these counts may increase.
...
B013 summary cards must visually follow the accepted B010 native embed: title `Meta APP - B013-5`, one concise live-data description, yellow `ATENÇÃO` only when pending profiles exist, and compact inline fields in this order: `ESTADO`, `CONTAGEM`, `PENDENTES`, `PÁGINAS`, `DTR`, `META`, followed by `PERFIL` and `PERFIL BACKUP` on the final row. Both profile values come fresh from the immutable current B013 1Password item under the consistency/fail-closed rule above. Keep DTR/Graph diagnostics out of the prose. The three-message layout remains: native summary embed, complete current-user table, then movements/confirmed removals. A force-live snapshot must not replay cached additions; show `ADICIONADOS AGORA` as `Nenhum.` unless that same fresh run proves a real addition. Omit the entire inconclusive section when there are no inconclusive profiles.
...
- Scope is the current 12-app role registry: B001-5, B002-5, B003-3, B004-6, B005-4, B006-4, B007-4, B008-3, B009-5, B010-3, B011-4, and B012-2. B013-5 remains excluded on its dedicated DTR/ChatPion route.
...
- Current Sheet assignment parsing must preserve alpha and hyphen suffixes such as `B001-5`, `B002-5`, `B003-3`, `B004-6`, `B005-4`, `B006-4`, `B007-4`, `B008-3`, `B009-5`, `B010-3`, `B011-4`, `B012-2`, and `B013-5`. The live assignment header is `NO APP` after the 2026-08-13 rebuild; do not normalize replacement labels to predecessor app keys, and do not fall back to blank `APP PROVISORIO` rows.
