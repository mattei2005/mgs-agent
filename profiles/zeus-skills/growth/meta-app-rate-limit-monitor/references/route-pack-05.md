...
If the same B013 run detects material link changes and `FORCE_LIVE_ALERT=1`, the automatic fresh change alert is the single canonical delivery. Automatic change alerts are event-based: `REMOVIDOS CONFIRMADOS` contains only profiles whose verdict first entered `unlinked_confirmed` in that cycle, so each confirmed removal is detailed exactly once. Later alerts triggered by unrelated additions or recoveries must not replay the persistent failure list; omit the entire confirmed-removals section when there is no newly confirmed removal. Keep the current pending count in the summary card and the persistent `X` in the Sheet/state until recovery. A manual force with no material change is an explicitly requested current-state snapshot and may list all currently confirmed failures once. Suppress the second force-snapshot family when the same run has material changes, but keep the embed description as `Alerta live solicitado` because the operator initiated the run. After any delivery bug, keep the accurate change alert, delete only the duplicate snapshot messages after Critical Subset confirmation, verify readback and persist `alerts_sent=1`.
