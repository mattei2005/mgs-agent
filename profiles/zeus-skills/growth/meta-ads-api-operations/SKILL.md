...
5. Run a real read-only Meta preflight plus a scheduler gate smoke that proves pre-start stdout is empty and the state hash is unchanged. A validated setup is still `scheduled`, not `operationally proven`, until the first natural scheduler tick creates and reads back an account.
6. After each successful tick, validate the exact Ad Account ID, Business owner, the selected account's semantic `People` tab, expected person with Full access and absence of payment changes before advancing `next_due_at` and emitting progress.
7. A visual mutation error permits at most one later controlled retry only after exact no-side-effect reconciliation. Reauth, security, maximum-account, multiple-new-ID or access-readback ambiguity blocks future writes and preserves the checkpoint.
