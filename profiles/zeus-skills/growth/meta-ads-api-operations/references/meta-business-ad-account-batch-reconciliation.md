...
- Scope access verification to the selected asset. Whole-page text can contain stale or unrelated `0 people`/`1 person` counts from virtualized rows. On the direct asset URL, click `page.getByRole('tab', {name: 'People', exact: true})`; wait first for the selected-account assignment sentence and then for the expected person plus `Full access`. Do not classify a temporarily absent lazy-loaded person row as `0 people`.
