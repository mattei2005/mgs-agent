...
## 📜 Documento mestre — AGENT.md

`/root/mgs-agent/AGENT.md` é a fonte canônica para autorização, níveis operacionais, Critical Subset, validação, erros e padrões de relatório.

Regra operacional para economizar tempo:
- Siga AGENT.md como política já carregada no contexto.
- NÃO leia AGENT.md automaticamente em toda tarefa.
- Leia AGENT.md sob demanda apenas quando a tarefa envolver autorização, segurança, Critical Subset, mudança estrutural, dúvida de escopo ou conflito entre regras.
- Para REC direto completo, a regra prevalente é: `mgs-rec-runner.py` uma vez → JSON → resumo final.
