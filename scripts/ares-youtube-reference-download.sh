...
# Download/analyze YouTube reference videos for Ares using a persistent authenticated cookie jar.
# Do NOT commit cookies. Default cookie path lives under the Ares profile private data dir.
...
OUT_DIR="${2:-/root/mgs-agent/data/ares/creative-ops/references/youtube}"
COOKIE_FILE="${ARES_YOUTUBE_COOKIES:-/root/.hermes/profiles/ares/secrets/youtube-cookies.txt}"
PROFILE_DIR="${ARES_YOUTUBE_PROFILE:-/root/.hermes/profiles/ares/browser-profiles/youtube-chromium}"
PROBE_SCRIPT="/root/mgs-agent/scripts/ares-youtube-persistent-browser.py"
...
if [[ -x "$PROBE_SCRIPT" ]] && command -v xvfb-run >/dev/null 2>&1; then
  if xvfb-run -a "$PROBE_SCRIPT" "$URL" --headed --out-dir "$OUT_DIR/persistent-probe" >/tmp/ares-youtube-persistent-probe.log 2>&1; then
    if uvx yt-dlp \
    ...
    tail -40 /tmp/ares-youtube-persistent-probe.log >&2 || true
  fi
fi
...
ffmpeg -y -i "$VIDEO_PATH" -vf 'fps=1,scale=540:-1' "$OUT_DIR/frames/frame_%03d.jpg" >/tmp/ares-youtube-frames.log 2>&1
