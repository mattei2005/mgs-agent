#!/usr/bin/env python3
"""Read-only Google Drive inventory for MGS-CRIATIVOS/UPLOAD_CANVAS.
...
UPLOAD_FOLDER_NAME = "UPLOAD_CANVAS"
...
def inventory(client: DriveClient) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    upload = next((f for f in root_children if f.get("name") == UPLOAD_FOLDER_NAME), None)
    if not upload:
        raise RuntimeError(f"{UPLOAD_FOLDER_NAME} not found under root folder")

    queue = collections.deque([(upload["id"], UPLOAD_FOLDER_NAME)])
    ...
    while queue:
        ...
        for child in client.list_children(parent_id):
            ...
            if child.get("mimeType") == FOLDER_MIME:
                ...
                continue
            top_folder = rel_path.split("/")[1] if "/" in rel_path else ""
