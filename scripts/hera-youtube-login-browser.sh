...
# Temporary noVNC session for logging Hera's persistent Chromium profile into YouTube.
...
STOP_FILE="/tmp/hera-youtube-login-browser.stop"
PID_FILE="/tmp/hera-youtube-login-browser.pid"
...
Xvfb "$DISPLAY" -screen 0 1365x900x24 -ac >"$LOG_DIR/hera-youtube-xvfb.log" 2>&1 &
XVFB_PID=$!
sleep 1
x11vnc -display "$DISPLAY" -localhost -nopw -forever -shared -rfbport "$VNC_PORT" >"$LOG_DIR/hera-youtube-x11vnc.log" 2>&1 &
X11VNC_PID=$!
websockify --web=/usr/share/novnc/ "127.0.0.1:${NOVNC_PORT}" "127.0.0.1:${VNC_PORT}" >"$LOG_DIR/hera-youtube-novnc.log" 2>&1 &
NOVNC_PID=$!

DISPLAY="$DISPLAY" PROFILE_DIR="$PROFILE_DIR" STOP_FILE="$STOP_FILE" python3 - <<'PY' >"/root/mgs-agent/logs/hera-youtube-login-browser.log" 2>&1 &
import os, time
