...
exec > >(tee -a "$LOG") 2>&1
printf '[%s] restarting Zeus/Atena/Ares/Hera gateways after Hermes a8 update\n' "$(date -Iseconds)"
systemctl restart zeus-gateway.service atena-gateway.service ares-gateway.service hera-gateway.service
sleep 20
printf '[%s] service active checks\n' "$(date -Iseconds)"
for s in zeus-gateway.service atena-gateway.service ares-gateway.service hera-gateway.service; do
  systemctl is-active --quiet "$s"
done
systemctl show zeus-gateway.service atena-gateway.service ares-gateway.service hera-gateway.service -p Id -p ActiveState -p SubState -p MainPID -p NRestarts -p ExecMainStatus --no-pager
