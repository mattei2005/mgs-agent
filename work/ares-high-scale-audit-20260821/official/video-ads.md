Title: Video and Carousel Ads | Developer Documentation

URL Source: http://developers.facebook.com/docs/marketing-api/guides/videoads/
...
*   Step 1: [Provide ad creatives](http://developers.facebook.com/docs/marketing-api/guides/videoads/#create-ad-creative)
*   Step 2: [Create ad campaign](http://developers.facebook.com/docs/marketing-api/guides/videoads/#create-ad-campaign)
*   Step 3: [Create an ad set](http://developers.facebook.com/docs/marketing-api/guides/videoads/#create-ad-set)
*   Step 4: [Create an ad](http://developers.facebook.com/docs/marketing-api/guides/videoads/#create-ad)
...
*   [a video uploaded](http://developers.facebook.com/docs/graph-api/video-uploads) to either the `act_{ad-account-id}/advideos` endpoint
...
If your goal is lowest cost-per-view possible, pair the video view campaign objective with an ad set’s `optimization_goal=THRUPLAY`. You can set `bidding_event` to `IMPRESSIONS` or `THRUPLAY`, to pay per impression or per video view. See [CPV bidding](http://developers.facebook.com/documentation/ads-commerce/marketing-api/bidding/guides/cost-per-action-ads).
...
Cost-per-view rates are lower for ad sets with `optimization_goal=THRUPLAY` compared to CPVs from Reach and Frequency buying optimized for video views. The end date must be in the future. See [Reference: Ad Set](http://developers.facebook.com/documentation/ads-commerce/marketing-api/reference/ad-campaign).
...
When a campaign objective is `VIDEO_VIEWS`, by default the ad gets the right [tracking specs](http://developers.facebook.com/documentation/ads-commerce/marketing-api/tracking-specs) which define actions tracked for an ad. For example, video views:

`{'action.type':'video_view','post':'POST_ID','post.wall':'PAGE_ID'}`

To extend the reach of a video to more people, use the video view campaign objective with [Reach and Frequency](http://developers.facebook.com/docs/reference/ads-api/reachandfrequency). You need to create a prediction, reserve it, and assign it to your ad set.

Follow the [video view creation](http://developers.facebook.com/docs/marketing-api/guides/videoads/#video_create), but apply Reach and Frequency to your ad set. Specify these additional parameters:

`-F "rf_prediction_id=<RESERVATION_ID>" \`

*   **Reach people who watched a video**. From awareness to affinity and consideration. See [remarketing](http://developers.facebook.com/docs/marketing-api/guides/videoads/#remarketing).
*   **Engage with brand and products**. Add a call-to-action to visit a specific page on your website. See [call to action](http://developers.facebook.com/docs/marketing-api/guides/videoads/#call_to_action).
...
*   `PAGE_LIKES`
*   [`LEAD_GENERATION`](http://developers.facebook.com/documentation/ads-commerce/marketing-api/guides/lead-ads/create#video)
*   [`LOCAL_AWARENESS`](http://developers.facebook.com/documentation/ads-commerce/marketing-api/guides/event-ads)
*   `LINK_CLICKS`
*   `CONVERSIONS`
*   [`APP_INSTALLS`](http://developers.facebook.com/documentation/ads-commerce/marketing-api/mobile-app-ads#create_video)
*   `VIDEO_VIEWS`
*   `BRAND_AWARENESS`
*   [Mobile app engagement](http://developers.facebook.com/docs/app-ads/formats/ad-for-mobile-app)
...
**The `action_video_type` option is in limited testing.** To identify clients with the breakdown, check `CAN_USE_VIDEO_METRICS_BREAKDOWN` for the [ad account](http://developers.facebook.com/documentation/ads-commerce/marketing-api/reference/ad-account).
...
*   Create an ad and unpublished page post in one call: [ad creative API](http://developers.facebook.com/docs/reference/ads-api/adcreative).
*   Create an [unpublished Page post](http://developers.facebook.com/docs/reference/ads-api/unpublished-page-posts) then create an [ad creative](http://developers.facebook.com/docs/reference/ads-api/adcreative) using the post (not available for video carousel).

**Carousel ads are not supported for Facebook Stories.**

Create a carousel ad page post while creating an ad creative. Specify the page post content in `object_story_spec`, which creates an unpublished page post from `adcreatives`. See [ad creatives](http://developers.facebook.com/docs/reference/ads-api/adcreative#object_story_spec). For example:
...
Create an unpublished Page post. `child_attachments` is an [array of link objects](http://developers.facebook.com/docs/marketing-api/guides/videoads/#spec). On each link object, `picture`, `name` and `description` are optional. You can post these as by the Page only with a Page access token.
...
![Image 1: Carousel Ad on iOS annotated with callouts mapping fields like message, child_attachments, name, description, picture, link, and caption](https://scontent-atl3-2.xx.fbcdn.net/v/t39.2365-6/592526578_1378922793966371_6556244734577500002_n.png?_nc_cat=105&ccb=1-7&_nc_sid=e280be&_nc_ohc=wLb5Z-0TT64Q7kNvwFFt_Iy&_nc_oc=Adr62lxThOuKqeU8kfVl5RpqXTmmjnK32QcN5JopwUDoy4rkuCA30gsMpz2IcoeHOlU&_nc_zt=14&_nc_ht=scontent-atl3-2.xx&_nc_gid=l-icyB9_dNaam2tutWRcjw&_nc_ss=7b289&oh=00_AQGDB97zbqobXBIivSn4ZgRU6e_ZTsE3Ad8gIKU6gIJiJA&oe=6AA20852)
...
| `child_attachments.image_hash` type: string | Hash of preview image associated with the link from your [image library](http://developers.facebook.com/documentation/ads-commerce/marketing-api/reference/ad-image); use 1:1 aspect ratio and a minimum of 458 x 458 px for best display. Either `picture` or `image_hash` must be specified. |
...
| `child_attachments.call_to_action` type: object | Optional call to action. See [Call To Action](http://developers.facebook.com/docs/marketing-api/unpublished-page-posts#cta-spec). You do not have to specify the link again in `call_to_action:{'value':{'link':... }}}`. |
| `child_attachments.video_id` type: string | ID of the [ad video](http://developers.facebook.com/documentation/ads-commerce/marketing-api/reference/ad-account/advideos). Can be used in any child element. If specified, must also set `image_hash` or `picture`. |
...
*   Carousel breakdown metrics for `action_report_time=impression` are inaccurate before June 20, 2015.
*   Carousel breakdown metrics for `action_report_time=conversion` are inaccurate before July 20, 2015.

If you only select `right_hand_column` as your placement, you can only use a single-video or carousel format in your ad group. The video format is not supported with only a `right_hand_column` placement selected. See [Advanced Targeting and Placement](http://developers.facebook.com/documentation/ads-commerce/marketing-api/audiences/reference/advanced-targeting).
...
*   [Upload Videos to Facebook Guide](http://developers.facebook.com/docs/graph-api/video-uploads)
*   [Carousel for Mobile App Ads](http://developers.facebook.com/ads/blog/post/2015/05/11/carousel-app-ads)
*   [Page Feed Graph API Reference](http://developers.facebook.com/docs/graph-api/reference/page/feed)
*   [Unpublished Page Posts](http://developers.facebook.com/docs/reference/ads-api/unpublished-page-posts)
*   [Ad Creatives](http://developers.facebook.com/docs/reference/ads-api/adcreative)
*   [Instagram Carousel Ads](http://developers.facebook.com/documentation/ads-commerce/marketing-api/guides/instagramads#carousel)
