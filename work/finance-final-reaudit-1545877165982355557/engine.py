...
def evaluate(node,book,sheet,cell):
 ...
 if fn=='MIN':return min(v for x in vals for v in flat(x) if isinstance(v,(int,float)))
 if fn=='SUMIF':
  import fnmatch
  data=flat(vals[0]);criterion=str(scalar(vals[1]));amounts=flat(vals[2]) if len(vals)>2 else data
  return sum(v for h,v in zip(data,amounts) if isinstance(v,(int,float)) and fnmatch.fnmatchcase(str(h),criterion))
