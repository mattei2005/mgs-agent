...
async def main():
 ...
 async with async_playwright() as pw:
  ...
  save_meta=await p.evaluate("""()=>{const el=document.querySelector('.action-button-save');const ev=(window.jQuery&&el)?(jQuery._data(el,'events')||{}):{};const vue=[];let v=el?.__vue__;while(v&&vue.length<8){vue.push({name:v.$options?.name||'',methods:Object.keys(v.$options?.methods||{}),events:Object.keys(v._events||{}),keys:Object.keys(v).filter(k=>/save|flow|editor/i.test(k))});v=v.$parent}return {outerHTML:el?.outerHTML||'',onclick:el?.getAttribute('onclick')||'',events:Object.fromEntries(Object.entries(ev).map(([k,v])=>[k,v.map(x=>String(x.handler))])),vue,scripts:Array.from(document.scripts).map(s=>s.src).filter(Boolean)}}""")
  ...
  result={'source_nodes':len(source['nodes']),'source_reachable':reachable(source),'before_nodes':len(before['nodes']),'before_reachable':reachable(before),'dry_nodes':len(dry['nodes']),'dry_reachable':reachable(dry),'dry_equals_source':dry==source,'api':api,'save_meta':save_meta}
