...
AUTH_MESSAGE_ID = '1540458246094065795'
TARGET_NAMES = [f'zy-gb-cc-en-drip {i:03d}' for i in range(1, 7)]
...
def sequence_index(route_name):
    if route_name == 'zy-gb-cc-en-drip-m0-1':
        return 0
    if route_name == 'zy-gb-cc-en-drip-nm':
        return 1
    match = re.fullmatch(r'zy-gb-cc-en-drip-m([1-9]|1[0-9]|2[0-8])-1', route_name)
    if not match:
...
def expected_groups():
    sequence = ['zy-gb-cc-en-drip-m0-1', 'zy-gb-cc-en-drip-nm'] + [f'zy-gb-cc-en-drip-m{i}-1' for i in range(1, 29)]
