...
RUN = BASE / 'work/sb-smart-routing/20260824T200031-0400-finanzas-four-publishers-us-cc-es-five-url'
BACKUP = BASE / 'backups/sb-smart-routing/20260824T200031-0400-finanzas-four-publishers-us-cc-es-five-url'
...
def route_core(r):
    return {'route':str(r.get('route') or '').strip(),'utm_content':str(r.get('utm_content') or '').strip(),'url':norm_url(r.get('url')),'jbf_operation':str(r.get('jbf_operation') or '').strip(),'healthy':bool(r.get('healthy',True)),'freeze':bool(r.get('freeze',False))}
...
def expected_drip_pairs(spec,all_routes):
    ...
    if sorted(extras)!=sorted(allowed): raise RuntimeError(f"{spec['slug']}: unexpected drip extras {extras}")
