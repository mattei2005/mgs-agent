...
class MGS_Quiz_Admin {
    ...
    private static function sms_default_presets() {
        return array(
            'G001' => array( 'preset_id' => 'G001', 'gestor_code' => 'G001', 'label' => 'G001 – Icaro',   'url' => 'https://v2.smsfunnel.com.br/integrations/lists/019ef608-21bf-70db-9ae2-26980d501d61/add-lead' ),
            'G002' => array( 'preset_id' => 'G002', 'gestor_code' => 'G002', 'label' => 'G002 – Geizian', 'url' => 'https://v2.smsfunnel.com.br/integrations/lists/019e4820-49be-734a-920a-9a15633f1cfd/add-lead' ),
            'G003' => array( 'preset_id' => 'G003', 'gestor_code' => 'G003', 'label' => 'G003 – Isliago', 'url' => 'https://v2.smsfunnel.com.br/integrations/lists/019ef5da-5176-723c-8de4-261e4777dba3/add-lead' ),
            'G004' => array( 'preset_id' => 'G004', 'gestor_code' => 'G004', 'label' => 'G004 – Joe',     'url' => 'https://v2.smsfunnel.com.br/integrations/lists/019ef608-5249-705a-9bda-df396a2f15d7/add-lead' ),
            'G005' => array( 'preset_id' => 'G005', 'gestor_code' => 'G005', 'label' => 'G005 – Kelly',   'url' => 'https://v2.smsfunnel.com.br/integrations/lists/019ef21e-3714-72e8-928c-6c731604631f/add-lead' ),
            'G006' => array( 'preset_id' => 'G006', 'gestor_code' => 'G006', 'label' => 'G006 – Nicolas', 'url' => 'https://v2.smsfunnel.com.br/integrations/lists/019ef608-7084-7075-b0d0-b0764085bbfc/add-lead' ),
        );
    }
    ...
    private static function normalize_sms_preset_id( $preset_id ) {
        $preset_id = trim( sanitize_text_field( (string) $preset_id ) );
        if ( preg_match( '/^G[0-9]{3,}$/i', $preset_id ) ) return strtoupper( $preset_id );
        return substr( preg_replace( '/[^a-zA-Z0-9_-]/', '', $preset_id ), 0, 80 );
    }

    private static function new_sms_preset_id( $presets ) {
        do {
            $preset_id = 'sms_' . str_replace( '-', '', wp_generate_uuid4() );
        } while ( isset( $presets[ $preset_id ] ) );
        return $preset_id;
    }

    private static function sort_sms_presets( &$presets ) {
        uasort( $presets, function( $a, $b ) {
            $by_code = strnatcasecmp( (string) ( $a['gestor_code'] ?? '' ), (string) ( $b['gestor_code'] ?? '' ) );
            return 0 !== $by_code ? $by_code : strnatcasecmp( (string) ( $a['label'] ?? '' ), (string) ( $b['label'] ?? '' ) );
        } );
    }

    private static function build_sms_presets_from_input( $preset_ids, $codes, $labels, $urls, $required_ids = array() ) {
        $preset_ids = array_values( is_array( $preset_ids ) ? $preset_ids : array() );
        ...
        foreach ( $codes as $idx => $raw_code ) {
            ...
            $preset_id = self::normalize_sms_preset_id( $preset_ids[ $idx ] ?? '' );
            if ( ! $preset_id ) $preset_id = self::new_sms_preset_id( $presets );
            if ( isset( $presets[ $preset_id ] ) ) return new WP_Error( 'duplicate_id', 'Identificador interno de lista duplicado.' );
            $presets[ $preset_id ] = array( 'preset_id' => $preset_id, 'gestor_code' => $code, 'label' => $label, 'url' => $url );
        }
        ...
        foreach ( (array) $required_ids as $required_id ) {
            $required_id = self::normalize_sms_preset_id( $required_id );
            if ( $required_id && ! isset( $presets[ $required_id ] ) ) {
                return new WP_Error( 'missing_list', 'Uma lista existente não pode ser removida nesta tela.', $required_id );
            }
        }
        self::sort_sms_presets( $presets );
        return $presets;
    }
    ...
    private static function sms_presets() {
        ...
        foreach ( $saved as $saved_id => $preset ) {
            if ( ! is_array( $preset ) ) continue;
            $preset_id = self::normalize_sms_preset_id( $preset['preset_id'] ?? $saved_id );
            $code = self::normalize_sms_code( $preset['gestor_code'] ?? $saved_id );
            $label = sanitize_text_field( $preset['label'] ?? '' );
            $url = esc_url_raw( $preset['url'] ?? '' );
            if ( $preset_id && $code && $label && self::is_valid_sms_url( $url ) ) {
                $presets[ $preset_id ] = array( 'preset_id' => $preset_id, 'gestor_code' => $code, 'label' => $label, 'url' => $url );
            }
        }
        self::sort_sms_presets( $presets );
    ...
    private static function sms_preset_id_for_row( $sms_row, $presets ) {
        $preset_id = self::normalize_sms_preset_id( $sms_row['preset_id'] ?? '' );
        if ( $preset_id && isset( $presets[ $preset_id ] ) ) return $preset_id;

        $code = self::normalize_sms_code( $sms_row['gestor_code'] ?? '' );
        $url = esc_url_raw( $sms_row['url'] ?? '' );
        $label = sanitize_text_field( $sms_row['label'] ?? '' );
        foreach ( $presets as $candidate_id => $preset ) {
            if ( $code === $preset['gestor_code'] && $url && $url === $preset['url'] ) return $candidate_id;
        }
        foreach ( $presets as $candidate_id => $preset ) {
            if ( $code === $preset['gestor_code'] && $label && $label === $preset['label'] ) return $candidate_id;
        }
        if ( $code && isset( $presets[ $code ] ) ) return $code;
        foreach ( $presets as $candidate_id => $preset ) {
            if ( $code && $code === $preset['gestor_code'] ) return $candidate_id;
        }
        return '';
    }
    ...
    private static function handle_sms_presets_save( $t ) {
        ...
        $preset_ids = isset( $_POST['sms_preset_ids'] ) && is_array( $_POST['sms_preset_ids'] ) ? wp_unslash( $_POST['sms_preset_ids'] ) : array();
        ...
        $presets = self::build_sms_presets_from_input( $preset_ids, $codes, $labels, $urls, array_keys( $existing ) );
        ...
        foreach ( (array) $rows as $row ) {
            ...
            foreach ( $sms_rows as &$sms_row ) {
                ...
                $preset_id = self::sms_preset_id_for_row( $sms_row, $presets );
                if ( ! $preset_id || ! isset( $presets[ $preset_id ] ) ) continue;
                $preset = $presets[ $preset_id ];
                if ( ( $sms_row['preset_id'] ?? '' ) !== $preset_id || ( $sms_row['gestor_code'] ?? '' ) !== $preset['gestor_code'] || ( $sms_row['label'] ?? '' ) !== $preset['label'] || ( $sms_row['url'] ?? '' ) !== $preset['url'] ) {
                    $sms_row['preset_id'] = $preset_id;
                    $sms_row['gestor_code'] = $preset['gestor_code'];
                    $sms_row['label'] = $preset['label'];
                    $sms_row['url'] = $preset['url'];
                    $changed = true;
                }
            }
            unset( $sms_row );
