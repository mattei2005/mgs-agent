...
class MGS_Quiz_Admin {
    ...
    private static function days_chart_data( $filters ) {
        ...
        $sql = "SELECT DATE(created_at) d, COUNT(*) c FROM {$lead_t} {$where} GROUP BY DATE(created_at) ORDER BY d DESC";
    ...
    private static function render_report_leads_inner( $leads, $total, $page, $pages ) {
        ...
        foreach ( $leads as $l ) {
            echo '<tr><td class="col-date">'.esc_html( date_i18n( 'd/m/Y, H:i', strtotime( $l['created_at'] ) ) ).'</td><td class="col-name">'.esc_html($l['name']).'</td><td class="col-phone">'.esc_html($l['phone']).'</td><td class="col-parcela">'.esc_html($l['parcela']).'</td><td class="col-gestor">'.esc_html(strtoupper($l['utm_medium'])).'</td><td class="col-campanha">'.esc_html($l['utm_campaign']).'</td><td class="col-sms">'.esc_html($l['sms_funnel_status']).'</td></tr>';
        }
    ...
    public static function render_leads() {
        ...
        foreach ( (array) $rows as $r ) {
            echo '<tr><td class="nowrap">'.esc_html($r['id']).'</td><td>'.esc_html( date_i18n( 'd/m/Y, H:i', strtotime( $r['created_at'] ) ) ).'</td><td>'.esc_html($r['quiz_slug']).'</td><td>'.esc_html($r['name']).'</td><td>'.esc_html($r['phone']).'</td><td>'.esc_html($r['parcela']).'</td><td>'.esc_html($r['utm_source']).'</td><td class="nowrap">'.esc_html(strtoupper($r['utm_medium'])).'</td><td>'.esc_html($r['fbclid']).'</td><td>'.esc_html($r['gclid']).'</td><td class="nowrap">'.esc_html($r['sms_funnel_status']).'</td></tr>';
        }
        echo '</tbody></table></div></div>';
    }
