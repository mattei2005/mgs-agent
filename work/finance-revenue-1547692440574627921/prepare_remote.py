...
ssh('if ! test -e '+shlex.quote(STAGE)+'; then sudo -n -u mgsfinance mkdir -p '+shlex.quote(STAGE)+' && sudo -n -u mgsfinance chmod 700 '+shlex.quote(STAGE)+'; fi; if ! test -e '+shlex.quote(BACKUP)+'; then sudo -n mkdir -p '+shlex.quote(BACKUP)+' && sudo -n chown zeus:zeus '+shlex.quote(BACKUP)+' && chmod 700 '+shlex.quote(BACKUP)+'; fi; test -d '+shlex.quote(STAGE)+' && test -d '+shlex.quote(BACKUP)+" && test \"$(stat -c %U:%G:%a "+shlex.quote(STAGE)+')\" = mgsfinance:mgsfinance:700 && test "$(stat -c %U:%G:%a '+shlex.quote(BACKUP)+')" = zeus:zeus:700')
...
ssh(PG+'pg_restore --list '+shlex.quote(dump)+' >/dev/null')
...
counts=ssh(PG+'psql -h '+SOCK+' -U mgs_pg -d '+DB+' -At -F "|" -c '+shlex.quote("SELECT (SELECT count(*) FROM scenarios),(SELECT count(*) FROM source_cells),(SELECT revision FROM scenarios WHERE id='workspace-2026-09')")).strip().split('|');assert counts[1]=='85868' and int(counts[2])==int(before['revision'])
files={p:dict(sha256=ssh('sha256sum '+shlex.quote(p)).split()[0],size=int(ssh('stat -c %s '+shlex.quote(p)).strip())) for p in [scenario,dump,STAGE+'/import-plan.json',STAGE+'/apply_revenue.mjs']}
