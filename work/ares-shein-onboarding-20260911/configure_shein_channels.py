...
- Fase atual: foco em criacao do zero, duplicacao e clone. Nao ha thread fixa; as seis threads inicialmente abertas foram excluidas por pedido de Rodolfo. Diario, Intraday/Otimizacao, Criativos e automacoes de relatorio ficam adiados ate Rodolfo avancar a fase.
...
- Criativos exigem idioma compativel, metadata limpa, reserva e reconciliacao Drive x Meta. `MGS-AGENTS/CRIATIVOS/SHEIN_US_EN` e o pool EN atual; `01_READY` nao prova elegibilidade ou ineditismo. `SHEIN_US_ES` ainda nao existe e sera criada apenas quando o intake espanhol comecar; EN e ES diferem somente pela lingua, sem reutilizar asset EN como se fosse ES.
- Cada gestor usara um perfil anunciante e um User Access Token proprio. O segredo fica no 1Password; nunca pedir ou aceitar token no Discord. Separar usuarios/tokens pode isolar o limite por usuario, mas nao remove limites compartilhados da Marketing API por conta, app, Business Use Case, access tier, burst ou objeto.
...
- Titulos de threads: etiqueta semantica curta de 3 a 6 palavras; nao sobrescrever titulo manual. Nao criar threads fixas nem threads de relatorio/otimizacao nesta fase.
"""
