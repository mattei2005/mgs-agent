...
- **MFA all-user preparado — `1548894447838695546`/`1548894912529961053`/`1548905421199118467`:** suporte individual autoassistido foi ampliado para Rodolfo, Geizian, Ícaro, Joe, Isliago, Kelly e Nicolas. Cada identidade recebe QR/segredo próprio, TOTP compatível com Google/Microsoft Authenticator, Authy e 1Password, dez recovery codes de uso único, bloqueio de replay e segredo AES-256-GCM no banco. Rodolfo terá seu material no próprio 1Password; os demais usam Google Authenticator ou compatível. `Confiar neste dispositivo por 30 dias` foi implementado com cookie aleatório `__Host-` HttpOnly/Secure/SameSite=Strict, hash no banco, vínculo a usuário/user-agent, máximo de cinco dispositivos e revogação no logout; ele dispensa somente TOTP após senha correta e não amplia a sessão de 8h/idle 30min. Schemas `auth_mfa` e `auth_trusted_devices` foram criados com zero linhas e grants mínimos; código/UI/dependência foram publicados com `mfa_required=false`, mantendo todos os logins atuais inalterados até o gate crítico final.
...
1. MFA de todos: `auth_mfa` e `auth_trusted_devices` existem, ambas com zero linhas e somente SELECT/INSERT/UPDATE para o role da aplicação. `mfa_required=false` e chave mestra ausente. Sete identidades continuam com senha até o cutover; Rodolfo será pré-configurado no 1Password e os demais farão enrollment próprio no próximo login. Trust opcional dispensa somente TOTP por 30 dias no mesmo navegador.
...
Hardening systemd, restore materializado e bloqueio do origin estão concluídos. O último full pré-MFA é `1_1Wpk6caJIMvfxs3b27HY5bmzj0b3h0f`, com 6 componentes, infraestrutura 17/17 e restore PASS, sem retenção destrutiva. O suporte MFA e trust de 30 dias para todos está publicado mas desligado; falta criar a chave mestra, pré-configurar apenas Rodolfo no 1Password, ativar `mfa_required=true`/`mfa_trust_days=30` e revogar as sessões atuais sob confirmação crítica. Os outros seis usuários configurarão o próprio Google Authenticator ou compatível no próximo login.
