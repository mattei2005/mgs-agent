...
Hardening systemd, restore materializado e bloqueio do origin estão concluídos. O último full pré-MFA é `1YqlYB_yULBVTRUA_KQUK_7m95G_4ED71`, com 6 componentes, infraestrutura 17/17 e restore PASS, sem retenção destrutiva. O suporte MFA e trust de 30 dias para todos está publicado mas desligado; falta criar a chave mestra, pré-configurar apenas Rodolfo no 1Password, ativar `mfa_required=true`/`mfa_trust_days=30` e revogar as sessões atuais sob confirmação crítica. Os outros seis usuários configurarão o próprio Google Authenticator ou compatível no próximo login.
