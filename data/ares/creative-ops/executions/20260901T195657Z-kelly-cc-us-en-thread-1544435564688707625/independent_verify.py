...
for item in plan['items']:
    ...
    source_ok = src.get('parents') == [report['legacy_parent_id']] and src.get('name') == item['source_filename'] and not src.get('trashed')
    dest_ok = dst.get('parents') == [report['ready_parent_id']] and dst.get('name') == item['destination_filename'] and not dst.get('trashed')
    if not source_ok:
    ...
    inv_ok = bool(inv and inv.get('asset_drive_id') == did and inv.get('canonical_filename') == item['destination_filename'] and inv.get('metadata_clean') is True and inv.get('reservation_status') == 'RESERVADO_PELO_GESTOR' and inv.get('ares_eligible') is False)
    if not inv_ok:
...
target = next(x for x in plan['items'] if x['source_filename'].startswith('10_22 '))
target_inv = by_source.get(target['source_drive_id'])
dup_src = mod.get_file(drive, DUP_ID)
dup_ok = bool(target_inv and DUP_ID in (target_inv.get('duplicate_source_drive_ids') or []) and dup_src.get('parents') == [report['legacy_parent_id']] and not dup_src.get('trashed'))
if not dup_ok:
    errors.append('duplicate source linkage/readback mismatch')
source_ids.append(DUP_ID)
dest_ids.append(report['items'][DUP_ID]['destination_drive_id'])

reject_src = mod.get_file(drive, REJECT_ID)
reject_inv = by_source.get(REJECT_ID)
reject_ok = bool(
    reject_src.get('parents') == [report['rejected_parent_id']]
    and not reject_src.get('trashed')
    and reject_inv
    and reject_inv.get('status') == '05_REJECTED'
    and reject_inv.get('performance_label') == 'REJECTED_TECHNICAL'
    and reject_inv.get('metadata_clean') is False
    and reject_inv.get('ares_eligible') is False
    and not reject_inv.get('asset_drive_id')
    and not reject_inv.get('canonical_filename')
)
if not reject_ok:
    errors.append('rejected source Drive/inventory mismatch')
source_ids.append(REJECT_ID)
...
result = {
    ...
    'legacy_sources_verified': sum(x['source_parent_verified'] for x in items) + int(dup_ok),
    'rejected_sources_verified': int(reject_ok),
    'inventory_primary_rows_verified': sum(x['inventory_verified'] for x in items) + int(reject_ok),
    'inventory_duplicate_links_verified': int(dup_ok),
    'reservation_fail_closed_verified': sum(x['inventory_verified'] for x in items) + int(reject_ok),
