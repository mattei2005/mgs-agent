...
INVENTORY_CSV = BASE / "inventory-poll-3/upload-manual-inventory-20260903T154737Z.csv"
TIMELINE_MANIFEST = BASE / "visual-review/20260903T154851Z/video-frame-sample-manifest.json"
...
# Visual review of all 14 multi-frame timelines. Every item is a valid
# English credit-card Story with a visible presenter and AVAILABLE LIMIT claim.
...
def prepare() -> dict[str, Any]:
    ...
    if len(csv_rows) != 14 or len(manifest.get("items", [])) != 14:
        raise RuntimeError("expected exactly 14 unique valid media items")
    ...
    if live_ids != expected_ids | SPECIAL_SOURCE_IDS:
...
def execute() -> dict[str, Any]:
    ...
    with lock_path.open("w") as lockfh:
        ...
        live_ids = {x["id"] for x in live}
        if live_ids != expected | SPECIAL_SOURCE_IDS:
        ...
        if pending_ids != SPECIAL_SOURCE_IDS:
            raise RuntimeError(f"UPLOAD MANUAL special-source drift: pending={len(pending_ids)}")
