...
OPERATION = 'CC_US_ES'
THREAD_ID = '1549108884638142507'
INVENTORY = Path('/root/mgs-agent/data/ares/creative-ops/inventory/assets.jsonl')
REPORT_DIR = Path('/root/mgs-agent/data/ares/creative-ops/executions/20260914T172636Z-kelly-cc-us-es-thread-1549108884638142507')
...
# Current batch classification from 34 real multi-frame timelines.
CLASSIFICATION = json.loads((REPORT_DIR / 'classification.json').read_text(encoding='utf-8'))['items']
...
def list_children(drive, parent_id: str) -> list[dict[str, Any]]:
    ...
    fields = 'nextPageToken,files(id,name,mimeType,parents,driveId,size,md5Checksum,createdTime,modifiedTime,trashed,webViewLink)'
    out: list[dict[str, Any]] = []
    page_token = ''
    while True:
        params = {'q': q, 'supportsAllDrives': 'true', 'includeItemsFromAllDrives': 'true', 'pageSize': '1000', 'fields': fields, 'orderBy': 'name_natural'}
        if page_token:
            params['pageToken'] = page_token
        url = 'https://www.googleapis.com/drive/v3/files?' + urllib.parse.urlencode(params)
        data = drive.request(url) or {}
...
def is_descendant_folder(drive, folder_id: str, ancestor_id: str) -> bool:
    current = folder_id
    seen: set[str] = set()
...
def recursive_media(drive, root_id: str) -> list[dict[str, Any]]:
    pending = [root_id]
    media: list[dict[str, Any]] = []
    while pending:
        parent = pending.pop()
        for child in list_children(drive, parent):
...
def canonical_group(cls: dict[str, str]) -> str:
    return f"CC_US_ES_VID_{cls['angle']}_{p_orient(cls['person'])}"
...
def main() -> int:
    ...
    input_rows = [
        r for r in csv.DictReader(open(args.inventory_csv, encoding='utf-8'))
        if r.get('format') == 'VID' and r.get('original_filename') in CLASSIFICATION
    ]
    if len(input_rows) != 10:
        raise RuntimeError(f'expected 10 scoped US-CC-ES videos, found {len(input_rows)}')
    ...
    lock_path = Path('/root/mgs-agent/tmp/ares-intake-locks') / f'cc_us_es-{batch_key}.lock'
    ...
    with lock_path.open('a+') as lock:
        ...
        for row in input_rows:
            ...
            parents = live.get('parents') or []
            if live.get('driveId') != ROOT_ID or len(parents) != 1:
                raise RuntimeError(f'source outside canonical upload/legacy parent: {row["original_filename"]}')
            source_parent = parents[0]
            if source_parent != legacy_id and not is_descendant_folder(drive, source_parent, upload_id):
                raise RuntimeError(f'source outside canonical upload subtree: {row["original_filename"]}')
            source_parent_by_id[row['drive_id']] = source_parent
            if source_parent != legacy_id and (not caps.get('canDownload') or not caps.get('canMoveItemWithinDrive')):
        ...
        for idx, row in enumerate(input_rows, 1):
            ...
            item = {
                ...
                'vehicle_type': cls.get('vehicle_type'), 'claim': cls['claim'], 'angle': cls['angle'], 'person': cls['person'],
        ...
        for item in plan:
            ...
            if item['disposition'] == 'UNIQUE_READY':
                ...
                current_parents = source_meta.get('parents') or []
                if current_parents != [legacy_id]:
                    if len(current_parents) != 1 or not is_descendant_folder(drive, current_parents[0], upload_id):
                        raise RuntimeError(f'source parent drift before LEGACY move: {item["source_filename"]}')
                    move_file(drive, key, current_parents[0], legacy_id)
                ...
                inventory_row = {
                    ...
                    'vertical': 'CC', 'country': 'US', 'language': 'ES',
                    ...
                    'source_path': 'MGS-AGENTS/CRIATIVOS/CC_US_ES/VID/99_LEGACY',
                    'asset_path': 'MGS-AGENTS/CRIATIVOS/CC_US_ES/VID/01_READY',
        ...
        remaining_media = recursive_media(drive, upload_id)
        if remaining_media:
            raise RuntimeError(f'UPLOAD MANUAL still contains {len(remaining_media)} media item(s) recursively')
        ...
        for r in results:
            final_source = api_get(drive, r['source_drive_id'])
            final_destination = api_get(drive, r['destination_drive_id'])
            if final_source.get('parents') != [legacy_id] or final_destination.get('parents') != [ready_id]:
        ...
        manifest = {
            ...
            'metadata_clean_verified': len(unique_dest_ids), 'raw_legacy_verified': len(results),
            'scoped_upload_manual_remaining_files': 0,
        ...
        print(json.dumps({'done': True, 'source_lineages': len(results), 'unique_ready_assets': len(unique_results), 'duplicate_sources': len(duplicate_results), 'scoped_upload_manual_remaining_files': 0, 'report_csv': str(csv_path), 'manifest': str(manifest_path)}, ensure_ascii=False, indent=2))
        return 0
