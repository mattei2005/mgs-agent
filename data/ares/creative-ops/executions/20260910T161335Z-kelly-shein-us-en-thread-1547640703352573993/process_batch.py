...
THREAD_ID = '1547640703352573993'
INVENTORY = Path('/root/mgs-agent/data/ares/creative-ops/inventory/assets.jsonl')
REPORT_DIR = Path('/root/mgs-agent/data/ares/creative-ops/executions/20260910T161335Z-kelly-shein-us-en-thread-1547640703352573993')
...
CLASSIFICATION = json.loads((REPORT_DIR / 'classification.json').read_text(encoding='utf-8'))
...
def canonical_group(cls: dict[str, str]) -> str:
    return f"SHEIN_US_EN_VID_{cls['angle']}_{p_orient(cls['person'])}"
...
def main() -> int:
    ...
    if len(input_rows) != 26:
        raise RuntimeError(f'expected 26 videos, found {len(input_rows)}')
    ...
    lock_path = Path('/root/mgs-agent/tmp/ares-intake-locks') / f'shein_us_en-{batch_key}.lock'
    ...
    with lock_path.open('a+') as lock:
        ...
        ready_id = drive.ensure_path(f'MGS-AGENTS/CRIATIVOS/{OPERATION}/VID/01_READY')
        legacy_id = drive.ensure_path(f'MGS-AGENTS/CRIATIVOS/{OPERATION}/VID/99_LEGACY')
        ...
        for row in input_rows:
            ...
            required_caps = ('canDownload', 'canEdit', 'canMoveItemWithinDrive', 'canModifyContent', 'canTrash', 'canDelete')
            missing_caps = [name for name in required_caps if not caps.get(name)]
            if live.get('parents') == [upload_id] and missing_caps:
                raise RuntimeError(f'missing required source capability {missing_caps}: {row["original_filename"]}')
        ...
        for idx, row in enumerate(input_rows, 1):
            ...
            item = {
                ...
                'product_type': cls['product_type'], 'claim': cls['claim'], 'angle': cls['angle'], 'person': cls['person'],
        ...
        inventory_backup = backup_dir / f'assets-before-kelly-shein-us-en-{backup_stamp}.jsonl'
        if INVENTORY.exists():
        ...
        for item in plan:
            ...
            if item['disposition'] == 'UNIQUE_READY':
                ...
                inventory_row = {
                    ...
                    'vertical': 'SHEIN', 'product_type': item['product_type'], 'country': 'US', 'language': 'EN',
                    'strategy': 'TRAFEGO_DIRETO', 'ad_account_id': None, 'source_drive_id': key, 'asset_drive_id': dest_id,
                    ...
                    'notes': f"Upload humano tratado por Ares. Produto visual: {item['product_type']}. Claim visual dominante: {item['claim']}. Original preservado em 99_LEGACY. Fail-closed até liberação/conciliação Meta × Drive.",
            ...
            results.append({
                ...
                'sha256_readback_verified': True, 'product_type': item['product_type'], 'person': item['person'],
        ...
        manifest = {
            ...
            'items': [{
                ...
                'disposition': r['disposition'], 'product_type': r['product_type'], 'claim': r['claim'],
