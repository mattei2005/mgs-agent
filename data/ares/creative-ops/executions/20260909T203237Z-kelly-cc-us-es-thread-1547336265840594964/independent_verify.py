...
BASE = Path('/root/mgs-agent/data/ares/creative-ops/executions/20260909T203237Z-kelly-cc-us-es-thread-1547336265840594964')
...
ready_id = mod.resolve_existing_path(drive, ['CRIATIVOS', 'CC_US_ES', 'VID', '01_READY'])
legacy_id = mod.resolve_existing_path(drive, ['CRIATIVOS', 'CC_US_ES', 'VID', '99_LEGACY'])
...
for idx, row in enumerate(rows, 1):
    ...
    if row['disposition'] == 'UNIQUE_READY':
        ...
        inv_ok = bool(inv and inv.get('asset_drive_id') == did and inv.get('canonical_filename') == row['destination_filename'] and inv.get('metadata_clean') is True and inv.get('reservation_status') == 'RESERVADO_PELO_GESTOR' and inv.get('ares_eligible') is False and inv.get('thread_id') == '1547336265840594964')
    else:
