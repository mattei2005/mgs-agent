...
ROOT=/root/mgs-agent
REPO=/root/.hermes/hermes-agent-stage-v2026-9-14-345cd2b0-mgs
NEW_LAUNCHER=/root/.local/bin/hermes-v0213-stable-345cd2b0-mgs
OLD_LAUNCHER=/root/.local/bin/hermes-main-67764dc0-mgs
CANONICAL=/root/.local/bin/hermes
EXPECTED_HEAD=b296c5e32eb21fe541066022a7c5d8cb21026dcc
EXPECTED_UPSTREAM=345cd2b057a452236de401d3534b8502a7465e8d
EXPECTED_TAG=v2026.9.14
PATCH=$ROOT/patches/hermes/mgs-runtime-customizations-2026-09-14-v0213-345cd2b0.patch
...
SNAPSHOT=/root/.hermes/secure-backups/vps-maintenance/20260914T165558Z-vps-hermes-v0213/activation-targets.sha256
PIP_FREEZE_SHA=/root/.hermes/secure-backups/vps-maintenance/20260914T165558Z-vps-hermes-v0213/candidate-pip-freeze.sha256
EVIDENCE=/root/.hermes/secure-backups/vps-maintenance/20260914T165558Z-vps-hermes-v0213
RESULT=$ROOT/data/hermes-v0213-activation-result.json
CHECKPOINT_ID=vps-hermes-controlled-update-20260914
THREAD_ID=1549096295338221660
AUTHORIZATION_MESSAGE_ID=1549100917511684167
LOCK=/run/lock/mgs-hermes-v0213-activation.lock
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
LOG=$ROOT/logs/hermes-v0213-activation-${STAMP}.log
SWITCHED=0
ROLLBACK_DONE=0
RUNTIME_ACCEPTED=0
...
row={"ts":datetime.datetime.now(datetime.timezone.utc).isoformat(),"event":event,"actor":"hermes-v0213-activation-finalizer","detail":detail}
...
data={
    ...
    'version':'0.21.3','release_tag':'v2026.9.14',
    'upstream_sha':'345cd2b057a452236de401d3534b8502a7465e8d',
    'port_commit':'b296c5e32eb21fe541066022a7c5d8cb21026dcc',
...
on_error(){
  ...
  audit hermes_v0213_activation_failed "rc=$rc line=$line switched=$SWITCHED runtime_accepted=$RUNTIME_ACCEPTED log=$LOG"
  if [[ "$SWITCHED" == 1 && "$RUNTIME_ACCEPTED" == 0 ]]; then
    log 'Starting automatic rollback to Hermes v0.21.1 main port'
    if atomic_launcher "$OLD_LAUNCHER" && prepare_and_run_restart 'rollback-hermes-v0213-activation' '/root/.hermes/hermes-agent-stage-main-67764dc0-mgs'; then
      ROLLBACK_DONE=1
  ...
  audit hermes_v0213_activation_result "status=$status rollback_done=$ROLLBACK_DONE result=$RESULT log=$LOG"
...
log 'START Hermes v0.21.3 controlled activation'
audit hermes_v0213_activation_started "authorization_message_id=$AUTHORIZATION_MESSAGE_ID target=$EXPECTED_UPSTREAM port=$EXPECTED_HEAD order=ares,atena,zeus log=$LOG"
...
req=urllib.request.Request('https://api.github.com/repos/NousResearch/hermes-agent/releases/latest',headers={'Accept':'application/vnd.github+json','User-Agent':'mgs-hermes-v0213-finalizer'})
with urllib.request.urlopen(req,timeout=30) as r: data=json.load(r)
assert data.get('tag_name')==sys.argv[1],data.get('tag_name')
print('latest_release_gate=PASS tag='+sys.argv[1])
PY
...
prepare_and_run_restart 'hermes-v0213-stable-activation' "$REPO"
...
for profile in ares atena zeus; do
  marker="MGS_V0213_${profile^^}_POST_OK"
...
data={
 ...
 'scope':'residues created by VPS + Hermes v0.21.3 update only',
 ...
 'retained':[
   ...
   '/root/.hermes/hermes-agent-stage-main-67764dc0-mgs',
   '/root/.hermes/secure-backups/vps-maintenance/20260914T165558Z-vps-hermes-v0213']}
fd,tmp=tempfile.mkstemp(dir=path.parent,prefix=path.name+'.',suffix='.tmp')
try:
 with os.fdopen(fd,'w',encoding='utf-8') as f:
  json.dump(data,f,ensure_ascii=False,indent=2);f.write('\n');f.flush();os.fsync(f.fileno())
...
write_result success 'Hermes v0.21.3 active; stable target 345cd2b0; MGS port b296c5e3; services/readiness 3/3; guard 544+6; regression 312/4 skipped; changed-surface 649; smokes/auth 3/3; VPS current; cleanup no deletion needed'
python3 "$ROOT/scripts/mgs-knowledge-control.py" checkpoint-upsert \
  --id "$CHECKPOINT_ID" --agent zeus --thread-id "$THREAD_ID" \
  --objective 'Atualizar VPS e Hermes pelo plano controlado MGS, preservando patches, rollback e zero commits pendentes dentro do target estável selecionado' \
  --state 'completed_validated: VPS APT/full-upgrade/Snap/npm zero, kernel 6.8.0-139, no reboot; Hermes v0.21.3 active at port b296c5e3 over stable target 345cd2b0; Ares→Atena→Zeus new PIDs and Discord ready; guard 544+6, regression 312 pass/4 skip, changed-surface 649, smokes/auth 3/3; backup control-plane and patch-control archives SHA verified; cleanup audit no deletion needed; old invalid 9.87GB partial preserved outside scope' \
  --next-step 'Monitoramento rotineiro; 6 commits pós-release em main ficaram fora do escopo estável e são atualização futura' \
  --source 'discord:thread:1549096295338221660;authorization:1549100917511684167;result:/root/mgs-agent/data/hermes-v0213-activation-result.json;evidence:/root/.hermes/secure-backups/vps-maintenance/20260914T165558Z-vps-hermes-v0213;patch:/root/mgs-agent/patches/hermes/mgs-runtime-customizations-2026-09-14-v0213-345cd2b0.patch'
...
PY
write_result success 'Hermes v0.21.3 active; stable target 345cd2b0; MGS port b296c5e3; services/readiness 3/3; guard 544+6; regression 312/4 skipped; changed-surface 649; smokes/auth 3/3; VPS current; cleanup no deletion needed' "$report_id"
