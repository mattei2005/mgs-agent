...
def build_plan(paths: dict[str, Path], *, rules_path: Path = RULES_PATH) -> dict[str, Any]:
    ...
    return {
        ...
        "mapping_authority_message_id": rules["authority"].get("openzed_br_ducapes_split_escalatepower_wavesbee", rules["authority"].get("mavroa_us_shein_es_direct_fallback", rules["authority"].get("gamezone_exception_and_autolend_confirmation", rules["authority"]["manager_fallback_and_sequence"]))),
        "source_hashes": source_hashes,
        ...
        "mapped_totals": {key: format(grouped_totals[key], "f") for key in sorted(source_totals)},
        "blocked_totals": {key: format(blocked_totals[key], "f") for key in sorted(source_totals)},
