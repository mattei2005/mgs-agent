...
def render_report(report):
 ...
 if attention:lines=['Os gastos confirmados foram preenchidos nos Relatórios Diários. Abaixo estão somente os pontos que não puderam ser confirmados; nenhum deles foi tratado como zero.','Período: '+start+' a '+end+'.']
 else:
 ...
 return {'title':(('Gastos — confirmar somente a exceção · ' if attention else 'Gastos por site · ') if actionable else 'Gastos diários confirmados · ')+end,'body':body,'attention':attention,'actionable':actionable,'new_accounts':bool(new or bound),'signature':signature}
