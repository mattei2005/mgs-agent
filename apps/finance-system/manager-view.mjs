...
export function managerView(s,source,period){
 ...
 for(const block of inferred.length?inferred:layout.blocks){const entries=[];for(let day=1;day<=p.days;day++){const r=block.row+1+day;entries.push({date:period+'-'+String(day).padStart(2,'0'),values:block.columns.map(c=>value(c.col+r))});}blocks.push({label:block.label,columns:block.columns.map(c=>c.label),monthly_values:block.columns.map(c=>value(c.col+(block.row+33))),rows:entries});}
 ...
 return {manager:key,display_name:'Nicolas',pilot:true,read_only:true,source:'dash',period:p,fx:s.result.results['principal|Agosto 2026|F1']?.actual??'',revision:s.revision,summaries,remuneration,blocks};
}
