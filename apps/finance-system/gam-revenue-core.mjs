...
export function validatePlan(plan){
 ...
 assert.equal(plan.partial,plan.blockers.length>0,'partial flag and blockers differ');
 assert.ok(plan.entries.length>0,'empty plan');
 const ids=new Set();
 const entryTotals={USD:0,CAD:0};
 for(const entry of plan.entries){
  ...
  entryTotals[entry.currency]+=number(entry.gross);
 }
 for(const currency of ['USD','CAD']){
  for(const totals of [plan.source_totals,plan.mapped_totals,plan.blocked_totals])assert.match(String(totals[currency]),/^\d+(?:\.\d+)?$/);
  assert.ok(near(entryTotals[currency],plan.mapped_totals[currency],2e-7),currency+' mapped total mismatch');
  assert.ok(near(number(plan.mapped_totals[currency])+number(plan.blocked_totals[currency]),plan.source_totals[currency],2e-7),currency+' source partition mismatch');
  if(!plan.partial)assert.ok(near(plan.blocked_totals[currency],0,2e-7),currency+' unexpected blocked total');
 }
 return plan;
}
...
export function prepareChange(row,plan,{spendUntil=null}={}){
 ...
 if(sameDate.length){
  ...
  if(cutoffRows[0].date===plan.date){
  ...
  const additions=[...row.additions.filter(item=>item.kind!=='data_cutoff'&&!(item.source_import_type==='gam_email_daily'&&item.source_date===plan.date)),...entries,cutoff];
 ...
 return {alreadyApplied:false,partial:plan.partial,completedPartial:false,entries,additions,cutoff};
}
...
export function validateCalculated(before,plan,prepared,result){
 ...
 return {partial:plan.partial,source_totals:plan.source_totals,mapped_totals:plan.mapped_totals,blocked_totals:plan.blocked_totals,cutoff:result.domain.realized.cutoff_date,gross_usd:result.domain.cash.gross,spend_usd:result.domain.cash.spend,company_expenses_usd:result.domain.cash.company_expenses,personnel_before_usd:before.result.domain.cash.personnel,personnel_after_usd:result.domain.cash.personnel};
}
