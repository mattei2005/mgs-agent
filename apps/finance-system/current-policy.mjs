...
export function desiredAdditions(additions,period){
 const {cutoff,sites}=policyRows(period);
...
export async function migrateCurrentPolicy(db,{authority=AUTH}={}){
 ...
 for(const s of rows){
  ...
  assert.equal(Number(result.domain.allocation?.active_units),Number(s.result.domain.allocation?.active_units),period+' active units');const expected=policyRows(period).cutoff.date,realized=result.domain.realized;assert.equal(realized.cutoff_date,expected);assert.equal(realized.elapsed_days,expected?Number(expected.slice(-2)):0);
  if(period==='2026-08')assert.ok(near(realized.profit,result.domain.cash.profit));
  if(period==='2026-09'){const fixed=Number(result.domain.cash.company_expenses)+Number(result.domain.cash.personnel),variable=result.domain.facts.filter(f=>f.date<=expected).reduce((n,f)=>n+Number(f.profit||0),0),estimate=(variable*30/9+fixed)/2;assert.ok(near(result.domain.projection.half_usd,estimate,1e-7));}
