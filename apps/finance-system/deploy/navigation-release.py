...
load_env();AUTH='1546682010066489394';STATE=ROOT/('private/navigation-'+AUTH);TARGET='/home/mgsfinance/releases/pg-auth-1545934831664242748';BACKUP='/home/zeus/mgs-finance-backups/'+AUTH;STAGE='/var/tmp/mgs-finance-navigation-'+AUTH;DB='mgs_finance_navigation_'+AUTH;NODE='/home/mgsfinance/runtime/node-v22.23.2-linux-x64/bin/node';phase=sys.argv[1]
OLD=['finance-ops.mjs','finance-ops-schema.sql','public/app.js','public/index.html','public/operations.js','public/operations.html'];FILES=OLD+['public/navigation.js','public/navigation.css'];local={f:hashlib.sha256((ROOT/f).read_bytes()).hexdigest() for f in FILES}
...
schema='\n'.join(line for line in (ROOT/'finance-ops-schema.sql').read_text().splitlines() if line.startswith('ALTER TABLE finance_users ADD COLUMN'))+'\n'
if phase=='prepare':
 ...
 with tarfile.open(fileobj=payload,mode='w:gz') as t:
  ...
  for f in ['manager-layout.json','finance-ops-schema.sql','deploy/meta-lookup-queue.py','deploy/finance-notices.mjs','tests/navigation-pg.mjs']:t.add(ROOT/f,arcname=f)
 ...
 print(ssh('sudo -n -u mgs_pg env FINANCE_NAVIGATION_DATABASE='+DB+' '+STAGE+'/node '+STAGE+'/tests/navigation-pg.mjs',timeout=480));d=json.loads(ssh('sudo -n -u mgs_pg python3 -c '+shlex.quote('from pathlib import Path;print(Path('+repr(STAGE+'/private/payments-test-readback.json')+').read_text())')));assert d['pass'];save('pg-exercise.json',d)
elif phase=='publish':
