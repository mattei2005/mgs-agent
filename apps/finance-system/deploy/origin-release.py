"""Bounded code-only release, fresh DB restore, no production financial migration."""
...
load_env();AUTH='1546618148571058266';STATE=ROOT/('private/origin-'+AUTH);TARGET='/home/mgsfinance/releases/pg-auth-1545934831664242748';BACKUP='/home/zeus/mgs-finance-backups/'+AUTH;STAGE='/var/tmp/mgs-finance-origin-'+AUTH;DB='mgs_finance_origin_'+AUTH;NODE='/home/mgsfinance/runtime/node-v22.23.2-linux-x64/bin/node';phase=sys.argv[1]
OLD=['worker.py','ui_model.py','site_catalog.py','workspace.mjs','accounts.mjs','auth.mjs','public/app.js','public/index.html','public/login.html','public/login.css','public/refinements.css'];FILES=OLD+['meta-lookup.mjs','deploy/meta-lookup-queue.py','public/mgs-logo.png','public/favicon.ico','public/favicon-32.png','public/apple-touch-icon.png'];local={f:hashlib.sha256((ROOT/f).read_bytes()).hexdigest() for f in FILES}
...
if phase=='prepare':
 ...
 with tarfile.open(fileobj=payload,mode='w:gz') as t:
  ...
  for f in ['deploy/meta-lookup-queue.py','tests/origin-pg.mjs']:t.add(ROOT/f,arcname=f)
 ...
 ssh('sudo -n python3 -c '+shlex.quote(code),timeout=180);assert hashes(STAGE,FILES)==local;save('prepared.json',{'pass':True,'backup':backup,'baseline':baseline,'expected':expected,'files':local,'stage':STAGE,'database':DB});print('Fresh backups, hash-matched second copy and isolated PostgreSQL restore PASS')
elif phase=='exercise':
 assert hashes(STAGE,FILES)==local
 print(ssh('sudo -n -u mgs_pg env FINANCE_ORIGIN_DATABASE='+DB+' '+STAGE+'/node '+STAGE+'/tests/origin-pg.mjs',timeout=480));d=json.loads(ssh('sudo -n -u mgs_pg python3 -c '+shlex.quote('from pathlib import Path;print(Path('+repr(STAGE+'/private/origin-test-readback.json')+').read_text())')));assert d['pass'];save('pg-exercise.json',d)
elif phase=='publish':
 ...
 try:
  ...
  ssh('sudo -n python3 -c '+shlex.quote(code));assert hashes(TARGET,FILES)==local;ssh('sudo -n systemctl start mgs-finance-dash.service',timeout=60)
 except Exception:
  ssh('sudo -n tar -xzf '+BACKUP+'/code-before.tar.gz -C '+TARGET+' && sudo -n systemctl start mgs-finance-dash.service',timeout=90);save('publication-failure.json',{'code_rollback':True,'backup':BACKUP});raise
 after=ssh(pg+'-d mgs_finance -c '+shlex.quote("SELECT json_agg(q) FROM (SELECT id,revision,md5(overrides::text) AS inputs,md5(additions::text) AS additions,md5(result::text) AS result FROM scenarios ORDER BY id) q")).strip();assert json.loads(before)==json.loads(after),'Concurrent data movement; investigate before closure';save('published.json',{'pass':True,'files':local,'production_financial_mutations':0,'database_preserved':True});print('Code-only production release; exact scenario/inputs/review readback unchanged PASS')
elif phase=='verify':
