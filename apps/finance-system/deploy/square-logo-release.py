...
load_env();AUTH='1546703031033405501';STATE=ROOT/'private/google-square-1546702931384991834';TARGET='/home/mgsfinance/releases/pg-auth-1545934831664242748';BACKUP='/home/zeus/mgs-finance-backups/'+AUTH;STAGE='/var/tmp/mgs-finance-square-'+AUTH;DB='mgs_finance_square_'+AUTH
FILES=['public/mgs-logo.png','public/login.css','public/navigation.css','public/refinements.css','public/login.html','public/navigation.js','public/app.js','public/operations.js','finance-ops.mjs'];local={f:hashlib.sha256((ROOT/f).read_bytes()).hexdigest() for f in FILES}
...
phase=sys.argv[1]
if phase=='prepare':
 assert not (STATE/'square-prepared.json').exists();expected=hashes(TARGET);before=ssh(pg+'-d mgs_finance -c '+shlex.quote(check)).strip()
 ...
 with tarfile.open(fileobj=payload,mode='w:gz') as t:
  ...
  for f in ['manager-layout.json','finance-ops-schema.sql','tests/self-profile-pg.mjs']:t.add(ROOT/f,arcname=f)
 ...
 save('square-prepared.json',{'pass':True,'expected':expected,'files':local,'backup':backup,'stage':STAGE,'database':DB,'before':before});print('Square logo: fresh dual backup/hash + isolated PostgreSQL restore + staged files PASS')
elif phase=='exercise':
 assert hashes(STAGE)==local
 print(ssh('sudo -n -u mgs_pg env FINANCE_NAVIGATION_DATABASE='+DB+' '+STAGE+'/node '+STAGE+'/tests/self-profile-pg.mjs',timeout=480))
 out=json.loads(ssh('sudo -n -u mgs_pg python3 -c '+shlex.quote('from pathlib import Path;print(Path('+repr(STAGE+'/private/profile-test-readback.json')+').read_text())')));assert out['pass'];save('profile-pg-exercise.json',out)
elif phase=='publish':
 p=json.loads((STATE/'square-prepared.json').read_text());assert hashes(TARGET)==p['expected'],'Concurrent code change: reconcile';assert hashes(STAGE)==local;assert json.loads((STATE/'square-local-tests.json').read_text())['pass']
 assert json.loads((STATE/'profile-pg-exercise.json').read_text())['pass']
 ...
 save('square-published.json',{'pass':True,'files':local,'financial_data_unchanged':True,'finance_app_restart':True,'gateway_restart':False});print('Square logo/profile code publication/readback PASS; no credential or financial write')
elif phase=='verify':
 ...
 save('square-deploy-readback.json',{'pass':True,'files':local,'services':services});print('Square logo hashes and services PASS')
