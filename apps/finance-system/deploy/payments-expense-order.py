"""Display-only whitespace correction for alphabetical expenses."""
...
ssh('test ! -e '+backup+'/expense-order-before.tar.gz && sudo -n tar -czf '+backup+'/expense-order-before.tar.gz -C '+target+' '+' '.join(files)+' && sudo -n chown zeus:zeus '+backup+'/expense-order-before.tar.gz && chmod 600 '+backup+'/expense-order-before.tar.gz')
import base64
raw=base64.b64decode(ssh('base64 -w0 '+backup+'/expense-order-before.tar.gz'),validate=True);copy=state/'expense-order-before.tar.gz';copy.write_bytes(raw);copy.chmod(0o600);assert hashlib.sha256(raw).hexdigest()==ssh('sha256sum '+backup+'/expense-order-before.tar.gz').split()[0]
payload=io.BytesIO()
...
ssh('sudo -n python3 -c '+shlex.quote(code));assert hashes(target)==current;manifest['files'].update(current);manifest['expense_order_polish']={'files':current,'before':expected,'backup':backup+'/expense-order-before.tar.gz'};(state/'published.json').write_text(json.dumps(manifest,indent=2));print(json.dumps({'pass':True,'files':len(files),'financial_mutations':0}))
