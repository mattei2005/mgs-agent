"""Square logo + contact profile correction. No credential, grant or schema changes."""
...
load_env();AUTH='1546702931384991834';STATE=ROOT/'private/google-accounts-1546702931384991834';TARGET='/home/mgsfinance/releases/pg-auth-1545934831664242748';BACKUP='/home/zeus/mgs-finance-backups/'+AUTH;STAGE='/var/tmp/mgs-finance-google-'+AUTH;DB='mgs_finance_google_'+AUTH
OLD=['accounts.mjs','finance-ops.mjs','public/app.js','deploy/meta-lookup-queue.py'];FILES=OLD+['google-lookup.mjs'];local={f:hashlib.sha256((ROOT/f).read_bytes()).hexdigest() for f in FILES}
...
phase=sys.argv[1]
if phase=='prepare':
 assert not (STATE/'prepared.json').exists();expected=hashes(TARGET,OLD);before=ssh(pg+'-d mgs_finance -c '+shlex.quote(check)).strip()
 ...
 with tarfile.open(fileobj=payload,mode='w:gz') as t:
  ...
  for f in ['manager-layout.json','finance-ops-schema.sql','deploy/meta-lookup-queue.py','tests/google-accounts-pg.mjs']:t.add(ROOT/f,arcname=f)
 ...
 proof=(ROOT/'private/origin-1546618148571058266/meta-probe.json').read_bytes();ssh('sudo -n -u mgs_pg tee '+STAGE+'/private/meta-probe.json',proof)
 save('prepared.json',{'pass':True,'expected':expected,'files':local,'backup':backup,'stage':STAGE,'database':DB,'before':before});print('Google account catalog: fresh dual backup/hash + isolated PostgreSQL restore + staged files PASS')
elif phase=='exercise':
 assert hashes(STAGE)==local
 print(ssh('sudo -n -u mgs_pg env FINANCE_GOOGLE_DATABASE='+DB+' '+STAGE+'/node '+STAGE+'/tests/google-accounts-pg.mjs',timeout=480))
 out=json.loads(ssh('sudo -n -u mgs_pg python3 -c '+shlex.quote('from pathlib import Path;print(Path('+repr(STAGE+'/private/google-test-readback.json')+').read_text())')));assert out['pass'];save('pg-exercise.json',out)
elif phase=='publish':
 p=json.loads((STATE/'prepared.json').read_text());assert hashes(TARGET,OLD)==p['expected'],'Concurrent code change: reconcile';assert hashes(STAGE)==local;assert json.loads((STATE/'local-tests.json').read_text())['pass']
 ...
 try:
  ...
  code='import pathlib,shutil,os,pwd;s=pathlib.Path('+repr(STAGE)+');t=pathlib.Path('+repr(TARGET)+');u=pwd.getpwnam("mgsfinance");'+ '\nfor f in '+repr(FILES)+':\n p=t/f;q=p.with_name(p.name+".square-pending");shutil.copy2(s/f,q);os.chown(q,u.pw_uid,u.pw_gid);os.chmod(q,0o600);os.replace(q,p)'
 ...
 save('published.json',{'pass':True,'files':local,'financial_data_unchanged':True,'finance_app_restart':True,'gateway_restart':False});print('Google account catalog code publication/readback PASS; no credential or financial write')
elif phase=='verify':
 ...
 save('deploy-readback.json',{'pass':True,'files':local,'services':services});print('Google account catalog hashes and services PASS')
