...
def pending():
 ...
 for f in sorted(DIR.glob('*.json')):
  ...
  if d.get('status')=='pending':
   ...
   rows.append({k:d[k] for k in ['request_id','account_id','requested_at']})
 return rows[:16]
if sys.argv[1]=='wait':
 ...
 for v in values:
  ...
  allowed={k:v[k] for k in ['status','name','currency','timezone','business_id','verified_at','error'] if k in v};assert allowed['status'] in ['ready','error']
  if allowed['status']=='ready':assert allowed.get('business_id')=='155263197283282' and allowed.get('name') and allowed.get('currency') and allowed.get('timezone')
