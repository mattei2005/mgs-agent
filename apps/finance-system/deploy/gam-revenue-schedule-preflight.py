...
checks=[next(x for x in candidates if x['hour']==8 and x['minute']==minute) for minute in selected]+[next(x for x in candidates if x['hour']==9 and x['minute']==minute) for minute in finalize];report={'pass':not unknown and all(not x['operational_conflicts'] for x in checks),'schedule':','.join(map(str,selected))+' 8 * * *','poll_minutes':selected,'finalize_schedule':','.join(map(str,finalize))+' 9 * * *','finalize_minutes':finalize,'timezone':str(TZ),'civil_dates':8,'operational_conflicts':[x for check in checks for x in check['operational_conflicts']],'baseline_collisions':[{'hour':check['hour'],'minute':check['minute'],**row} for check in checks for row in check['baseline_collisions']],'unknown':unknown,'resource_guards':['own GAM flock lock','validated intake runs media spend synchronously before revenue','media-spend state plus remote spend-ledger gate','media-spend and quote-sync locks serialize writers','09:03 spend and 09:22/09:31/09:41 revenue are recovery fallbacks','read-only IMAP BODY.PEEK','one 1Password credential resolution per run','transaction revision guard and deterministic source identity'],'candidate_summary':[{'hour':x['hour'],'minute':x['minute'],'operational_conflicts':len(x['operational_conflicts']),'baseline_count':x['baseline_count']} for x in candidates]};path=D/('schedule-post.json' if args.minutes and args.finalize_minutes else 'schedule-preflight.json');path.write_text(json.dumps(report,indent=2));print(json.dumps({k:v for k,v in report.items() if k!='candidate_summary'}));assert report['pass']
