...
- MFA individual é a etapa final de hardening. O suporte autoassistido está implantado em modo desativado para as sete identidades atuais: Rodolfo, Geizian, Ícaro, Joe, Isliago, Kelly e Nicolas. Cada usuário terá QR/segredo próprio, TOTP compatível com Google/Microsoft Authenticator, Authy ou 1Password, dez recovery codes de uso único e bloqueio de replay. Segredos ficam AES-256-GCM no banco; somente hashes dos recovery codes são persistidos. `mfa_required=false`, `mfa_key` ausente e `auth_mfa` vazio mantêm os logins atuais intactos até a confirmação crítica de cutover.
- Rodolfo terá TOTP e recovery material no próprio item do 1Password; os demais configuram QR individual no Google Authenticator ou compatível, sem depender do 1Password. A opção `Confiar neste dispositivo por 30 dias` usa cookie aleatório HttpOnly/Secure/SameSite=Strict com hash no servidor, vinculado a usuário e user-agent; dispensa somente TOTP depois da senha correta, não prolonga a sessão de 8h/idle 30min, limita cinco dispositivos e é revogada no logout. `auth_trusted_devices` existe com zero linhas e grants mínimos enquanto o enforcement está desligado.
