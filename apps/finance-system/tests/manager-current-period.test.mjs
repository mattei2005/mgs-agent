...
function fixture(){
 ...
 const expenses=[{id:'personnel|148',label:'Joe - Gestor:',category:'personnel',manager:'joe',mode:'COMMISSION_FLOOR',brl:'-3000.00',usd:'-600',floor_brl:'3000',activity:'ATIVO',payroll_rule:'monthly-v1'}];
...
test('current manager blocks merge GAM revenue with legacy media and dynamic sites',()=>{const view=managerView(fixture(),[],'2026-09','joe'),infinity=view.blocks.find(x=>x.label==='Infinitynexx'),cpv=view.blocks.find(x=>x.label==='CreditoParaVeiculo'),yolo=view.blocks.find(x=>x.label==='Yolokfx'),portal=view.blocks.find(x=>x.label==='Portal Relevante');assert.ok(infinity&&cpv&&yolo&&portal);assert.equal(value(infinity,'2026-09-10','Infinitynexx · CAD · US'),'140');assert.equal(value(infinity,'2026-09-10','Infinitynexx · GROSS · US'),'100');assert.equal(value(infinity,'2026-09-10','Gastos · US'),'-50');assert.equal(value(infinity,'2026-09-10','LUCRO LIQUIDO · US'),'34.55');assert.notEqual(value(infinity,'2026-09-10','ROI · NET · US'),'');assert.ok(cpv.columns.some(x=>x.endsWith(' · BR')));assert.ok(!cpv.columns.some(x=>x.endsWith(' · US')));assert.equal(value(cpv,'2026-09-10','Gastos · BR'),'-25');assert.equal(value(yolo,'2026-09-10','Yolokfx · GROSS · US'),'50');assert.equal(value(yolo,'2026-09-10','Gastos · US'),'-10');assert.equal(value(portal,'2026-09-10','Portal Relevante · GROSS · US'),'30');assert.equal(value(portal,'2026-09-10','ROI · GROSS · US'),'');assert.equal(value(portal,'2026-09-10','ROI · NET · US'),'');assert.equal(value(portal,'2026-09-11','Portal Relevante · GROSS · US'),'');assert.ok(view.blocks.every(block=>!block.spend_only));});
...
test('manager cards project actual result by completed days and select the current 7% or 10% band',()=>{
 ...
 const payroll={brl:'-3000',usd:'-600',floor_brl:'3000',activity:'ATIVO',payroll_rule:'monthly-v1'},floor=managerCardSummary({profit:'101.01'},'5',p,{cutoff_date:'2026-09-10',elapsed_days:10},payroll);assert.equal(floor.remuneration.status,'floor');assert.equal(floor.remuneration.due.brl,'3000');assert.equal(floor.remuneration.floor_switch_result_brl,'42857.22');assert.equal(floor.remuneration.remaining_to_floor_switch_brl,'42352.17');
 const variable=managerCardSummary({profit:'12000'},'5',p,{cutoff_date:'2026-09-10',elapsed_days:10},{...payroll,brl:'-4200',usd:'-840'});assert.equal(variable.remuneration.status,'commission7');assert.equal(variable.remuneration.remaining_to_high_threshold_brl,'40000');
 const ten=managerCardSummary({profit:'20000'},'5',p,{cutoff_date:'2026-09-10',elapsed_days:10},{...payroll,brl:'-10000',usd:'-2000'});assert.equal(ten.remuneration.status,'commission10');assert.equal(ten.remuneration.due.brl,'10000');
 const view=managerView(fixture(),[],'2026-09','joe');assert.deepEqual(view.card_summary,floor);assert.notEqual(view.card_summary.estimated.result.usd,view.summaries.find(x=>x.row===14)?.profit);
});
