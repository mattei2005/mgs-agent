...
try{
 ...
 const context=await browser.newContext({viewport:{width:1440,height:900},locale:'pt-BR',timezoneId:'America/New_York'});page=await context.newPage();const errors=[];
 ...
 await page.waitForFunction(()=>location.pathname==='/'||!document.querySelector('#mfaPanel')?.hidden);
...
}catch(error){console.log(JSON.stringify({pass:false,stage,error_type:error.name,message:String(error.message).slice(0,300),url:page?.url(),ui_error:page?await page.locator('#error').textContent().catch(()=>null):null}));process.exitCode=1;}finally{await browser?.close();}
