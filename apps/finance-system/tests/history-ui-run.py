...
with (D/(phase+'-browser.log')).open('w') as log:
 p=subprocess.run(['node','tests/history-ui-public.mjs',phase]+(['resume'] if len(sys.argv)>2 and sys.argv[2]=='resume' else []),cwd=R,input=json.dumps(credentials),text=True,stdout=log,stderr=subprocess.STDOUT,timeout=570)
