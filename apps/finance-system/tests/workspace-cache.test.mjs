...
test('workspace response cache is revision-aware and preserves exact payload',{timeout:180000},async()=>{
 ...
 try{
  ...
  const second=await fetch(base+'/api/workspace?period=2026-08'),secondText=await second.text();assert.equal(second.status,200);assert.equal(second.headers.get('x-mgs-workspace-cache'),'hit');assert.match(second.headers.get('server-timing')||'',/^workspace;dur=\d+$/);assert.equal(secondText,firstText);
