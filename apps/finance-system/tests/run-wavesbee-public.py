"""Read-only authenticated public WavesBee currency browser acceptance, credentials in stdin only."""
...
r=subprocess.run(['node','tests/wavesbee-browser.mjs'],input=json.dumps({'username':'rodolfo','password':password}),text=True,capture_output=True,cwd=ROOT,env={**os.environ,'FINANCE_PUBLIC':'1'},timeout=300)
print(r.stdout)
