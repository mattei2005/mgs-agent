...
try{
 ...
 if(phase==='stage'){
  for(const file of ['app.js','navigation.js','operations.js'])await context.route('**/'+file+'*',route=>route.fulfill({path:root+'/public/'+file,contentType:'application/javascript'}));
 ...
 for(const period of ['2026-09','2027-12']){await page.goto(origin+'/?view=accounts&period='+period);await page.waitForSelector('[data-revenue-attribution]');const text=await page.locator('[data-revenue-attribution]').innerText();for(const value of ['Atribuição de receita por gestor','g001-d','g001-s','openzed.com','fincgriffin.com','creditoparaveiculo.com','yolokfx.com','autolendpro.com','GameZoneAd'])assert.ok(text.includes(value),value);assert.ok(text.includes('Também: Ícaro'));assert.ok(!text.includes('autolendpro.comd'));}
 ...
 await page.evaluate(()=>window.MGSNavigation.mount('partner','overview',false,{username:'geizian',display_name:'Geizian'}));const top=(await page.locator('#nav > a.nav-item').allTextContents()).map(x=>x.trim()),bottom=(await page.locator('.nav-bottom > a.nav-item').allTextContents()).map(x=>x.trim()),manager=(await page.locator('#managerToggle').textContent()).trim();assert.deepEqual(top,['Dashboard','Relatório Diário']);assert.deepEqual(bottom,['Despesas Gerais','Despesas Funcionários','Câmbio e Inválidos','Pagamentos']);assert.ok(manager.includes('Gestores'));for(const forbidden of ['Cadastro de Domínios','Contas de Anúncio','Histórico de Atividades','Aprovações','Usuários'])assert.ok(!(top.concat(bottom).join('|')).includes(forbidden));
