...
export async function installWorkspace(app,db,mutate){
 ...
 app.get('/api/workspace',async(req,res)=>{
  ...
  const ad=await accountDocument(db),cacheKey=[s.id,s.revision,ad.revision??0,quotes.updated_at??''].join(':');if(responseCache.has(cacheKey)){res.set('X-MGS-Workspace-Cache','hit');return res.type('application/json').send(responseCache.get(cacheKey));}
