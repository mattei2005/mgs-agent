...
def history_request(row,target):
 ...
 with (ROOT/'private/history-refresh.lock').open('a') as lock:
  fcntl.flock(lock,fcntl.LOCK_EX);existing=sorted(outdir.glob('*.json'));docs=[json.loads(p.read_text()) for p in existing] if len(existing) in (5,6) else history.capture(outdir,periods=[period],authority='1547732274936553532')['documents'];assert len(docs) in (5,6) and {d['period'] for d in docs}=={period} and all(d['source_authority']=='1547732274936553532' for d in docs);payload=json.dumps(docs,ensure_ascii=False).encode();result=json.loads(ssh('sudo -n -u mgsfinance /home/mgsfinance/runtime/node-v22.23.2-linux-x64/bin/node '+TARGET+'/history-source-cli.mjs mgs_finance 1547732274936553532',payload,timeout=300));assert result['pass'] and result['verified']==len(docs) and result['months']==1
