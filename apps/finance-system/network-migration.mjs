...
export function networkSeed(s){
 ...
 for(const site of siteCatalog(s.result.domain,s.additions)){
  ...
  const row={...prior,kind:'site',id:site.id,name:site.name,new:site.new,status:site.status,network,partner:network,invalid_source:networks[network].invalid_source,network_policy:networkRules.version,network_authorization:networkRules.authorization,network_additional_authorization:networkRules.additional_authorization};
...
export async function migrateNetworks(db,{onProgress=()=>{},ids=null}={}){
 ...
 for(const id of ids){
  ...
  await db.transaction(async tx=>{
   ...
   await tx.query('INSERT INTO audit_events(scenario_id,actor,action,before_data,after_data) VALUES($1,$2,$3,$4::jsonb,$5::jsonb)',[id,'Zeus / '+networkRules.authorization,'SITE_NETWORK_POLICY_APPLIED',JSON.stringify({additions:s.additions,overrides:s.overrides,cash:s.result.domain.cash}),JSON.stringify({authorization:networkRules.authorization,additional_authorization:networkRules.additional_authorization,networks:result.domain.site_catalog.map(x=>({id:x.id,name:x.name,network:x.network})),cash:result.domain.cash})]);
  });
