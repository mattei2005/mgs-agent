...
import {prepareChange,validateCalculated,validatePlan} from './gam-revenue-core.mjs';
...
try{
 if(phase==='rehearse'){
  const row=await current();const prepared=prepareChange(row,plan);
  ...
  const outcome=await db.transaction(async tx=>{
   ...
   const prepared=prepareChange(row,plan);
  ...
  const row=await current();const prepared=prepareChange(row,plan);assert.ok(prepared.alreadyApplied);const metrics=validateCalculated(row,plan,prepared,row.result);
  ...
  const row=await current();const prepared=prepareChange(row,plan);assert.ok(prepared.alreadyApplied,'daily import absent');const metrics=validateCalculated(row,plan,prepared,row.result);
