...
const recoveryId='recovery-gam-email-'+(partial?'partial-':'')+plan.date+'-'+plan.source_bundle_sha256.slice(0,12);
const reclassifyRecoveryId='recovery-gam-reclassify-'+plan.date+'-'+plan.mapping_rules_sha256.slice(0,12);
const recoveryAction=partial?'GAM_EMAIL_DAILY_PARTIAL_RECOVERY_CREATED':'GAM_EMAIL_DAILY_RECOVERY_CREATED';
const importAction=partial?'GAM_EMAIL_DAILY_PARTIAL_IMPORTED':'GAM_EMAIL_DAILY_IMPORTED';
...
try{
 if(phase==='rehearse'){
  ...
  const outcome=await db.transaction(async tx=>{
   ...
   await tx.query("INSERT INTO scenarios(id,import_id,name,state,revision,overrides,additions,result) VALUES($1,$2,$3,'locked',$4,$5::jsonb,$6::jsonb,$7::jsonb)",[recoveryId,row.import_id,(partial?'Recovery before partial daily GAM email ':'Recovery before daily GAM email ')+plan.date,row.revision,JSON.stringify(row.overrides),JSON.stringify(row.additions),JSON.stringify(row.result)]);
   await tx.query('INSERT INTO audit_events(scenario_id,actor,action,after_data) VALUES($1,$2,$3,$4::jsonb)',[plan.scenario_id,'zeus',recoveryAction,JSON.stringify({authorization:plan.authorization_message_id,processing_policy_authority:plan.processing_policy_authority_message_id,recovery_scenario_id:recoveryId,date:plan.date,source_bundle_sha256:plan.source_bundle_sha256,source_hashes:plan.source_hashes,source_revision:row.revision,partial})]);
   ...
   const audit=await tx.query('INSERT INTO audit_events(scenario_id,actor,action,before_data,after_data) VALUES($1,$2,$3,$4::jsonb,$5::jsonb) RETURNING id',[plan.scenario_id,'zeus',importAction,JSON.stringify({revision:row.revision,cutoff:row.result.domain.realized?.cutoff_date}),JSON.stringify({authorization:plan.authorization_message_id,processing_policy_authority:plan.processing_policy_authority_message_id,date:plan.date,source_bundle_sha256:plan.source_bundle_sha256,source_hashes:plan.source_hashes,source_rows:plan.source_rows,groups:prepared.entries.length,source_totals:plan.source_totals,mapped_totals:plan.mapped_totals,blocked_totals:plan.blocked_totals,partial,cutoff:metrics.cutoff})]);
  ...
  const audit=(await db.query("SELECT id,after_data,created_at FROM audit_events WHERE scenario_id=$1 AND action=$2 AND after_data->>'date'=$3 ORDER BY id DESC LIMIT 1",[plan.scenario_id,importAction,plan.date])).rows[0];assert.ok(audit,'daily import audit absent');assert.equal(audit.after_data.source_bundle_sha256,plan.source_bundle_sha256);
