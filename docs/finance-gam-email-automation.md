...
- Mapeamentos permanentes confirmados por Rodolfo `1550483550027911290` para `2026-09-17` e relatórios futuros: `xyvlov.com` US → `us-cc-en`; placement principal `zuout` → `zuout.com` US / `us-cc-en`; o cadastro canônico já existente `finanzas.zuout.com` permanece separado em US / `us-cc-es`; Zyclor usa o identificador operacional `zyclor`, país DE, vertical `de-cc-de` e fallback MGS/ChatPion `g002-d`. Zuout principal e Finanzas têm Nicolas/G006 como responsável padrão desde `2026-09-18`; medium canônico explícito da fonte continua vencendo conforme a regra global, e ausência de medium usa `g006-d`. O literal não cadastrado `finanzas.zuou.com` não foi criado como domínio nem alias.
