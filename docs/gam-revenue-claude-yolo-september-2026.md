...
## Dashboard preenchida e Excel canônico — 1547692440574627921

## Supersessão permanente — mapeamentos de 14/09, 1549411618570633227

Rodolfo tornou permanentes os seguintes pares domínio+país+vertical: `openzed.com` BR → `br-car-br`; `ducapes.com` US → `us-cc-es`; `finance.ducapes.com` US → `us-cc-en`; `escalatepower.com` US → `us-cc-en`; `wavesbee.com` US → `us-cc-en`. Os placements observados `ducapes`, `escalatepower` e `wavesbee` são aliases dos respectivos domínios principais. Esta decisão supersede o limite “somente nesta revisão” das confirmações antigas de Openzed BR e Escalatepower US, sem autorizar inferência para outro país. Gestor/operação seguem os cadastros canônicos e mediums válidos da própria linha.

Rodolfo autorizou preencher `dash.mgsdigitalcorp.com` com a receita GAM de01–09/09/2026 e reenviar o Excel corrigido. Produção concluída/readback: workspace Setembro revisão79→80;513 grupos (42 inputs USD de Fincgriffin +471 fatos nativos com moeda/gestor);18 controles moeda/dia exatos; audit452; recovery bloqueado `recovery-gam-1547692440574627921`; repetição idempotente sem write. CAD permaneceu CAD na origem e foi convertido pelo câmbio provisório da competência; gastos/status/contas/despesas gerais preservados, folha recalculada como consequência esperada da receita por gestor. Detalhes e falhas recuperadas: `reports/finance-gam-revenue-import-1547692440574627921.md`. Procedimento: skill `mgs-finance-dashboard`, referência `gam-revenue-dashboard-import.md`.

Durante o readback canônico, `context/sites.md` confirmou `helixenit.net`; o primeiro Excel usava `helixenit.com` por regra genérica antiga. Foi criado o arquivo canônico `work/finance-revenue-1547692440574627921/Receita-01-09set-2026-Zeus-Final-Corrigido.xlsx`, SHA256 `98efcd052b0ff3e5552dc67c62e5e5f30b12a083e1be9694f00ad99a403770e5`:27 rótulos corrigidos, zero mudança numérica/país/vertical/gestor/moeda. A dashboard já apontava a receita para a entidade Helixenit canônica; audit458 registra a ponte de hashes. Anexo final enviado/readback em `1547701048372756541`; anexos anteriores ficam superseded, não apagados. Cephyric/Escalatepower/Mavroa aparecem como fatos de site pendente, sem cadastro/ativação ou mudança no rateio de despesas, pois isso excederia o pedido.
...
## Supersessão permanente Mavroa US — 1549069898674606352

Rodolfo confirmou `pl_digital-trust_mavroa_us` como `mavroa.com`, país US, vertical permanente `us-shein-es` — EUA, SHEIN, espanhol. Enquanto o `utm_medium` não identificar um gestor válido, a receita usa MGS/tráfego direto `g002-s`; um futuro medium canônico `g001–g006` continua vencendo para a própria linha. A regra vale para os próximos relatórios com o mesmo placement US. País novo continua exigindo confirmação separada e não é remapeado por inferência. Esta decisão supersede o limite “apenas nesta revisão” da seção abaixo somente para Mavroa US; o histórico 01–09/09 permanece preservado. A primeira aplicação permanente foi a linha de 13/09/2026, CAD `0.001441427765333875`.
