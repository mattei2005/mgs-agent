...
- Despesas da empresa passa a **Despesas Gerais**; rótulo “Despesa da planilha” removido; nomes visuais SB LeadsOn Hub / SB Tech Bot / SB Wire Fee; ordem alfabética e linhas compactas. Identidades e valores originais preservados.
...
## Fechamento de gastos por total mensal — correção de Rodolfo neste turno
Fonte: continuação direta de Rodolfo na thread 1545426987756298340, durante o pedido 1546682010066489394 após capturas 1546685677599465513. Rodolfo explicou: “na semana do pgto eu confiro o total e mudo o valor do ultimo dia apenas para bater o total ... se o total bater entao ta certo.”
Regra ativa: conferir total mensal de cada conta na moeda original. Ajustar o último dia para fechar esse total é prática legítima e não deve ser classificado como divergência/anomalia ou exigir paridade por dia. O requisito de paridade diária da migração geral não é critério de aceitação dos gastos publicitários fechados por esta regra; supersessão limitada a essa conferência. Nunca confundir total mensal geral entre contas/moedas com total de cada conta.
Na futura coleta, preservar bruto da API e ajuste de fechamento auditável, sem desfazer o último dia ajustado manualmente. Depois de conferido, só uma mudança no total mensal da fonte gera pendência; redistribuição entre dias com o mesmo total não reabre a conferência. Topfeed julho 12319.31 API/Sheet/captura satisfaz o critério. Não há autorização para corrigir linhas ou meses históricos automaticamente.
