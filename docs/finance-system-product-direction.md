...
Status: aplicação própria em produção, autenticada e operando sobre PostgreSQL 18 após confirmação crítica 1545934831664242748. **O preenchimento diário de receitas GAM e gastos Meta/Google está automatizado e auditado; isso não automatiza fechamento, pagamentos, taxas efetivas, inválidos confirmados ou decisões de cadastro/status.** A planilha permanece fonte oficial para as competências e regras ainda dependentes dela.
...
- Agosto/2026 usa cutoff 31/08; setembro/2026 avança diariamente pelo estado durável reconciliado de receita e gastos, sem uma data fixa neste documento; competências futuras começam sem cutoff. O corte avança somente quando receita e gastos do mesmo dia estão conciliados. Realizado usa apenas fatos até o corte e apropria Despesas Gerais/funcionários proporcionalmente. Estimativa projeta o operacional até o corte e inclui as despesas mensais uma única vez.
- Todos os sites aparecem em uma tabela com `Rateio das Despesas Gerais`: `Participa` ou `Não participa`. `Não participa` mantém receita e gastos nos totais, mas fica fora do rateio. Cephyric, Escalatepower, Mavroa e Boostingecon são MGS/G002 sem rateio desde setembro; receita original CAD é preservada. Boostingecon pode receber receita orgânica residual e usa exclusivamente `g002-d` / `us-cc-en` conforme Rodolfo `1548317688051277918`.
...
Produção validada em 24/24 competências, desktop/mobile, owner e os cinco gestores ativos; botão real de julho retornou fonte já atualizada, seis documentos e correção CAD preservada. O baseline dessa entrega permanece na referência operacional; a contagem atual de testes deve ser lida no último relatório de auditoria integral, não congelada neste documento. Fonte operacional: `mgs-finance-dashboard/references/current-realized-cutoff-site-status-history-refresh.md`; evidência `/root/mgs-agent/work/finance-current-policy-1547732274936553532/`.
...
- Crons ativos: intake read-only às 08:03/08:08/08:18/08:28, gastos às 09:03 e finalização da receita às 09:22/09:31/09:41 Eastern. Zero colisões operacionais na expansão global de oito dias; finalização exige state e ledger de gastos na mesma data.
...
- A regra de remuneração está confirmada e ativa desde agosto/2026: gestor ativo recebe o maior entre o piso mensal de R$3.000 e 7% do resultado líquido; a partir de R$100.000 de resultado, recebe 10% sobre todo o resultado; gestor inativo recebe zero. O piso não é somado à comissão. A visão do gestor distingue comissão calculada e remuneração devida; Pagamentos mostra somente a obrigação efetiva.
...
## Movimento, extratos, aprovações e piloto Nicolas — histórico supersedido pelo rollout geral
...
Esta decisão iniciou o piloto com Nicolas e supersedeu a restrição histórica de leitura de Despesas apenas a Rodolfo, **somente no aplicativo financeiro**. O piloto foi posteriormente expandido e validado para Joe, Isliago, Kelly, Ícaro e Nicolas; não deve mais ser usado como trava de rollout. Isso não muda autorizações de agentes/Discord nem aposenta as planilhas. O login Rodolfo e sua credencial existente permanecem intactos.
